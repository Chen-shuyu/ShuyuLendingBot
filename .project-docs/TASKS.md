...
**狀態（2026-08-16 夜間第二段）**：main 在 `123c606`（PR #28 已合併並自動部署）。
分支 `fix/pricing-market-floor` 待開 PR——內含今晚的文件規劃（D031／D032）
與**定價事故的修正**（D033）。
...
- [ ] **P1-4　🔴 重掛決策要看排隊位置，不能只看利率差**（2026-08-16 新增，見 DECISIONS.md D031）
