...
1. **確認掛單能不能成交**——目前利率年化約 19%，市場在 5～10%，很可能借不到。
   對策見 TASKS.md「明天第一件事」：調低 `premium_rate`，或改用 FRR 浮動掛單
   （後者會動到 D011 的「每輪全取消重掛」設計前提，要先寫決策）。
2. **接 `earnings_daily` 的資料來源**（Bitfinex ledger 端點）——
   要有真實利息入帳才驗證得了，所以必須等第 1 項成交之後才做得下去。
3. **`maxtolend` 升級為含已放貸的真實總曝險版**——目前只管本輪掛單總額，
   等資金規模放大、或同時有多筆在放貸時才會真正需要。
4. **B5／B6**（錯誤分類、測試替身校正），不擋前三項，但 B6 建議排在第 2 項之前做。
