...
**條數會過期**（D042、D043 加進來時都沒有更新到，2026-08-25 才發現差了兩條）。
**要數就跑**：`grep -c '^## D0' .project-docs/DECISIONS.md`
——理由同 STATUS 對「程式碼裡有幾處 `D0xx`」的處置：**寫死在文件裡的數字，
每加一條就過期一次，而沒有人會為了它去重數。**
...
| D027 | 測試替身一律取自真實回應，不得自己編「乾淨」版本 | 🟢 **08-29 補第 4 點**：八支實打，五支跟替身不一樣（credits 已補齊，loans 仍抄不到） |
...
| D043 | 決策落地：`pricing_decisions` 每評估一輪一列，寫在日誌那一行的旁邊 | 🟢 **已部署（08-24 22:48）；08-26 00:14 五條驗收全數通過** |
| D044 | 餘額不足是「本輪略過」，分類看訊息不看例外型別 | 🟡 **已部署（08-24 23:29）；可能永遠不觸發，「沒發生」不等於驗收通過** |
| D045 | 等待估計要有校準報告：實測八筆全部高估，總量比 4.25 倍 | 🔴 **08-29 開獎：預測輸了（借 1.98h，實得 3.02%）**，而它顯示**誤差幾乎全來自 `P` 不是 `W`**——D045 一路在調整比較小的那一項，真正的槓桿是 D1 |
| D046 | 重掛政策：往上調價目前**完全沒有判準**，而該用的訊號不是閒置時間 | 🟡 **形狀已定，數字等 M2**（M1-c 先落反事實資料） |
| D047 | **乾旱期會讓等待估計自己變好看**：右設限的補值被窗尾夾住 | 🔴 **機制已量出（08-30），刻意先不修**——第三個誤差源，與 D1 同向且相乘。⚠ **追記：只在乾旱前半段成立**，久了會掉頭 |
| D048 | **M2 的驗收標準跑不起來**（比它依賴的那張表早寫了六天），換成 `--verify` 逐點對照正式紀錄 | 🟢 **已實作**（M2 第 1 步）；⚠ 換上的這條比原本那條**弱**，抓不到「策略本身錯了」 |
| D049 | **成交模擬上線，D1 第一次有答案**：方向是「48 太高」 | 🟡 **方向成立**；🔴 **它列的兩條「不能改」的理由已被 D055 推翻**（都是單一起跑點的假象）。「`P` 在替 `W` 擦屁股」那一條仍然有效 |
| D050 | **A2-b 的答案是否定的**：六種重掛政策分不出勝負，成因是**候選價位是落後訊號** | 🔴 **不該繼續挑門檻，該換訊號**；⚠ **結論仍成立但理由被 D054 換掉**——那是單一起跑點的結果 |
| D051 | **帳本接上了（P2-2）**，而它把前面每一個績效數字都往下修 | 🟢 已實作；🔴 **7.99% → 6.54% → 帳本 5.34%**——每一層推論都偏樂觀，方向全部相同 |
| D052 | **帳本同步放迴圈層並吞掉所有例外**：報表不可以把管真金的機器人弄停 | 🟢 已實作；⚠ **寬鬆的 `except` 當場咬了我一口**——漏掉一行 import，測試全綠、機器人照跑 |
| D053 | **日結收益摘要：判準是資料不是時鐘**；狀態存新表 `bot_kv` 而不是加欄位 | 🟢 已實作（P2-4 完成）；**不寫「實得年化」四個字**——那個詞有定義，這則訊息算不出來 |
| D054 | **訊號偵測得很好（5/5、零誤報、門檻不敏感），但拿它降價賠 1.97pp** | 🟡 上下半場同等重要；🔴 **附帶推翻了 D049／D050 的比較方式**——單一起跑點的比較會被「相位運氣」主導，符號都可能相反 |
| D055 | **用對方法重做之後，D1 的證據夠了：那個 48 該改** | 🟡 **證據已足夠，改不改由使用者決定**（定價路徑）；🔴 推翻 D049 的兩條「不能改」的理由，並收窄 D054 的推論 |
| D056 | **合約天期與算式裡的 `P` 拆成兩個設定**，`P` 改成 12 小時 | 🟢 **已上線**（D1 收尾）；⚠ D055 建議的改法**本身是錯的**（`offer_period` 身兼二職、整數、交易所最短 2 天） |
| D057 | **持有時間量測把同一筆錢數了兩次**：`loan` 與 `credit` 是同一筆資金的兩個狀態 | 🔴 **已確認的量測缺陷**，17 筆其實是 14 筆；平均持有 16.93h → **20.45h**、完成率 35.3% → **42.6%**。⚠ 它是 D055／D056 選那個 12 的輸入之一（**中位數幾乎沒動，所以 D056 方向不變**） |
| D058 | **使用者的假說：`P` 可能不是常數，而是 `r` 的函式**（越便宜借越久） | 🟡 **方向與資料一致**（`rho` = −0.29）**但強度遠不足**：n=14、只有 7 個相異利率，**同一個價位內部就差 38 倍**。列為待量測，接縫（`HoldModel`）早就在了。🔴 **2026-09-05 升級為 D062**：`rho` −0.29 → **−0.383**，而且它不只是「值得量」——**目標函數在定義上就是自我矛盾的** |
| D059 | **接下來的工作分三條車道**，判準是「會不會改變送到交易所的東西」 | 🟢 **排程判準**：甲道（行為零變化）隨時可做、乙道要等完整循環、丙道要拍板。🔴 觀察 D056 是以**循環**計不是以小時計——錢鎖在部位裡時 `pricing_decisions` 一列都不寫。🟡 **2026-09-05 車道內容已被 D060～D063 重排**（判準本身仍有效） |
| D060 | **D056 的驗收：它有效（實得年化 5.72% → 7.75%、利用率 98.8%），但它的理由是錯的** | 🟢 乙道結案；🔴 `P=12` 被實測否定（8.76～9.00% 那一帶**借滿 48h**）。**它賺到的是利用率，不是它宣稱的持有時間**；瓶頸已從「借不出去」換成「賣太便宜」 |
| D061 | **選中哪個價位目前是由浮點雜訊決定的**：前兩名差 4×10⁻⁵ pp | 🔴 已確認的缺陷。成因有二：高原（8.3～9.0% 只差 0.19pp）＋ **平手系統性偏便宜**（`>` ＋升冪）。改法＝**高原容差 ＋ 取最高價**；**它不依賴 `P(r)`，是唯一可以先做的定價改動**。🟢 **2026-09-05 已上線**（PR #61，`ev_plateau_tolerance_pct: 0.5`）——🔴 **量出來的答案跟直覺相反**：1.0% 贏 94/113 個起跑點卻**平均是負的**，且符號在持有模型與切半之間都會翻；**只有 0.5% 全部同向**。🔴 **上線當天就咬了一口**：三個報告端各自 `max(...)` 重算「誰被選中」，會**一致地**記下一個機器人沒掛過的價格（D026 家族第六次現身）——已修，來源收斂成 `last_chosen`。✅ **2026-09-06 三條驗收全過**：實際選 **8.9279%**（舊行為重播是 8.7600%）、4 列選價完全一致、零額外重掛 |
| D062 | **目標函數自我矛盾**：用常數 `P` 最佳化 `r`，而 `P` 是 `r` 的函式 | 🔴 **D058 從假說升級為定義問題**（`rho` −0.29 → **−0.383**）。`P` 填 12／24／48／72 → 選價 8.76／8.93／9.40／10.22%。🔴 **回測也不能當裁判**：`empirical_hold()` 假設 `P ⊥ r`，等於用結論證明結論。🟢 **2026-09-05 已修（PR #64），而結論沒有翻**：兩個裁判都指向 `P=12`，換裁判動 0.065pp、換假設動 1.267pp（**差 19.4 倍**）。🔴 **而「結案」是有條件的**——選價 8.93% 在分界線 9.31% 底下，那一階從沒被跨過；**是 D056 把作業點壓到線下，才讓這個疑慮無關緊要**。**D062 因此是 D063 的前置條件，不是獨立的改動** |
| D063 | **所有樣本都是模型自己選出來的**，所以量再久也不會有新資訊 | 🔴 **D045／D047／D058 三者共同的上游病因**。12 個等待樣本 10 個擠在 8.76～9.50%；M1-c 長了 187 列而 **186 列 `hold_matched`（零資訊）**。決定：**加入探索**＋**M1-c 換問題**；🔴 **甲-2 因此要排在探索之後**。🔴 **2026-09-05 追記：3-2（M1-c 換問題）不做**——沒掛出去的價位結果不可觀測，換候選只是把模型的意見存進 DB；事後用 K 線檢查那件事 `--simulate` 已經在做了。**併進 3-1** |
| D064 | **驗收工具靜默失效了六天**：`--verify` 拿今天的設定去重跑當初的決策 | 🟢 已修（`pricing_knobs_json` ＋ 每列用自己的設定重播，**40 列 → 40/40 一致**）。🔴 **D048 的同一個病第二次現身**——這次更難發現，因為它從頭到尾都有輸出，而「不一致」跟「工具壞了」長得一模一樣。**驗收標準本身不會被驗收**。✅ **2026-09-06 第一列自帶旋鈕的紀錄落地**（完整九個旋鈕），不再靠慣例回推 |
| D065 | **實得年化的分母不是「本金」，是「已部署資金」**（錢包 ＋ 場上掛單 ＋ 部位） | 🟢 已實作。🔴 **推翻了 `_maybe_sync_earnings()` 自己寫的那句「本金不填」**——那句話對本金成立，但**空窗正是我們要懲罰的東西**，把閒置的錢從分母拿掉等於讓「借不出去」變免費。**三項都是觀測值不是推論**。順帶：帳本多一段毛／淨對帳（**80.3% 對抽成參考線 85%**）——在此之前**沒有任何東西在看帳本** |
| D066 | **`median` 是更準的預測值，卻是更差的決策規則** | 🟢 旋鈕已上線、**預設不變**。🔴 **把 D045 第三則追記的方向反過來**：`median` 跨 114 個起跑點**輸 1.0pp**——長期年化是 `E[利息] ÷ E[週期]`，**那要用平均**；D045 量的是點預測準度，**而且在成交那半的子樣本上**。🔴 意外發現：**現行的 `window_tail` 比整批丟掉還低估**（40.57h 對 46.41h） |
| D067 | **等更多單子不會改變答案，而探索的檢定力差了 20 倍** | 🟢 **停在這裡的決定**，三條獨立依據：①新樣本全落在便宜組（D056 後昂貴組 **+0 段**）；②**連「每筆都借滿 48h」都不能讓最佳 `P` 離開 12**——`assumed_hold_hours` **不是持有時間估計，是「多討厭等待」的旋鈕**；③沒有不花錢的替代路徑（345 USD 佔成交量 0.0002%）。🔴 原規劃「每 20 次探索 1 次」湊到 70 段要 **50 個月**。**重啟條件寫在 ADR 裡**。🟡 **2026-09-06 追記：條件 2 可能正在發生**（12 小時換手 3 次、持有 1.2～1.3h，而先前中位是 16.29h）——**但 n=2 不算數**，判準寫在 ADR 裡 |
| D068 | **一個中文檔名讓「只動了文件就別部署」的閘門失效** | 🟢 已修（`core.quotePath=false`，**白名單一個字沒改**）。🔴 **同一症狀第三次，但病因完全不同**：前兩次是涵蓋範圍，這次是 `git diff` 把非 ASCII 路徑**加引號**。⚠ **閘門是 fail-open，所以它失效的樣子就是「多部署一次」——與 D064 同一個形狀** |
| D069 | **放寬合約天期到 30 天**（`offer_period` 2 → 30、`assumed_hold_hours` 12 → 48） | 🟢 **2026-09-12 上線**，是 D030 那句「驗證階段不要把資金鎖住」的**到期解除**，不是新發現。🔴 **先修正一個被用了一週的錯數字**：STATUS 的「實得年化 7.75%」是拿**入帳日數**當分母算的，誠實的 7 日滾動是 **6.83%**、上線至今 **6.14%**——D067 的重啟條件 3（跌破 7%）**其實已經成立兩週**。🔴 **9% 的目標從一開始就差一個 0.85**：Bitfinex 抽 15%，9% 淨年化需要 10.59% 名目 ＋ 100% 利用率。**選價 9.125% → 10.500%**（高原，命中 46 次；**不是**那個命中 7 次的 11.268%）。⚠ **賭的是持有時間會變長，而那件事現在量不到**（24 筆有 9 筆被 48h 上限截斷，KM 在 48.6h 仍有 37.5% 存活）。🔴 **K 線缺根讓等待被低估 1.25 倍**，修正後損益兩平點 15.4h → **19.2h**，落在實測中位（16.29h）之上、實測平均（23.24h，本身是下界）之下——**餘裕很薄，靠的是截斷論證**。停損：7 日滾動跌破 **6.0%** 就改回天期 2。🔴 **追記：修報告時又抓到兩個鄰居**——曆日數少算一天（年化高估 14%）＋ 毛／淨對帳兩半涵蓋不同期間（`--since` 一短就印出 27.1% 的假警報）。兩個都修了，分解值與帳本差 0.00～0.06pp。**這支報告以前一條測試都沒有**，已補 `test_sync_earnings.py`，測試 936 → 960 |
| D070 | **長天期第一週的成績，以及一個我用錯的判準**（維持天期 30 跑滿一個月到 2026-10-14，D069 的停損撤銷） | 🟢 **2026-09-19 定案**。🔴 **先記一個我自己用錯的判準**：D069 的硬停損 6.0% 校準自「現有設定**成熟後**的區間 6.10%～8.02%」，而那三個字把短天期前六個窗（4.03%～5.64%）排除了——**短天期自己的第一週是 4.03%，比長天期第一週（5.14%）還差**，那條線在 08-23 就存在的話短天期活不過第一週。**校準的樣本期間與適用的樣本期間不同，而那個「不同」躲在一個形容詞裡。** 第一週量到：利用率 **90.2% → 51.1%**、名目 **8.96% → 10.87%**（那 2pp 確實拿到了）、等待 **1.89h → 15.64h**（中位 112 倍）。🔴 **D069 賭的截斷論證量到了而且是反的**：上限放到 720h 後三筆持有 2.40／32.17／25.36h，**沒有一筆碰到舊的 48h**；`hold_report` 同向（便宜組中位 45.08h vs 昂貴組 11.62h）——**決定持有時間的是價格，不是天期**（但 n=3，是觀察不是結論）。副作用兩個都不修：`estimate_wait()` 在天期 30 **低估 3.1 倍**（天期 2 下是高估，**方向隨天期翻轉**）、`market_rate()` 冷門天期樣本不足觸發 **298 次**但**實際被擋住 0 次**。排除市場因素：period=2 成交中位這週 5.64% → **7.65%**（市場變好了）。🔴 **最便宜的那個數字**：天期 30 只佔簿子 **0.4%**、天期 2 佔 **71.4%**——`market_snapshots` 從 08-23 起就查得到，**不必花一週真金去問**。**期間一行程式不改**（測試期間改程式正是會讓測試不公平的那件事）；代價 **0.20 美元**。10-14 的判準已先寫死在第七節 |
...
- 附帶發現（原記為 TASKS.md B5，**已於 2026-08-24 修正，完整敘述見 D044**）：
  ccxt 把這個**餘額不足**的錯誤歸類成 `AuthenticationError`，
  於是日誌寫的是「建立放貸掛單認證失敗」。訊息會把人引去查 API 金鑰，
  而真正的問題在金額——而且分類成 `FatalError` 會讓機器人**永久停止**。
...
### 追記（2026-08-29）：第一次真的照做，抓到四件替身不知道的事

分支 `feature/calibrate-test-doubles`（TASKS.md 的 B6）。三個公開端點 ＋ 四個
唯讀私有端點各實打一次，把型別照抄進替身。**七支裡有四支跟替身不一樣**：

| 端點 | 替身以為的 | 真實回應 |
|---|---|---|
| `public_get_ticker_symbol` | 4 欄、原生 `float`／`int` | **17 欄全字串**，且 **[13][14] 是 `None`**；文件只列到 [15]，實際多一欄 [16] |
| `private_post_auth_r_funding_offers_symbol` | **16 欄**，[10] 是 `None` | **21 欄**，[10]=STATUS 是 `'ACTIVE'` |
| `fetch_balance` | `{"USD": {"free": 344.12}}` | 還有 `used`／`total`／`info`；**可用餘額八位小數** `0.00175079` |
| `fetch_balance` 的 `info` | （不存在） | **指定 `type='funding'` 也照樣混著別的錢包**（這次第二列是 `exchange`／`UST`） |

**最值得記的一條是 offer 陣列的 16 vs 21**：`_parse_offers()` 的 docstring
從 2026-08-16 起就寫著「21 欄，全部是字串」——**程式碼知道真相，替身不知道，
而兩邊從來沒有人去對過**。這正是 D027 想擋的那種落差的完整型態：
它不是「沒有人查過真實回應」，而是**查過了、寫進了註解、卻沒有流進替身**。
目前讀到的最大索引是 15 所以還沒出事，但哪天要靠 STATUS 濾掉非作用中的掛單，
測試會拿 16 欄替身裡的 `None` 走完全程然後亮綠燈。

**所以 D027 要補第 4 點**：

4. **註解裡寫下的真實形狀，要有一條測試直接吃未加工的真實列。**
   「照抄」與「照抄後還是綠的」是兩件事——把實打回應存成 `REAL_*` 常數，
   讓至少一條測試吃它，形狀漂移才會變成紅燈而不是註解與程式的沉默分歧。

**兩支抄不到，原因各自不同**（記在 TASKS.md 的 B6，不重複）：
`funding_credits`／`funding_loans` 打的當下沒有部位在生息，回空陣列——
**下次有部位時補打即可**；`submit`／`cancel` 是寫入端點，不動真錢就抄不到，
這一點 D027 本文已經預先寫明。

### 追記（2026-08-29 21:5x）：`funding_credits` 補打成功，第 4 點又抓到一件

08-29 20:52 成交的部位 `464812689` 還在生息時補打了一次唯讀查詢，
**credits 那一半不再是猜的**（PR #48）。三件替身原本不知道的事：

1. **credits 是 22 欄**，`[21]=POSITION_PAIR` 這次拿到 `'tBTCUSD'`
   ——借款人把這筆錢用在哪個交易對上。`_parse_positions()` 的 docstring
   一開始就寫著這件事，這是第一次拿真實回應對上。
2. **`[8]=RATE_TYPE` 真實是 `'FIXED'`，替身留 `None`。**
   **這是同一個坑的第三次現身**：第一次是 `REAL_FUNDING_OFFER[10]` 的
   `'ACTIVE'`，第二次是 `fetch_balance` 的 `info` 混著別的錢包。
   欄位存在、替身用 `None` 走完全程然後亮綠燈——哪天要靠 `RATE_TYPE`
   濾掉 FRR 浮動利率的部位就會出事。
3. **`[3]`／`[4]`（MTS_CREATE／MTS_UPDATE）有值且與 `[13]` 同值**，替身只填 `[13]`。

**這一列走完整條鏈路核對過**：交易所原始列 → `_parse_positions()` →
DB 的 `funding_positions`，`amount` 345.02、`rate` 0.0003、`period` 2、
`opened_at` 1788007933000（= 2026-08-29T20:52:13+08:00）四處完全一致。
**這是 B6 第一次做到「整條鏈路」而不只是「端點回應」。**

🟡 **`funding_loans` 仍抄不到**：它要「已被借走但還沒用掉」的時刻，
而補打當下錢已經被借走了。`length=21` 這個預設值仍取自官方文件，不是抄來的。
...
### 驗收結果（2026-08-26 00:14，五條全數通過）

部位 `464372858` 於 **00:14:07** 收回（到期 08-25 23:50，逾期 24 分），第一列就落在那一輪。

| # | 條件 | 實測 |
|---|---|---|
| 1 | 出現第一列 | ✅ `id=1`、`decided_at` `2026-08-26T00:14:12+08:00` |
| 2 | `snapshot_id` 指得回同一輪 | ✅ 327 →`market_snapshots.captured_at` 與 `decided_at` **字串完全相同** |
| 3 | `chosen_rate` 等於日誌那一行 | ✅ `0.00024971` = 年化 9.1144%，日誌寫「選中年化 9.11%」；`mean 8.04h／median 2.5h／p75 13.5h／命中 66 次／對照 5.47%` 五個數字全部對得上 |
| 4 | 候選陣列長度＝`candidate_count` | ✅ 115 = 115，已排序，`chosen_rate` 在集合內 |
| 5 | 資金再借出後不再多一列 | ✅ 00:30:51 成交後**又跑了 127 輪巡檢，仍是 2 列** |

**第二列（00:24:19）不是多寫的**：那一輪掛單還在場上，策略照樣評估過，
所以照設計要寫一列；該輪走 `_plans_match()` 的 2% 容差維持不動，沒有重掛。
**「評估過就寫」與「掛單才寫」是兩件事，這裡採的是前者**——D3 要回答的
「候選集少了誰」需要每一輪的候選集，只有掛單那幾輪會漏掉重估的過程。

**同時證實了 D041 的保護**：127 輪走餘額守門檻的巡檢，一列都沒有多寫。
...
## D044 — 餘額不足是「本輪略過」，而分類要看訊息、不看例外型別

- 日期：2026-08-24（驗收條件補記於 2026-08-25 00:00）
- 背景：這是 **D025 的附帶發現**，在待辦清單裡當了 9 天的 B5。
  Bitfinex 回的是
  `Invalid offer: not enough USD balance available in deposit wallet`
  ——一個**金額**問題——但 ccxt 把它包成 `ccxt.AuthenticationError`，
  於是它走 `except ccxt.AuthenticationError` 那一支，被分類成 `FatalError`。
  後果有兩層，而**第二層比第一層貴得多**：
  - 日誌寫「建立放貸掛單認證失敗」，**把人引去查 API 金鑰**，而問題在掛單金額
  - `FatalError` 讓機器人**永久停止**（離開碼 2、`RestartPreventExitStatus=2`
    不重啟）。餘額問題下一輪重算金額就可能自己好，卻換來一整夜的停機
- **為什麼現在做（不是「反正不急」）**：部位 `464372858` 於 **2026-08-25 23:50**
  到期，資金收回、重新掛單那一刻正是最可能踩到金額問題的時候，而那是半夜。
  這一項在 TASKS 裡本來標著「不急」，理由是「金額 bug 修掉之後這條路徑不會再被觸發」
  ——**那句話講的是機率，不是後果**。它的後果是整夜停機。

### 決策：三件事，其中兩件是取捨不是實作

1. **分類看訊息、不看例外型別。** 型別在這裡本來就不可信：ccxt 把金額問題包成
   認證錯誤，而它下次改版包成哪一種沒有人保證。所以 `AuthenticationError` 與
   `ExchangeError` 合併成一個 `except`，先問訊息、再決定分類。
   **同一個問題要問兩次**：例外那條路，以及回應信封（`status != "SUCCESS"`）那條路
   ——只修一條的話，回應形式一改就漏掉一半。
2. **判斷條件刻意收窄：只認 `not enough` 與 `insufficient`，不單獨認 `balance`。**
   兩個方向的誤判**代價不對稱**：
   - 把餘額問題當成致命 → 停機一夜（就是現在要修的病）
   - 把金鑰問題當成「本輪略過」→ 機器人**拿無效金鑰空轉一整天**，
     而且每一輪都自我安慰地寫「本輪略過」，看起來像正常運作
   後者更貴，因為它**看起來沒事**——這是 D026「靜默失效」家族的形狀。
   所以寧可少認幾種餘額訊息（頂多多停一次機，有告警、有人會發現），
   也不要多認一種認證訊息。「查餘額失敗」之類的訊息也含 `balance`，所以它出局。
3. **留痕照舊，但不推 LINE。** 只改 client 的話會有一個沒人要求的退化：
   這種拒單會從「落一筆 `failed` ＋ 推一則通知」掉成**只剩一行日誌**
   ——因為 `bot_engine` 的留痕分支只接 `RetryableError` / `FatalError`。
   所以引擎補一條 `SkipCycleError` 分支：**照樣落帳，但不推播**。
   理由是 D024 的額度：致命拒單罕見（實單至今一次），而餘額不足**可能連續好幾輪
   發生**，每月 200 則的額度撐不起這種事件。

### 這個改動拿掉了一條停機路徑，所以風險寫在這裡

`loan_offers` 的 `status='failed'` 現在有**兩種意義**：致命拒單（機器人停了）
與可略過拒單（機器人還在跑），**`detail` 欄位是唯一的區分**。
日後統計「掛單失敗率」時把兩者加在一起，會得出一個沒有意義的數字。

### 什麼事件發生才算「被執行到」

**照 D041 那條該推廣的做法**：合併時就寫下驗收條件，不等部署完再回頭想。

⚠ **這一項與 D043 有一個關鍵差別：它可能永遠不被觸發。**
D043 的第一列是**必然**會出現的（部位一到期、資金一回來就會寫），
而 B5 這條路徑要「掛單金額真的超過餘額」才會走到，而 D025 的金額 bug 已經修掉了。
所以：

- **「至今沒出現過餘額不足」不可以讀成「B5 驗收通過」。**
  那只是「還沒有機會犯錯」，而不是「有機會犯錯而沒有犯」
  ——中間隔的正好是一次驗收（D041 的原話）
- **不要為了驗收去製造一次拒單**：那要動真錢、要故意掛超額單。
  這一項接受「等它自己發生」，狀態就一直掛在 🟡
- **最可能觸發的時刻是 2026-08-25 23:50** 部位收回後的第一輪重新掛單

驗收條件（下一次真的出現餘額不足的拒單時，五條一起看）：

1. 日誌那一行是 **WARNING**，內容是
   `建立放貸掛單失敗：融資錢包餘額不足（…）`（或走信封時的
   `建立放貸掛單被拒：融資錢包餘額不足（…）`），
   **而且整份日誌不再出現「建立放貸掛單認證失敗」**
2. 緊接著有一行 `本輪略過：融資錢包餘額不足，本輪不掛單：…`
3. **機器人沒有停止**：下一輪巡檢照常出現在日誌裡（間隔 600 秒），
   容器狀態仍是 `healthy`
4. `loan_offers` 多一列 `status='failed'`、`offer_id IS NULL`、`detail` 含「餘額不足」；
   **LINE 一則都沒有收到**
5. 🔴 **反查誤判（這一條是這次降級的全部風險所在）**：同一輪日誌裡的可用餘額數字，
   要**真的**小於當輪掛單金額。**如果餘額其實是夠的，那就是誤判——
   立刻回退這個改動**，因為那代表某種非餘額問題的錯誤訊息裡含著那兩個字樣。

查法：

```sql
SELECT id, created_at, amount, rate, status, detail
FROM loan_offers WHERE status = 'failed' ORDER BY id DESC LIMIT 5;
```

```bash
grep -E "餘額不足|認證失敗" logs/*.log | tail -20
```

### 這一項不做什麼

- **不改 `FatalError` 對真正認證問題的處置**：金鑰無效、簽章錯誤、權限不足
  一律照舊停機。測試用 `apikey: invalid`、`Invalid X-BFX-SIGNATURE`、
  `permission denied` 三種訊息守住這件事
- **不加重試**：掛單不重試是 D013 的決定，這裡只改分類，不碰那條線
- **不推 LINE**：理由見上面第 3 點
- **不動 `_split_amount()`**：D025 的金額計算沒有改，這一項只處理「萬一又算超了，
  機器人該怎麼反應」


## D045 — 等待估計要有校準報告：實測五筆全部高估，平均 3.9 倍

**日期**：2026-08-26　**狀態**：🟢 有效（量測，不改策略）

### 起點是一個使用者提的問題

> 「為什麼掛了幾次都是同樣的利率 0.00024971 出去？看起來應該還可以掛到更漂亮的利率吧？」

問題問得對，而查下去發現**兩個各自獨立的答案**——一個是機制，一個是這條決策的本體。

### 一、為什麼老是同一個數字（機制，不是 bug）

`choose_rate()` 的候選集是 `sorted({self._quantize(high) for high in highs})`
——**168 小時窗內出現過的 K 線最高價，量化後去重**。於是：

1. **一根 K 進到窗裡就待滿 7 天**，候選集每小時只換掉一根，本來就幾乎不動；
2. **巡檢 600 秒一輪，K 線一小時才換一根**，所以同一小時內的輸入完全相同、
   輸出必然相同——`pricing_decisions` 的 id=1 與 id=2（00:14 與 00:24）
   每一個欄位的值都一樣，就是這件事；
3. **跨天也還是同一個數字**：08-23 23:04 與 08-26 00:14 相隔 73 小時，
   兩次都選 `0.00024971`，因為那根 K 還在窗內，而且——見下一節——**沒有東西推得動它**。

這一段是設計的必然結果，不是缺陷。**真正該追問的是第 3 點的「推不動」。**

### 二、期望值曲線是平的，所以「選中」這個動作幾乎不帶資訊

用 316 根真實 K 線離線重跑 `choose_rate()`（沒有改任何程式），
把 125 個候選的實質年化全部印出來之後：
...
**從 8% 到 11.35%，實質年化全部落在 6.9%～7.9% 之間。**
第 1 名與第 2 名差 **0.0005 個百分點**，前八名的跨距只有 0.19 個百分點。

也就是說：**這不是「挑出最好的價位」，是在 125 個實質上打平的選項裡取 argmax。**
一個平到這種程度的目標函數，argmax 落在哪裡由雜訊決定——
**D3 記的「一根 K 滾出窗，價格目標就自己跳一階」正是同一件事的另一個切面**，
兩者可以合併理解：不是價格目標會亂跳，是它本來就沒有被決定。

⚠ **這不構成「所以改掛 11.3%」的理由**：曲線平代表**模型分辨不出來**，
不代表高價比較好。分辨不出來的時候往高處押，賭的是模型的誤差方向，不是模型的結論。

### 三、模型的誤差方向第一次被量到了——而且指向高利率

`offer_wait_forecasts` 從 D038 起記下「掛出去那一刻我們以為要等多久」，
`loan_offers` 與 `funding_positions` 記著實際成交時間。**兩者從來沒有被對照過。**
對照結果：

| 掛單時間 | 年化 | 預估平均等待 | 實際等待 | 高估倍數 |
|---|---|---|---|---|
| 08-20 15:15 | 9.50% | 6.10h | 3.93h | 1.6× |
| 08-21 16:09 | 9.50% | 6.32h | 0.70h | 9.0× |
| 08-21 19:11 | 9.50% | 6.29h | 3.33h | 1.9× |
| 08-23 23:04 | 9.11% | 8.69h | 0.77h | 11.3× |
| 08-26 00:14 | 9.11% | 8.04h | 0.28h | 29.0× |

**五筆全部高估**，平均 7.09h 對實際 1.80h＝**3.9 倍**（逐筆倍數 1.6×～29.0×，離散很大）。

**這件事之所以重要，是因為 `W` 正是算式裡懲罰高利率的那一項**：

    實質年化 = r × P ÷ (W + P)

`W` 被系統性放大，等於高利率被系統性懲罰過頭。使用者的直覺「應該掛得到更漂亮的利率」
**方向與這個量測一致**。

### 四、但證據還不足以改價，理由有三條

1. **五筆全部落在 9.11%～9.50%**，也就是模型自己選出來的那個窄帶。
   **9.5% 以上一個樣本都沒有**，而那正是要外推過去的區間。
2. **唯一的反例就在高價那一端**：08-19 05:03 掛 9.78%，在場 **34.2 小時沒成交**，
   被下一張單取代。它早於 `offer_wait_forecasts` 這張表，比不出高估倍數
   ——但它是我們掛過最高的價，也是唯一沒成交的那一張。
3. **只看成交的單會系統性看低等待**：沒成交就被取代的單是右設限樣本，
   丟掉它們，「實際等待」必然偏短。上面那張表就有這個病，
   所以校準報告必須把右設限單獨列出來——這是 D040 已經踩過的同一個坑。

另有一個方向相反的偏誤要一起記住：`P` 寫死 48 小時而實測完成率只有 51.8%（D040），
**那一項是把高利率評得過高**。兩個偏誤反向、量級都還沒定，
**淨效果不能用手算宣告**——那正是 M2 的題目。

### 決定

**建校準報告，不改策略。**

- 新增 `core/wait_time.py` ＋ `scripts/wait_report.py`，與 D040 的
  `core/hold_time.py` ＋ `scripts/hold_report.py` **成對**：
  一個量算式裡的 `P`（實際借多久），一個量算式裡的 `W`（實際等多久）。
  **算式的兩項到這裡都有了自己的校準報告。**
- 報告必須把**沒成交就被取代的單**當右設限單獨列出，不併進平均（理由見上面第 3 點）。
- **`ev_window_hours`、`hold_hours`、`minimum_rate` 一個都不動。**

### 這一項不做什麼

- **不改 `estimate_wait()`**：知道它高估，不等於知道該怎麼改。
  高估倍數逐筆從 1.6× 到 29.0×，用任何一個代表值去除，都是拍一個新數字（D036）。
- **不改掛單利率、不加「往高處靠」的偏置**：見第二節最後那句。
- **不做 M2**：這份報告是 M2 的輸入，不是 M2。
- **不推 LINE**：校準是分析材料不是事件（同 D040、D043 的額度判斷）。

### 驗收條件（照 D041／D043 的做法，合併時就寫下來）

1. `python3 scripts/wait_report.py` 在正式 DB 上跑得出報告，
   **逐筆列出的成交等待與本節第三張表一致**（3.93／0.70／3.33／0.77／0.28h）
2. **08-19 05:03 那張 9.78% 的單出現在報告裡，標成右設限「至少 34.2h 未成交」**
   ——它是這份報告唯一的反例，漏掉它報告就在說謊
3. 沒有 `offer_wait_forecasts` 紀錄的早期掛單，**報告要說「沒有預估值」而不是填 0**
4. 高估倍數只用「有預估值 ＋ 已成交」那幾筆算，且**倍數的離散一起印出來**
   （只印 3.9× 會讓人以為它是個穩定的常數）

### 驗收結果（2026-08-26，四條全數通過）

`BFX_DB_PATH=<正式 DB> python3 scripts/wait_report.py --since 2026-08-18`：

1. ✅ 逐筆等待 3.93／0.70／3.33／0.77／0.28h，與上面第三張表一致
2. ✅ 08-19 05:03 的 9.78% 列為「至少掛了 34.20 小時**沒有成交**，被下一張單取代」
3. ✅ D038 之前的掛單印「掛出時沒有留下預估值（早於 D038）」，沒有填 0
4. ✅ 「總預估 ÷ 總實際 = 3.9 倍（逐筆：1.6×、1.9×、9.0×、11.3×、29.0×）」，
   並印出「逐筆倍數的中位數是 9.0 倍」與「校準樣本全部落在年化 9.11%～9.50%」

實作時**被實跑正式資料抓到一個會靜默吃掉樣本的 bug**，記在這裡因為它是同一族的病：

> 掛單期間的合併規則原本只有「連續 ＋ 同利率」一條。08-20 15:15、08-21 16:09、
> 08-21 19:11 三張 9.50% 的單**各自都成交過**（資金出去又回來），卻被併成一段
> ——**五個校準樣本靜靜地變成兩個**，而報告照樣印得出來、數字也不離譜。
> 修法是加上第二條：**中間成交過就切段**。
> 這是 D026「靜默失效」的又一次現身，而它是被「拿正式資料跑一次」抓到的，
> 不是被測試抓到的——**測試是照著我當時的理解寫的，而錯的正是那個理解**。

測試 660 → **676 項**（新增 16）。依 D025 的教訓，
**兩處實作各自反轉過一次確認新測試會紅**（合併切段、同輪兄弟單）。

### 追記（2026-08-29）：多三個樣本之後，倍數從 3.9 變成 5.4，而新樣本極端得多

08-28 那天成交的三筆進了報告，**校準樣本從 5 段變成 7 段**（另一段等待為 0，
算不出倍數）。`python3 scripts/wait_report.py` 現在印的是：

| 掛單 | 實際等待 | 預估 | 高估 |
|---|---|---|---|
| 08-28 00:46 | **0.06h**（3.6 分） | 6.91h | **108.6×** |
| 08-28 02:07 | **0.00h**（11 秒） | 6.90h | —（除不了） |
| 08-28 13:44 | **0.10h**（6 分） | 6.89h | **71.0×** |

- **總預估 ÷ 總實際：3.9 → 5.4 倍**
- **逐筆倍數：1.6×、1.9×、9.0×、11.3×、29.0×、71.0×、108.6×**，
  中位數從 9.0 變成 **11.3 倍**
- **七段全部高估，零段低估**（原本是五段全部高估）

**倍數的離散度比倍數本身更值得注意**：1.6× 到 108.6× 橫跨快兩個數量級，
而報告自己那句「與總量比差得越多，代表越不該用單一倍數轉述這件事」在這裡更成立了。
**拿 5.4 去除任何一筆都是拍數字。**

⚠ **三個新樣本仍然全部落在年化 9.11% 這一個點上**——帶寬不但沒變寬，還更窄了。
**帶外依舊零樣本，而唯一的帶外實測仍是反方向的那一筆**（08-19 掛 9.78%，
**34.20 小時沒成交**，至今仍是報告裡最長的一段右設限）。
所以這三筆**沒有**讓「該掛高一點」變得更有根據，它們只是讓
「模型對 9.11% 這個點的等待估計錯得更離譜」這件事更確定。改法仍然是 M2 的題目。

**這件事的下游後果已經看得到**（見 D046 的驗收狀況）：等待估得太長 →
單子秒成交 → **「單子躺在場上」這個狀態一輪都沒出現過** →
M1-c 裝好了但一列都沒長。

### 追記（2026-08-29）：高估倍數不是常數，是一個駝峰——而校準樣本全落在峰頂

**起因**：08-29 15:40 部位 `464715251` 收回後，模型第一次選了 **年化 10.95%**
（日利率 0.0003），而不是連續八筆的 9.11%。這是 D045 缺了兩週的**帶外樣本**。
使用者問的問題很直接：掛這麼高有機會成交嗎，會不會等太久反而虧。

**用了一個不需要等成交的量法**：把模型對**整條候選階梯**的等待估計反推出來
（`eff = r × P ÷ (W + P)` → `W = P × (r ÷ eff − 1)`），跟 384 根 1 小時 K 線
「這一小時的最高成交利率碰到這個價位幾次」並排。結果不是一個常數：

| 年化 | 模型估的 W | K 線掃到的頻率 | 倍數 |
|---|---|---|---|
| 8.00% | 1.4h | 1.4h | **1.0×** |
| 9.11% | 3.9h | 1.9h | 2.0× |
| **9.14～9.55%** | 9.0h | 2.2h | **4.2×** ← 七個校準樣本全在這 |
| 10.00% | 9.7h | 3.5h | 2.8× |
| **10.95%（08-29 掛的）** | 11.4h | 8.0h | **1.4×** |
| 11.30% | 15.5h | 15.4h | 1.0× |

**這推翻了本決策自己寫的那句推論的前提。** D045 本文寫「`W` 被系統性放大 →
高利率被系統性懲罰過頭 → 選出的價位會偏低」——**那句話只有在偏誤均勻時才成立**。
實際的偏誤集中在 9.1～9.5%，而**那正好是所有校準樣本落的那條窄帶**。
換句話說：**我們是在駝峰的峰頂上量出 5.4 倍的**。
所以「把 W 除以 5.4」在兩端都會錯，而修正之後價位往哪一邊移，
方向跟本文原先的推論**相反**。這一段直接進 M2 的題目清單。

### 這件事對「該不該掛高」給出的第一個可算門檻

高價（`r_高`、等 `W_高`）勝過簿子底部（`r_低`、等 `W_低`）的條件，代數上是
**`借出時間 > W_高 − 2 × W_低`**。以 08-29 的數字（10.95% 對 5.47%、
8.0h 對 0.5h）代入是 **7 小時**——而 16 筆已結束部位裡有 9 筆超過 7 小時（56%）。

反過來問「躺多久之後就不划算」，用實測的 `P` 代入：

- 用實測**平均**持有 17.9h → 躺超過 **18.9 小時**才輸給底部
- 用實測**中位** 11.6h → 躺超過 **12.6 小時**

**這是目前唯一有資格當 A2-b 判準的候選數字**（見 D046：往上調價從頭到尾沒有判準）：
它不是拍出來的門檻，是從實測的 `P` 與 K 線的掃單頻率推出來的。
**但它還不能直接寫進程式**——理由見下面那條反例，而且該由 M2 回測驗過再說。

⚠ **K 線掃單頻率是必要條件，不是充分條件。** 「市場掃到這個價位」不等於
「掃到我們這一張」——排在我們前面還有 100～275 萬 USD（而且那是下界，
簿子被截斷了）。**反例打得很重**：08-19 那筆 9.78%，掃單頻率說 2.6 小時一次，
實際掛了 **34.2 小時沒成交**，差 13 倍。當時沒有簿子快照，查不出是排隊位置
還是市場狀態不同。**所以這張表講的是形狀，不是可以拿去外推的數值。**

### 順帶修掉一個會誤述這件事的報告 bug

`describe_spell()` 對所有右設限的期間都寫「被下一張單取代」，而那是一個
沒有查證過的原因。08-29 那筆 10.95% 還躺在場上、才等不到兩小時，報告卻說
它被取代了——**偏偏它就是撐起這一整段追記的那個帶外樣本**。
把「還在計時」講成「已經結束」，等於把一個會繼續長的下界固定成量測值。
已修（`WaitSpell.replaced` ＋ `WaitSummary.ongoing`），見 CHANGELOG 2026-08-29。

### 追記（2026-08-29 20:52）：那張 10.95% 成交了，帶外樣本第一次落地

**掛出 15:40:56，成交 20:52:13，等了 5.19 小時**（部位 `464812689`、345.02 USD）。
上一段追記寫的時候它還躺在場上，整段推論靠的是反推值；現在它有實測了。

| | 模型估的 | 實際 | 倍數 |
|---|---|---|---|
| 平均（策略用的那個） | 11.81h | 5.19h | **2.3×** |
| 中位數 | 5.50h | 5.19h | **1.06×** |
| 上一段追記反推的 K 線掃單頻率 | 8.0h | 5.19h | 1.5× |

**駝峰站住了。** 9.11% 那批的高估倍數是 29×～108.6×，10.95% 只有 2.3×
——高估確實不是常數，而校準樣本確實全落在峰頂。上一段追記那句
「把 W 除以 5.4 在兩端都會錯」到這裡從推論變成量測。

### 追記（2026-08-29）：也許不是要除以一個倍數，而是一開始就選錯了統計量

D045 從頭到尾把「W 估不準」當成一個要除掉的數字在談：本文 3.9 倍、
第一則追記 5.4 倍、第二則追記發現它是駝峰所以任何單一倍數都不對。
**三次都在問「除以多少」，沒有人問過「為什麼用平均」。**

`estimate_wait()` 一次算出三個統計量、`offer_wait_forecasts` 三個都存了，
但 `expected_value.py` 只拿 `mean_hours` 當 `W`
（`effective = rate × P ÷ (mean_hours + P)`），而 `wait_report.py` 也只印平均
——**握著三份證據只看了一份，看了兩週**。八個可校準樣本並排之後：
...
**離散度比代表值更值得注意**：平均橫跨 1.6×～108.6×（快兩個數量級），
中位數只有 0.9×～9.0×。這不是「準一點」的差別，是**能不能拿來算期望值**的差別。

**為什麼平均會這麼糟（假說，等 M2 驗）**：`estimate_wait()` 從 168 個起點各算
一次等待、右設限以窗尾計入，得到的分佈右尾很長，平均被乾旱期與設限值拉走，
中位數不受影響。而且**我們並不是從隨機時刻進場的**——單子總是在部位收回的
當下掛出，那個時刻與市場活躍度相關，正好違反 `estimate_wait()` docstring 裡
「從任意時刻進場」那個前提。

⚠ **不要據此改 `expected_value.py`。** 用 08-29 的 168 根 K 線離線試算，
換統計量會把選中價位移動約 2 個百分點（平均 → 10.91%、中位數 → 10.59%、
四分之三 → 9.01%），而且三者的實質年化算式分母不同、數字不可直接比較。
**這是 M2 回測的題目**——先改參數再建量測正是 D036 記下的那個錯誤。
報告已經把三個數字並排（PR #49），選哪一個沒有在報告裡拍板。

### 🔮 寫在開獎之前：這筆 10.95% 要借滿 25 小時才算贏

**部位 `464812689` 還在生息中，結果還不知道。** 驗收條件寫在這裡，
理由同 D044——「沒發生」不等於通過，而事後補寫的條件不算條件。

已結束 credit 部位的實得年化（`r × P ÷ (W + P)`，`W` 與 `P` 都用實測）：

| 部位 | 年化 | 等待 | 借出 | 實得年化 |
|---|---|---|---|---|
| 464372858 | 9.11% | 0.77h | 48.39h | 8.97% |
| 464505426 | 9.11% | 0.28h | 48.26h | 9.06% |
| 464662934 | 9.11% | 0.06h | 1.28h | 8.68% |
| 464670446 | 9.11% | 0.00h | 11.62h | 9.11% |
| 464715251 | 9.11% | 0.10h | 25.84h | 9.08% |

**9.11% 那批的實得年化約 9.07%。** 10.95% 這筆等了 5.19h，
追平需要 `9.07 × 5.19 ÷ (10.95 − 9.07)` = **借滿 25.0 小時**。

**歷史 13 筆已結束 credit 部位裡，借滿 25.0 小時的只有 5 筆（38%）。**

- 借滿 25.0h 以上 → 掛高這一次是對的
- 不到 25.0h → 5.19 小時的等待沒賺回來，**這一筆是輸的，而輸了尤其要記**

⚠ **這不是乾淨的反事實。** 15:40 當下市場已經漲上去（常態成交價年化 9.00%、
FRR 11.36%），9.11% 那個價位並不是同時可選的替代方案——這個比較說的是
「掛高這個決定事後看划不划算」，不是「兩個價位同時擺著該選哪個」。

⚠ **一筆不算數。** 就算贏了也只是一個樣本，而 D040 記過「越貴借越短」
如果成立，高價位的 `P` 會系統性偏短，正好打在這個門檻上
——這筆收回後它也會是那個假設的第一個帶外樣本。

### 開獎（2026-08-29 22:50）：❌ **輸了，而且輸得很乾脆**

**借了 1.98 小時就被提前還款**（完成率 4%），門檻是 25.0 小時。
...
| 等待 `W` | 5.19h |
| 持有 `P` | **1.98h** |
| **實得年化** | **3.02%**（**至今最低**，前一個最低是 5.26%） |

**等待時間是持有時間的 2.6 倍。** 9.11% 那批的實得年化約 9.07%，
這一筆 3.02%——不是小輸，是差了三倍。

#### 🔴 而這一筆推翻了 D045 自己的重心：誤差幾乎全部來自 `P`，不是 `W`

D045 從 08-26 到 08-29 寫了本文加三則追記，**全部在講 `W`**。
把這一筆拆開來看：
...
`W` 那一項的誤差值約 1 個百分點，`P` 那一項約 7 個百分點。

**D045 的三則追記都沒有錯，但它們一直在調整比較小的那一項。**
真正的槓桿是 D1（`P` 寫死 48 而實測完成率 35.3%），而 D1 從 08-22
量測完之後就標著 `[~] 等 M2`，**沒有再被推進過**。

⚠ **一筆仍然不算數**，而且這一筆的方向與 D040 的「越貴借越短」一致
（10.95% 是至今最高的名目利率，也是第二短的持有），
但 9.11% 那一叢的持有時間從 0.50h 橫跨到 48.39h——**組內變異遠大於組間**，
所以這仍然是「值得繼續蒐集」，不是「假設成立」。

#### 這次做對的一件事：條件寫在開獎之前

**如果是事後才算這個門檻，25.0 小時這個數字會很難不被挑成剛好贏的那個。**
D044 立的規矩在這裡第一次真的擋住了東西——輸的結果被完整記下來，
而不是被重新敘述成「其實也還好」。

**下游動作**：`wait_report.py` 從此直接印實得年化（PR #50）
——在這之前 `hold_report` 只有 `P`、`wait_report` 只有 `W`，
**乘起來那個數字每次都是手算的**，於是「名目最高、實得最低」這種事
沒有任何一份報告看得出來。

#### 那個 48 到底值多少：離線試算（2026-08-29 23:3x）

**這是「P 主導」那句話的量化版本。** 拿當下的 168 根 1 小時 K 線離線跑
`choose_rate()`，只換假設、不改程式，看選中的價位往哪裡走：

| | `W`=mean（現行） | `W`=median |
|---|---|---|
| **`P` = 48h（模型現在寫死）** | **10.91%** | 10.85% |
| `P` = 17.9h（實測平均持有） | 9.09% | 9.86% |
| `P` = 11.6h（實測中位持有） | **7.99%** | 9.86% |

**對照組**：機器人 23:27 的日誌實際選了 **10.91%**，試算重現得到同一個數字
——所以這張表不是另一套算法算出來的。

🔴 **`P` 寫死 48 正是模型一直往高價跑的原因。** 機制在算式裡看得出來：
`eff = r × P ÷ (W + P)`，`P` 很大時 `P/(W+P)` → 1，**等待幾乎不被懲罰，
判準就退化成「挑最高的利率」**；`P` 縮到與 `W` 同量級，等待才真的有代價。
換成實測持有時間，選中價位掉 **1.0～2.9 個百分點**。

**兩項一起修**（`W` 用中位數 ＋ `P` 用實測中位）落在 **9.86%**
——而 9.11% 那一批的實得年化約 9.07%，是至今表現最好的一群。
**兩個偏誤方向相反，一起修比只修一個更接近實際表現過的區間**
（D040／D045 都寫過「兩者反向」，這是第一次看到它們相消之後落在哪裡）。

⚠ **這仍然只是一個時間窗、一個時刻的算術**，而且 `P` 用的是全體中位數，
**沒有處理「`P` 會隨利率而變」**——那正是 D1 真正要解決的事，
也是為什麼答案是 M2 回測而不是把 48 換成 11.6。


## D046 — 重掛政策：往上調價目前完全沒有判準，而該用的訊號不是閒置時間

**日期**：2026-08-26　**狀態**：🟡 形狀已定，數字等 M2
...
提案的方向是對的，而且它正好落在 TASKS.md 的 **A2-b**（新策略下的重掛政策）
——那一項自 2026-08-18 拆出來之後就開著、沒有主人。查證之後有四個事實，
其中三個支持這個提案，一個要修正它。

### 一、每十分鐘確實已經在比了——這一半已經有了

`core/bot_engine.py` 每一輪的順序是：重新定價 → 量測閒置 → `_plans_match()`
（利率差 2% 以內視為沒變）→ 往下調價的守門檻 → 取消重掛。
**所以「每十分鐘看一次市場並跟場上那張比」這件事，現在就在做。**

### 二、但往上調價**完全沒有判準**（這是真正的缺口）

`_cheaper_repost_is_not_worth_it()` 的第一件事就是
`if candidate.rate >= live_rate: return None`——**它只管往下**。
於是往上調價的實際規則是：

> 新算出來的價位只要比場上那張高過 2%，就直接取消、重掛，**不問划不划算**。

兩個方向一個有測試、一個沒有，而**沒有測試的那個方向正是使用者問的那個**。

### 三、D034 當初為「往上不設限」給的理由，被 D040／D045 推翻了一半
...
> - **往上調**：多賺的利息是**確定的**，付出的速度是**估的**
>
> 所以只在「不可靠的那半邊是行動的理由」時才要求它先過關。

**「多賺的利息是確定的」這句話有一個隱含前提：那張單會成交。**
沒成交的話拿到的不是「少賺一點」，是 **0%**。而反例已經有了：
**2026-08-19 掛 9.78%，在場 34.2 小時沒有成交**，最後被換掉（D045 的報告列著它）。

所以往上調價的取捨其實是「**確定的閒置成本 × 不確定的成交機率**」
——兩邊都不確定，D034 的不對稱論證在這個方向上不成立。

### 四、機會成本每十分鐘都在算，但**算完就丟掉**

`_log_idle_time()` 每一輪算出 `idle_hours` 與 `forgone`（已閒置多久、
這段時間本來會賺到多少），寫進日誌，然後**函式回傳 `None`**。
**沒有任何判斷式讀得到這兩個數字。**

使用者問的「符不符合機會成本」，要的正是把這兩個數字接上決策。
它們已經算好了，只是沒有出口。

**第五個相關事實**：`build_offer_plan()` 的參數裡沒有「場上那張單」。
策略每輪從零開始定價，於是「要不要調高」是「從零定價的結果剛好高過 2%」的
**副作用**，而不是一個被問過的問題。

### ⚠ 提案裡要修正的那一點：閒置久不是漲價的理由

這一條是這則決策最重要的部分，因為它是直覺最容易走反的地方。

「等太久 → 該調整」——方向對，**但調哪一邊會弄反**：

- **「等太久」是「這個價位成交比預期慢」的證據**，它支持的是**降價**，不是漲價。
  D038 的 `p75` 對照（`已超出當初預估的四分之三分位，等待估計偏樂觀`）
  講的就是這件事。
- **支持漲價的證據是另一份資料**：市場整體成交價往上走
  （`market_snapshots` 的 `trade_rate_*`、K 線的 `high` 分佈）
  ——**別人成交在更高的價位**，不是「我等很久」。

**兩者混在一起的話，會在市場走弱、我們的單掛不出去的時候去漲價**，
那正好是最糟的時機。所以規則裡這兩個訊號要分開餵，不能合成一個「等太久就動」。

### 還有一個提案沒提到的風險：棘輪

如果每輪都「市場漲了就跟著漲」，市場連漲時會一路追高、**永遠不成交**。
需要一個上限（同一筆資金調價幾次／累計調幅多少／調過之後鎖多久），
而**那個數字同樣不能拍**——`target_queue_usd` 就是拍出來的，D032 記著它怎麼死的。

### 順帶量到的一件事：這個機制的作用窗口很短

`scripts/wait_report.py` 的實測：08-18 之後六段成交的掛單，
**在場時間平均 2.10 小時、中位數 2.05 小時**——約 12 輪巡檢。
資金其餘時間都鎖在部位裡，場上根本沒有單。

**所以這個機制平常只有十幾輪可以作用**，真正需要它的是 08-19 那種
34 小時（205 輪）的長尾。這不是不做的理由，而是**它的價值集中在尾端**
——設計時要拿長尾當主場景，不要拿中位數。

### 決定

**分兩段，第一段現在就能做且不擋 M2，第二段等 M2。**

**第一段（M1-c　重掛反事實落地）——只落資料，不改任何行為**

1. `_log_idle_time()` 把算出來的 `idle_hours` / `forgone` **回傳**而不是丟掉
2. 每一輪有場上掛單時，落一列「兩條路的並排比較」：
   場上那張的利率／已閒置多久／當初的預估、本輪候選價位／它的預估等待、
   **兩者的實質年化並排**，以及「照現在的規則會不會動它」
3. **這一段刻意不改變任何行為**：它產出的是「如果當時調高了會怎樣」的
   反事實資料，而那正是 M2 要吃的東西。
   **沒有這批資料，第二段就只能拍數字。**

**第二段（A2-b　重掛政策）——等 M2**

- 用 M2 在歷史資料上回測「調高門檻設多少」實際會賺多少
- 往上調價的判準要包含：**市場價位的走向**（不是閒置時間）、成交機率的變化、
  棘輪上限、空窗與排隊位置的成本
- **往下調價的判準一併重寫**：A2-a 修好之後（D039），現在擋著降價的只剩
  `_plans_match()` 的 2% 容差

### 這一項不做什麼

- **不現在改重掛行為**：包含不加「漲價門檻」、不動 `rate_tolerance_pct` 的 2%。
  第一段的整個意義就是「先有反事實資料再談門檻」（D036）。
- **不把閒置時間接成漲價的觸發條件**：見上面那條警告，方向是反的。
- **不做 M2**：第一段是 M2 的輸入，不是 M2。
- **不改 `build_offer_plan()` 的簽章讓策略看見場上那張單**：那會讓策略層
  變得有狀態，而 D041 的兩個 bug 都是策略層跨輪殘留造成的。
  兩條路的比較放在引擎層做，策略層繼續只回答「從零開始該掛多少」。

### 驗收條件（第一段，照 D041／D043 的做法先寫下來）

1. 有場上掛單的輪次，DB 多一列並排比較；**沒有場上掛單的輪次一列都不寫**
2. 那一列的「照現在的規則會不會動它」，與同一輪日誌實際發生的事**一致**
   ——不一致代表落下來的反事實在說謊，比沒有更糟
3. **行為零變化**：部署前後，同一份市場資料下掛出去的單完全相同
4. 08-19 那種長尾重現時（單子掛超過 p75），這批資料**答得出**
   「當時每一輪的候選價位是多少、比場上那張高還是低」

### 第一段（M1-c）實作結果：2026-08-28，PR #46

**狀態**：✅ 已部署。條件 3 已驗、條件 1／2 待正式環境出現「場上有掛單」的輪次、
條件 4 只能等長尾重現。

新表 `repost_comparisons`，34 欄。三處改動：

1. **`ExpectedValueStrategy.evaluate_rate()`** —— 對**任意**利率用本輪那一窗重估。
   這件事是必要的而不是方便：場上那張的利率**通常不在候選集裡**
   （`choose_rate()` 的候選只取自窗內出現過的 `high`，而 `plan.rate` 還經過
   成交價下限與 spread 的加工），所以「保住 vs 改掛」不能拿 `last_evaluation`
   撈一撈就算完。**兩邊都走同一個函式、同一窗 `high`**，比出來的差額才是
   市場的差、不是尺的差（同 `_queue_ahead()` 的「同一把尺」）。
2. **`_log_idle_time()` 回傳算出來的值** —— 決定的第 1 條。
3. **落地點在兩個判斷求值之後、三條出口之前**。

**條件 2 是靠位置解決的，不是靠測試。** 原本的寫法是
`if existing and _plans_match(...)` 先 return、不成立才求 `not_worth_it`；
改成兩個都先求值、落地讀結果、三條路照著同樣那兩個值走。**求值順序與短路完全不變**
（`matched` 為真時守門檻依然不會被呼叫，連它那條棄權日誌的行數都一樣），
於是 `action` 是**讀出來的，不是預測的**——落在 `_log_idle_time()` 旁邊的話
就得預測下面的判斷結果，而預測與實際可能分岔，那正是條件 2 要擋的事。
**手法與 M1-b 把決策落地緊貼日誌那一行完全相同（D043）：靠位置保證，不靠承諾。**

**條件 3 有直接證據，不只是回歸網**：把 `HEAD` 的舊引擎與新引擎並排，餵**同一份**
市場資料跑，比對 `client.offers`／`cancel_calls`／離開方式——**六個情境全部相同**
（場上沒單、維持不動、往上調價重掛、往下重掛不划算、餘額不足、拿不到簿子）。
另在**正式 DB 的副本**上跑過一次初始化：新表建起來，9 張舊表列數一格沒動。

**兩件實作時才算出來的事**：

- **一列 418 位元組**（寫 500 列再 VACUUM 量的），比 `pricing_decisions` 的
  4,137 B 小一個數量級——那張表的兩排候選集就佔 2.6 KB，而這張沒有陣列欄位。
  最壞情況（每輪都有場上掛單）一年 21 MB。
- **`live_effective` 可以是 NULL，而那一列照樣要寫。** 窗內命中不足 `ev_min_hits`
  時算不出實質年化，此時 `evaluate_rate()` **仍回一列**並填上實際 `hits`：
  「一次都沒掃到（0）」與「掃到了但不夠（3 < 5）」意義完全不同，回 `None`
  會把兩者壓成同一個空白。**08-19 那張掛了 34.2 小時沒成交的單落下來就長這樣**
  ——而那正是條件 4 要靠的那一列。

**一個事後看資料的人會踩到的差異**：`candidate_amount` 記的是「做決定當下」的金額，
走重掛那條路時，取消完會用**真實餘額**再算一次計畫（D025），金額因此可能差一點。
**利率不會差**（`choose_rate()` 只吃 K 線、不看餘額）。落地點刻意留在取消之前——
這張表要答的是「當時拿什麼在做比較」，不是「最後掛了多少」，後者在 `loan_offers`。

### 驗收狀況（2026-08-29 14:5x，部署後 38 小時）

**條件 1 的負向那半 ✅ 通過、條件 3 ✅ 通過；條件 1 的正向那半與條件 2
「測不到」——不是沒通過，是那個情境一次都沒出現。**

部署後 **228 輪巡檢**的分佈（`podman logs` 自 08-28 00:36 起實際計數）：
...
`repost_comparisons` 因此是 **0 列，而這完全正確**——「沒有場上掛單的輪次一列都
不寫」正是條件 1 自己要求的，225 輪都守住了。

**為什麼一次都沒出現：三張單全部在一個巡檢週期（600 秒）之內就成交了。**
...
於是每一輪的開頭都只有兩種狀態：**單還沒掛**，或**單已經變成部位**。
「單子躺在場上等成交」這個 M1-c 要觀測的狀態，中間沒有任何一輪巡檢看得到。

### 這件事本身就是一個發現，而且比原本估的嚴重

D046 當初寫過「作用窗口很短（成交的單在場中位數 2.05 小時 ≈ 12 輪），
價值集中在 08-19 那種 34 小時的長尾」。**實測比那個估計還要極端一個數量級**：
不是 12 輪，是 **0 輪**。

**所以 M1-c 目前是一台裝好但還沒被觸發過的儀器。** 它要等的是
「掛出去之後沒有馬上成交」的那種輪次——也就是市場走弱、或我們把價格掛高的時候。
**這不是把它拆掉的理由**（那正是最需要重掛判準的情境），但要誠實記著：
**在目前這個「掛什麼都秒成交」的區間裡，它一列都不會長。**

⚠ **還有一條實作上的界線要寫下來**：`_record_repost_comparison()` 在
`not existing or not plans` 時直接 return，所以「**場上有單、但策略本輪沒有計畫**」
那條路（`if not plans:` → 維持場上既有掛單 → `raise SkipCycleError`）**也不寫列**。
理由是沒有候選價位就沒有東西可以並排比較。**這是刻意的，但它讓條件 1 的字面
（「有場上掛單的輪次就多一列」）有一個例外**，事後查資料的人要知道。
這條路在部署後 228 輪裡出現 **0 次**，所以目前也還沒被實測過。

---

## D047 — 乾旱期會讓等待估計自己變好看：右設限的補值被窗尾夾住

**日期**：2026-08-30
**狀態**：🔴 已量出機制，**刻意先不修**——改法是 M2 的題目（同 D1、D045 的處置）

### 現象：同一個價位、同一份市場，掛越久沒成交，模型算出來的實質年化越高

08-29 22:51 掛出的那張 10.91%，到 08-30 12:05 已經**閒置 13.24 小時**沒成交。
`pricing_decisions` 逐小時的紀錄是這樣的：

| 時間 | `W` 平均 | 選中價位的實質年化 |
|---|---|---|
| 08-29 22:00 | 9.95h | 9.03% |
| 08-30 00:00 | 9.43h | 9.11% |
| 08-30 04:00 | 8.54h | 9.26% |
| 08-30 08:00 | 7.84h | 9.37% |
| **08-30 12:05** | **7.33h** | **9.46%** |

**15 個小時區間，單調上升，一次都沒有回頭。** 這段期間選中的價位完全沒變
（`0.00029879`，逐位元相同），市場也沒有往我們這邊靠——同一份快照裡
最近 385 分鐘的 10,000 筆成交，**中位數是年化 5.47%、最高只有 10.00%**，
**沒有一筆碰到我們掛的 10.91%**。

**模型在它最該懷疑自己的時候，對自己越來越有信心。**

### 機制：補值被「離窗尾多遠」夾住

`estimate_wait()` 對等不到命中的起點記的是「至少等到窗尾」：

```python
if target is None:
    waits.append((total - start) * self.candle_hours)   # 右設限
```

**離窗尾越近的起點，這個數字越小。** 於是尾端的乾旱每多長一小時，
就多灌進一個**很小**的「等待」把平均往下拉——
**乾旱靠自己的新鮮度替自己脫罪。**

離線重跑證實（唯讀，沒有改任何程式；`rate` 固定 `0.00029879`，
把窗尾逐步往回退，模擬「這場乾旱還沒發生」到「乾旱越拖越久」）：

| 窗尾往回 | `W` 平均 | `W` 中位數 | 右設限根數 | 實質年化 |
|---|---|---|---|---|
| 16h（乾旱剛開始） | 10.52h | 5.5h | 0 | **8.95%** |
| 12h | 9.43h | 4.5h | 4 | 9.11% |
| 8h | 8.54h | 4.5h | 8 | 9.26% |
| 4h | 7.84h | 4.5h | 12 | 9.37% |
| **0h（現在）** | **7.33h** | **4.5h** | **16** | **9.46%** |

**命中次數全程都是 43，一次都沒變**——變的只有設限那一段怎麼被記帳。
重跑的數字與 DB 逐列吻合，所以這不是另一套算法算出來的。

### 這跟 D038 已經寫過的「這是下界」不是同一件事

D038 記的是「以窗尾計入算出來的是**下界**，所以要一起印 `censored_ratio`」，
那句話沒有錯。**沒有被記下來的是：這個下界會在市場剛離開我們的那一刻鬆得最厲害。**

差別在於方向。「是個下界」聽起來像一個固定的保守偏差；實際上它是
**一個會隨乾旱長度反向擴大的偏差**，而乾旱長度正是我們最需要它誠實的變數。

### 為什麼它重要：這是一個對抗降價的正回饋圈

**市場離開 → 乾旱在窗尾累積 → `W` 的平均被拉低 → 這個價位的實質年化算得更高
→ 更不可能改掛 → 乾旱繼續長。**

這解釋了一件本來只是「巧合」的觀測：`repost_comparisons` 至今 **111 列，
`action` 全部是 `hold_matched`**，而候選利率與場上利率**逐位元相同**。
D046 的驗收條件 2（要看到 `repost` 或 `hold_cheaper_not_worth_it`）
因此測不到——**不是情境還沒出現，是這個回饋圈正在把它擋掉。**

### 它與 D045／D1 的關係：第三個誤差源，而且方向一致

| 誤差源 | 方向 | 目前狀態 |
|---|---|---|
| `P` 寫死 48（D1） | 把高利率評得**過高** | 🔴 最大的一項（約 7 個百分點，D045 開獎） |
| `W` 的平均被乾旱與設限值拉走（D045） | 把高利率評得過低（**在已成交的樣本上**） | 🟡 約 1 個百分點 |
| **`W` 的設限補值被窗尾夾住（本則）** | 把高利率評得**過高**（**在沒成交的樣本上**） | 🔴 剛量出來 |

⚠ **本則與 D045 的「高估 4.25 倍」不衝突，因為它們看的是不同半邊的資料。**
D045 的八個校準樣本**全部是等到成交的期間**，右設限的那些依定義不在裡面
——而本則這個機制**只活在右設限的期間裡**。兩件事各自為真。

🔴 **三項裡有兩項都在把高利率評得過高，而它們是相乘的。**

### 順手落下的第四個觀察：這一項打 `mean` 遠比打 `median` 重

上表裡 16 小時的乾旱讓 `mean` 走了 3.19 小時（10.52 → 7.33），
`median` 只走了 1 小時（5.5 → 4.5）。**中位數對窗尾的補值幾乎免疫**
——因為補進來的是一批小值，改變的是分佈的重心而不是它的中點。

這是「該用 `mean` 還是 `median`」這個問題的**第三個獨立論據**
（前兩個在 D045 的第三則追記：8/8 較準、總量比 1.67× 對 4.25×）。
**但它仍然不是拍板的理由**，見下一節。
...
| 補值方式 | 選中價位 | 實質年化 | `W` 平均 |
|---|---|---|---|
| **現行（等到窗尾）** | **10.91%** | 9.46% | 7.33h |
| 丟棄設限樣本 | 10.91% | 9.48% | 7.20h |
| 補成一整窗（168h） | 10.42% | 7.97% | 14.74h |

**換一個補值就是換一個拍出來的常數，正是 `target_queue_usd` 的死法（D032）。**
而且「補多少」本身沒有正確答案——真實等待是被右設限的，
它要的是存活分析那一類的處理，不是一個更大的填充值。

**所以本則的產出是「把機制與量測寫下來」，不是改程式。**
它成為 M2 回測工具的**第三個題目**（前兩個是 D1 那個 48、`W` 用哪個統計量），
並且是一個**有已知答案的迴歸釘子**：同一份 K 線、`P=48`、現行補值 → 必須選出 10.91%。

### 誠實界線

- **只有一場乾旱（n=1）。** 08-19 那段 34.20 小時的空掛沒有留下預估值（早於 D038），
  所以無法拿來對照。
- **上面每一列都是離線試算**，機器人的行為一格都沒有被改動。
- **「市場沒有碰到我們的價位」用的是 385 分鐘的成交窗**，不是完整的 13 小時
  ——`market_snapshots` 每輪只存最近一批成交的摘要，中間的時段沒有逐筆留存。
- 這張單**至今仍掛在場上，刻意不動**：它是第一段「有預估值、而且實際等待已經
  超過預估」的右設限期間（掛出時預估平均 9.95h／中位數 4.50h），
  對 M2 的價值高過它 0.0569 USD 的機會成本。

### 🔴 追記（2026-08-30 傍晚）：這個方向**不是通則**，只在乾旱還短的時候成立

上面整則都在講「乾旱把估計拉好看」，而那是在**一段乾旱**上觀察到的。
做 D050 時追第二段空掛（第 131 根起，38.5 小時），**方向是相反的**：

| 空掛 | 典型等待 | 乾旱長度 | `W` 平均怎麼走 |
|---|---|---|---|
| 第 387 根起（本則記的那段） | ~10h | 16h | **10.52 → 7.33，變好看** |
| 第 131 根起（38.5h 那段） | ~5.9h | 33h | **5.88 → 8.14，變難看** |

兩邊都是同一份程式、同一條規則，用的也是同一種「窗長固定往回滑」的比法。

**機制講清楚就不矛盾了**：設限的起點記「至少等到窗尾」，最後 `k` 根貢獻的是
`k, k-1, …, 1`，平均約 `k/2`。這批數字**取代**了那些起點原本的等待 `w`：

- `k/2 < w` → 把平均**拉低**（本則觀察到的）
- `k/2 > w` → 把平均**拉高**（第 131 段）

**所以「越等越有信心」發生在乾旱還短的時候**——`k` 還小、`k/2` 還沒超過典型等待。
乾旱夠久之後，命中從窗的前端滾出去，平均就掉頭往上。

⚠ **本則的正文因此要這樣讀**：機制描述是對的，「同一個價位掛越久算起來越好」
**那句話要加上「在乾旱的前半段」**。

🔴 **但這不代表問題變小了，反而更難處理**：模型最樂觀的時刻，
正好是**乾旱剛開始、還來得及改掛**的那段時間。等到估計掉頭往上、
模型願意降價時，那段等待多半已經快走完了——**D050 量到的正是這件事**
（最長那段空掛裡，候選價位前 32 小時完全沒動）。

已加迴歸測試釘住兩個方向（`TestD047的機制不是永遠同一個方向`）。

---

## D048 — M2 的驗收標準跑不起來，換成一條跑得起來的

**日期**：2026-08-30
**狀態**：🟢 已實作（M2 第 1 步，分支 `feature/backtest-replay`）

### 原本的驗收標準

PLAN.md 第 1 期給 M2 寫的是：

> **驗收標準：拿它跑 `orderbook_depth`，要能重現「賣在區間底部」這個已知結論。**
> 重現不出來就是工具不對，不是策略對。

**那句話的精神是對的**——驗收要拿一個**已知答案**去釘工具，不能讓工具自己
宣告自己對。它今天仍然是這則決策的前提。
...
2. **要餵的簿子不存在。** 「賣在區間底部」是 2026-08-16～17 的結論（D033／D035），
   而 `market_snapshots`（M1-a／D042）**2026-08-23T17:21 才寫下第一列**。
   那七天的簿子沒有被存下來，**而且再也回不來**。

### 🔴 這件事本身是 D036 的另一種長相

**驗收標準是 2026-08-17 寫的，比它所依賴的那張表早了六天。**
寫的時候那份資料還不存在——寫的人（我）當時並不知道自己在依賴一份還沒有的資料，
因為 M1-a 也還沒開始做。而寫完之後的十三天裡，**沒有人回頭確認它變得可跑了沒有。**

D036 記的是「決策用一個時間切片做出來，而專案沒有量測基礎去否證它們」。
這一則是同一個病往前一步：**連驗收標準本身都可能是用一個時間切片寫的，
而它同樣需要被否證。** 一條沒有人跑過的驗收標準，跟沒有驗收標準的差別
只在於它看起來像有——**這是 D026「靜默失效」在文件層的第一次現身。**

### 換上的那一條

`scripts/backtest_report.py --verify`：拿 `pricing_decisions` 裡**正式環境真的
做過**的決策，逐點對照重播結果。鍵是 `candle_latest_mts`（那一輪窗尾那根 K）
——**它是唯一不會因為時區設定而跑掉的欄位**，同 `market_candles` 的選擇理由。

**2026-08-30 第一次跑：正式紀錄 25 列，逐點一致 25 列、不一致 0 列。**

再加三組寫進單元測試的迴歸釘子（`tests/unit/test_backtest_regression.py`），
資料是 `market_candles` 倒出來的 200 根真實 K 線：

| 釘子 | 期望值 | 來源 |
|---|---|---|
| `P = 48h` | 選中 10.91%、實質 9.46%、`W` 平均 7.3273809523809526h、候選 132 | **`pricing_decisions` 第 118 列**（08-30 12:05:39） |
| `P = 16.93h` | 選中 9.09%（比 48h 低 **1.82 個百分點**） | 08-30 離線試算 |
| D047 的乾旱 | 窗往回滑 16h → 實質 8.95%；滑回來 → 9.46%，而選中價位一路沒變 | 同上 |

### 誠實界線：換上的這一條比原本那條**弱**

| | 原本那條 | 現在這條 |
|---|---|---|
| 證明什麼 | 工具能重現一個**獨立於工具**的市場結論 | 工具**沒有偏離正式環境** |
| 抓得到什麼 | 重播的邏輯錯了、資料餵錯了 | 同左 |
| **抓不到什麼** | — | 🔴 **策略本身是錯的**——重播與正式環境跑同一條路徑，兩邊會一起錯 |

**所以 `--verify` 全綠不代表 10.91% 是個好價格**，它只代表「同一份輸入算出
同一個答案」。輸出裡把這句話印出來了，不只寫在這裡——
會被拿去下判斷的是終端機上那幾行，不是 DECISIONS.md 裡的某一段。

**真正能否證策略的驗收要等 M2 第 2 步（成交模擬）**：那時候才有一個
「當時可達到的最佳」可以對照，也就是 M3 要的那一對數字。

### 沒有做的事

**沒有把 `orderbook_depth` 的重播能力補起來。** 它需要從
`market_snapshots.book_curve_json` 還原簿子，而那張表只涵蓋 08-23 之後——
**能重播的期間裡，`orderbook_depth` 一次都沒有上線過**（08-17 起就換成
`expected_value` 了）。補了也沒有對照組。
如果之後要拿它當對照策略跑，那是 M2 第 2 步的事，且要先講清楚拿什麼當已知答案。

---

## D049 — 成交模擬上線，D1 第一次有答案：方向是「48 太高」，但**不足以拿來改參數**

**日期**：2026-08-30
**狀態**：🟡 **方向已量出，數值仍不可用**——改那個 48 的條件寫在最後一節

### 先說結論的三層，因為它們很容易被壓成一句

1. ✅ **方向**：策略假設 `P = 48h` 的實得年化，**低於**假設較小的 `P`。
   在**四種**「實際借多久」的假設下都成立（實測分佈、固定 48／16.93／11.61）。
2. 🔴 **但跨時段站不住**：把歷史切一半，**前半的結論反過來**
   （前半 48h 較好 7.21% vs 7.05%，後半 16.93h 較好 7.59% vs 5.93%）。
3. 🔴 **而且細掃出來的「最佳值」是雜訊**：14h 那一列最高（7.38%），
   但 20h 是 7.28%、12h 是 7.23%，曲線**不是單調的**，高低差不到 0.5 個百分點
   ——而樣本只有 16.8 天、十幾次循環。**挑最高的那一列就是 `target_queue_usd`
   的死法（D032）。**

**第 2、3 條與第 1 條同等重要**，而且被寫進同一份測試檔（`Test這個方向還不足以拿來改參數`）
——**免得日後只記得第 1 條。** 這個專案已經有六個決策互相推翻的紀錄（D036），
每一次的形狀都一樣：**一個時間切片上的結論被當成了通則。**

### 成交規則，以及它憑什麼可以當裁判

規則只有一句：**從掛出那一刻往後走，第一根 `high >= 掛單利率` 的 K 線就是成交**，
時點取那根的中點（與 `estimate_wait()` 同一個約定，兩邊不一致的話「預估 vs 實際」
會多出一個純粹來自記帳方式的偏差）。

**這條規則是被驗證過的，不是假設的**——拿 `wait_report` 配對好的 12 筆真實成交
回頭跑一次：
...
模擬器準了一個數量級——**這才是它能拿來當裁判的理由**，而不是「因為它是回測」。

⚠ **要讀的是總量比，不是逐筆倍數**：12 筆裡有 7 筆的實際等待不到一小時，
而 K 線一小時一根，那些筆的 `0.00×` 與 `inf×` 是解析度雜訊。

### 🔴 最違反直覺的那個結果：就算 `P` 真的是 48，假設 48 仍然不是最好的

| 實際借多久 | 策略假設 48h | 策略假設 16.93h |
|---|---|---|
| 實測分佈（17 筆） | 6.57% | **7.26%** |
| **固定 48h（模型的世界觀完全成立）** | 7.27% | **7.61%** |
| 固定 16.93h | 4.91% | **6.87%** |
| 固定 11.61h | 4.62% | **5.76%** |

**第二列是重點**：即使每一筆都真的借滿 48 小時，假設一個較小的 `P` 仍然比較好。

**機制**：假設較小的 `P` 讓策略更怕等待，而那正好補償了 `W` 被低估
（D047 的乾旱回饋圈）。**兩個錯誤部分互相抵銷。**

🔴 **所以「把 48 改成實測值」不是一個安全的動作——它會拆掉其中一半。**
D1 從 08-22 就寫著「不可以直接把 48 換成實測值」，理由當時是「逐筆變異太大」；
**現在有第二個、而且更硬的理由**：那個 48 正在替 `W` 的偏差擦屁股，
單獨改它會讓 `W` 的偏差裸露出來。

**D047 與 D1 必須一起處理，不能各改各的。**

### 空等與空轉都進分母

`run_policy()` 算的是整段歷史的**時間加權實得年化**：`Σ(r × P) ÷ Σ(W + P)`，
**沒等到成交的每一個小時、以及策略不掛單的每一個小時，都以 0% 計入分母**。

`wait_report` 印的 7.99% 就是因為少了這一塊才偏樂觀（最長的一段空掛 34.20 小時
完全沒被算進去）。這裡不重蹈——也因此這份報告的數字（6.5%～7.4%）
**看起來比 `wait_report` 低，那不是變差，是分母變誠實了**。

### 實作時被自己咬了一口：有狀態的持有模型

第一版的 `empirical_hold()` 回傳一個帶 `state["index"]` 的閉包，
於是**同一個模型實例跑第二次會從序列中間接下去**——同一組設定先後跑出
**7.26% 與 6.70%**，而兩個數字看起來都很正常。

「依序循環而不是隨機抽」的**重點本來就是可重跑**，一個會累積狀態的實作正好
毀掉那件事。改成把「第幾次循環」當參數傳進去，模型成為純函式，重跑必然同值。
已加測試釘住（`Test持有模型必須無狀態`）。

### 四條做不到的事，每一條都讓結果偏樂觀

1. **不看排隊位置與金額**——`high >= rate` 只說有需求掃到這個價位，
   沒說掃掉的量足夠輪到我們那 345 USD。0.93× 有一部分就是這個。
2. **K 線一小時一根，解析度到此為止。**
3. **樣本全落在年化 5.47%～10.95%**，那正是模型自己選出來的帶——**帶外不能外推**
   （與 D045 同一條界線）。
4. **`P` 與 `r` 被當成獨立的**，而 D040 的「越貴借越短」至今比不出來
   （分組是 1 筆對 16 筆，且不會自己好）。

### 要動那個 48，需要什麼

**不是更多的參數掃描**，是這三件其中至少兩件：

1. **一段模型沒有參與過的歷史**。現在這 16.8 天是 `expected_value` 自己選價、
   自己成交出來的，拿它評價自己有循環論證的成分。
2. **跨時段站得住的結論**——切半就翻掉的東西不能上線。
3. **`W` 的偏差先被處理**（D047），否則改 `P` 只是拆掉抵銷的一半。

在那之前，`--simulate` 的輸出是**用來讀方向的**，那段警語印在報告裡，
不只寫在這裡——會被拿去下判斷的是終端機上那幾行。

---

## D050 — A2-b 的答案是否定的：**訊號太慢，而不是門檻挑錯**

**日期**：2026-08-30
**狀態**：🔴 **量出來的是一個否定結果**——A2-b 不該繼續挑門檻，該換訊號。
**機器人的重掛邏輯一行都沒有改。**

### 問題原本長什麼樣

D046 記的是「往上調價從頭到尾沒有判準」，並提出該用的訊號是**市場價位的走向**，
不是閒置時間。D045 的追記推導出一個門檻候選：躺超過 **12.6～18.9 小時**
就輸給簿子底部。M2 建好之後，這件事第一次可以用比的。

### 拿 403 根真實 K 線比了六種政策，結果是**分不出勝負**
...
**最好與最差差不到 0.05 個百分點**，而「候選一變就跟」這個上界對照
——重掛成本在模擬裡是零，它應該最占便宜——**也沒有贏**。
...
| | 值 |
|---|---|
| 等待佔整段歷史 | **30.1%**（107.0h／355.3h） |
| 15 段等待裡 ≥ 6 小時的 | **5 段** |
| 最長的一段 | **38.5 小時**，佔全部等待的 36% |

**機會很多。政策就是沒有動。**

### 成因：候選價位是個**落後**訊號，因為它來自 168 小時的窗

追進最長那段空掛（第 131 根起，38.5 小時），逐小時看候選價位：
...
| 16 | 9.78% | 0.00% |
| 28 | 9.78% | **0.00%** |
| **32** | 9.50% | **2.88%** ← 這時才第一次越過 2% 容差 |

**前 32 個小時，候選價位一動也不動**——而那段等待總共只有 38.5 小時。
政策終於被觸發時，**那段等待已經走完 83%**。

現在場上這一段（16 小時）更乾脆：**候選價位的漂移是 0.00%。**

**結構性的原因**：定價用的是 168 小時的窗，而一場十幾小時的乾旱在 168 根裡
只佔不到一成。**要一個 168 小時的平均去反映十幾個小時的變化，本來就辦不到。**

### 所以下一步不是挑門檻，是換訊號

**重掛的判斷需要比定價更短的視野。** 這與 D046 說的「該用的訊號是市場價位的
走向」方向一致，但把它講得更死：**不能拿定價用的那個候選價位當重掛訊號**，
不管在它後面接多聰明的門檻——**因為它在該動的時候還沒動。**

候選訊號（都還沒量，列在這裡是為了下次不用重想）：
- **短窗的成交價中位數**（`market_snapshots.trade_rate_median` 已經每輪在存）
- **簿子最高可見價位的走向**（⚠ D039 警告過它有一部分是簿子密度的假象）
- **「我掛的價位在最近 N 小時被掃到幾次」**——直接對應成交規則本身

⚠ **這三個都還沒有被比過**，寫在這裡不代表哪一個會贏。

### 誠實界線

1. **模擬看不到重掛的兩項成本**，兩項都讓重掛顯得比實際划算：
   取消會把排隊位置歸零；**取消當下就成交的風險**（D031 在 2026-08-16 19:31
   真的發生過——送出取消、25 秒後那張單成交了）。**所以上表的差距是上界。**
2. **只有 16.8 天、15 段等待、其中 1 段長尾。** D046 早就寫過「價值集中在
   08-19 那種 34 小時的長尾」——**而一個樣本裡只有一個長尾，量不出長尾政策。**
   這一條與結論並不矛盾：分不出勝負**有可能**只是樣本不足；
   但「訊號前 32 小時不動」是**機制**，那一項不依賴樣本數。
3. **沒有比「換訊號」的版本**——那需要先實作新的訊號，而那是下一批的事。

---

## D051 — 帳本接上了，而它把前面每一個績效數字都往下修

**日期**：2026-08-30
**狀態**：🟢 已實作（P2-2）；🔴 **它推翻的東西比它新增的多**
...
| 數字 | 怎麼來的 |
|---|---|
| 成交時間 | 靠「利率相同 ＋ 開倉時間落在掛單存活區間」**配對推出來的**（D045） |
| 還款時間 | 靠**巡檢偵測**，每筆被高估最多 10 分鐘（D040） |
| 實得年化 | 上面兩者相乘 |
| 回測的實得年化（D049） | 建立在上面那些推論之上，**驗收標準也是對照它們** |

**那是一條沒有錨點的推論鏈。** D049 的 `--verify` 全綠只證明「重播沒有偏離
正式環境」，不證明正式環境的數字對——**兩邊會一起錯**，那句話當時就寫下來了。

### 🔴 錨放下去之後：每一層推論都偏樂觀

| 來源 | 實得年化 | 分母裡少了什麼 |
|---|---|---|
| `wait_report`（D045） | **7.99%** | 沒等到成交的期間（自己標註過偏樂觀） |
| 回測 `run_policy`（D049，P=48） | **6.54%** | 成交時點是模擬的，而模擬**偏快**（0.93×） |
| **交易所帳本** | **5.34%** | **什麼都沒少**（0.75517665 USD／本金 344.31／08-15→08-30 共 15 天） |

**三個數字一路往下，而真金落在全部之下。**
（用上線的確切時刻 08-15 15:09 而非日曆日算是 **5.42%**；報告用日曆日，
因為帳本只給到入帳當下，硬要精確到小時是假精確。）

**這不是說前兩個數字算錯了**——它們各自的算式都對，
**錯的是每一個都少扣了一點東西，而少扣的方向全部相同**。

### B6 預測中了：ledger 是下一個解析重災區

TASKS 早就寫著「**ledger 是下一個回應解析的重災區**，P4-1 建議排在這一項之前」。
2026-08-30 唯讀實打，三件事與官方文件不同或值得記下：

1. 🔴 **`[2]` 不是 placeholder，是錢包名稱**（實測 `'funding'`／`'exchange'`）。
   官方文件把 `[2]`、`[4]`、`[7]` 都標成 placeholder，而 `[4]`／`[7]` 確實是
   `None`。**照文件寫會丟掉唯一能分辨錢包的欄位**——而那個欄位是下面陷阱 2 的解藥。
2. **數值全是字串**（`'0.04203999'`）——**D027 第 3 點的第四次現身**。
3. 🔴 **帳本裡混著別的東西**：27 列裡利息 20、錢包轉帳 6、幣別兌換 1。

### 兩個陷阱，而「只取正數」也擋不住

**陷阱一：把 `amount` 加總。** 實測會得到 343.71，而真正的利息是 0.042
——**差了八千倍**。

**陷阱二：只取正數。** 一樣錯，得到 528.01。
因為**同一筆轉帳會出現兩列、正負相反、掛在不同錢包上**
（`... on wallet funding` ＋ `... on wallet exchange`）。

⚠ **而且「兩列相消」是運氣不是保護**：兩條腿都落在查詢區間內才會相消，
`--since` 切在中間就只撈到一條——那時候帳面會多出 184.3 USD，
**看起來就像某一天賺了 184 塊**。已加測試釘住這個情況。

**所以分類一定要同時看 `description` 與 `wallet`**，兩個都要。

### 同步要能重跑，所以不能用累加的那支介面

`Repository.upsert_daily_earning()`（M3 就寫好的）對同一天是**累加**的，
理由是「同一天可能分多次補入帳」。**但帳本每次都給出那一天的完整金額**，
用累加的話**重跑一次就把當天的利息變兩倍**，而且是靜悄悄地變（D026 那一族）。

新增 `set_daily_earning()`（覆蓋語意）給帳本同步用。
**同步一定會被重跑**——補歷史、修 bug、換機器都會重跑。
實測連跑兩次，合計都是 `0.75517665`。

⚠ **`daily_earnings()` 必須先把同一天的多筆合併再寫**：覆蓋語意下，
沒先合併的話同一天的第二筆會蓋掉第一筆。已加測試。

### 寫報告時我自己踩了兩次同一個坑

**都是「分母少扣東西」，也就是這一則在講的那個病**：

1. **分母的起點用了「第一筆入帳」而不是「期間的開始」。** 錢 08-15 就進來了，
   第一筆利息 08-17 才入帳——用第一筆當起點等於把「錢進來了但還沒借出去」的
   兩天從分母裡刪掉，算出 **6.16%** 而不是 5.34%。
   改成 `--since` 給了就從那天算起，**沒給就印一行紅字說這個數字偏樂觀**。
2. **跨過 7 週的空窗期照算。** 2026-06-30 到 08-17 之間錢被搬去 exchange 錢包，
   全期算出來是 2.18%，那個數字沒有意義。現在會印出「期間內有幾天完全沒有
   利息入帳」。

**同一個病在同一天出現在三個地方**（`wait_report`、回測、這支報告）
——它不是某一次的疏忽，是這類算式的預設失敗模式。

### 沒有做的事

- **沒有接進主迴圈。** 目前是手動跑 `scripts/sync_earnings.py --write`。
  要不要讓機器人自己每天同步一次，是另一個決定：它牽涉到
  「帳本查詢失敗時該不該影響巡檢」，而那不該順手決定。
- **沒有算平均本金。** `principal_avg` 目前是呼叫端給的常數。
  帳本只看得到餘額，而餘額含已賺到的利息、也含還掛在場上沒借出去的錢
  ——**猜一個本金出來，這個數字就又變成推論了**，那就失去這一則的意義。
- **P2-4 的日結摘要仍未做**，但它等的利息資料源現在有了。

---

## D052 — 帳本同步放在迴圈層，而且吞掉所有例外
...
第一直覺是放進 `run_once()` 的結尾。**那會永遠不執行。**

`run_once()` 有**五條提早 `raise SkipCycleError` 的路**（策略無計畫、維持場上既有
掛單、重掛不划算……），而目前正式環境**幾乎每一輪都走其中一條**
——場上有單、候選價位沒變 → 略過。放在結尾等於寫了不會跑的程式碼。

**這正是 D038 踩過的同一課**：閒置量測原本擺在提早 return 之後，
於是**永遠量不到閒置最久的那些輪**——也就是最需要它的那些輪。

所以放在 `run_forever()` 的迴圈裡，`run_once()` 的例外分類**之後**、
`time.sleep()` 之前。那個位置每一種結果都會經過（正常、略過、可重試錯誤），
只有 `FatalError` 不會——而那時候機器人本來就要停了。

**額外的好處：它完全不在掛單那條路徑上。** 定價、重掛、取消、下單四條路
一行都沒有被碰到。

### 🔴 吞掉所有 `Exception`，這是它存在的前提
...
**不是防禦性寫法，是設計的一部分**：收益統計是**事後才看**的東西，
而掛單是**現在正在跑**的錢。**讓一份報表把一台在管真金的機器人弄停，
任何理由都不划算。**

`KeyboardInterrupt` 那一族（`BaseException`）刻意不吞——那是使用者要它停。

**也不可以讓它累積 `consecutive_failures`**：連續三次就會推一則「機器人可能壞了」
的告警，而機器人好得很。已加測試。

### ⚠ 而寬鬆的 `except` 立刻就咬了我一口

實作當下漏掉了 `from core import earnings` 這一行（改檔的腳本在前一個
assert 就中止了，而我沒檢查）。結果是：

- **機器人照跑、日誌只有一行 warning、測試全綠**——因為那個 `NameError`
  被自己的 `except Exception` 吞掉了。
- 抓到它的是**兩條「成功時真的有寫進去」的正向測試**，不是那六條「失敗時不要爆」
  的測試。

**教訓寫死在這裡：寬鬆的 `except` 一定要配一條證明 happy path 真的有效的測試。**
只測「壞掉時不會爆」的話，一個從來沒成功過的功能會完美通過所有測試
——**這是 D026「靜默失效」在測試設計上的變形。**

### 節流：時間戳記在同步之前，不是成功之後

```python
self._last_earnings_sync_at = now   # 先記，再同步
```

**一個一直失敗的同步不該變成每 10 分鐘打一次交易所。** 失敗了就等下一個
區間再說——反正同步是冪等的（D051 的覆蓋語意），晚一點補上沒有代價。

### 節流狀態只放記憶體，不動 `bot_state` 的 schema

重啟後會多同步一次。**而那沒有代價**——同步是冪等的，寫進去的值一模一樣。
拿「多一個要維護的欄位 ＋ 一次 schema 變更」去換「重啟後少打一次唯讀 API」，
不划算。

預設 6 小時（一天 4 次），而交易所一天只結一次息（約 09:30 CST）
——這個頻率不是為了跟上結息，是為了不必等太久。設 `earnings_sync_hours: 0` 關掉。

### 本金一律不填

`principal_avg` 傳 `None`。理由同 D051：帳本只看得到餘額，而餘額含已賺到的利息、
也含還掛在場上沒借出去的錢。**猜一個本金出來，`earnings_daily` 就又變成推論了**
——那正好毀掉 D051 存在的意義。要算實得年化，由呼叫端把本金講清楚
（`scripts/sync_earnings.py --principal`）。

---

## D053 — 日結收益摘要：判準是資料，不是時鐘

**日期**：2026-08-30
**狀態**：🟢 已實作（P2-4 最後一塊）

P2-4 從 2026-08-16 起就標著「只剩日結摘要，仍待 P2-2 的利息資料源」。
D051／D052 把資料源接上之後，這一塊終於做得了。
**`CATEGORY_EARNINGS` 與 📊 圖示從 D029 就定義好了，一直沒有東西可以放。**

### 什麼時候推：不是「每天 10:00」

**時鐘式的觸發在兩件事上會出錯**：

1. 機器人在那個時間剛好在重啟 → **整天不推**。
   （08-30 這一天部署了五次，時鐘式的很容易正好錯過。）
2. 交易所結息時間漂移 → **推到一天還沒有資料的空摘要**。

改成事件式：**`earnings_daily` 出現了沒推過的日期就推**。
結息一落地就推，而且**停機幾天回來只補最新的一則**，不會連發
——停機三天推三則，是把額度花在沒有人會回頭看的東西上。

### 狀態存 DB，不存記憶體

存記憶體的話，**每次部署都會重推一則**。08-30 一天部署五次 = 五則，
而 LINE 免費方案一個月只有 200 則（D024）。

**存哪裡：新表 `bot_kv`，不是往 `bot_state` 加欄位。**
這個專案沒有 migration 機制，`_create_schema()` 跑的是 `CREATE TABLE IF NOT EXISTS`
——**對已經存在的表，在 CREATE 語句裡新增欄位完全沒有效果**，
正式環境那顆 DB 不會長出新欄位，**而且不會有任何錯誤訊息**（D026 那一族）。
新表則天然安全。

⚠ `bot_kv` **不是什麼都能塞的抽屜**：會被查詢、會被統計的東西應該有自己的表和欄位。
目前唯一的用途就是這一個鍵。

### 先記再推

```python
self.repository.set_kv(self.DIGEST_STATE_KEY, latest["date"])   # 先記
self.notifier.send(...)                                          # 再推
```

**一個推不出去的摘要，每 10 分鐘重試一次，一天就是 144 則。**
與 D052 的節流是同一條理由，只是那裡燒的是 API 呼叫，這裡燒的是 LINE 額度。

### 年化那一行：分母是錢包餘額，而且標籤要講清楚

D051 立過「**不猜本金**」的規矩。所以摘要裡那一行寫的是
**「相對錢包餘額的年化」**，分母是當天收盤的錢包餘額——那是帳本上的事實。

**它會讓年化偏低**（沒借出去的錢也在分母裡），而**偏低是可接受的方向**：
這個專案已經有三個數字因為分母少扣東西而偏樂觀（D051）。

🔴 **刻意不寫「實得年化」四個字。** 那個詞在這個專案有明確定義
（要扣掉沒借出去的時間），而這則訊息算不出來——寫了就會被當成實得年化在讀，
那條好不容易放下去的錨就又鬆了。要完整數字的人，訊息最後一行給了指令。

**餘額取不到就整行略過**，不印一個來歷不明的百分比。**少一行，好過多一個假數字。**

### 與掛單的隔離同 D052

吞掉所有 `Exception`、受 `line.push_trade_events` 總開關管（它同樣是花額度的推播）。
**一則通知不可以把一台在管真金的機器人弄停。**

### 額度試算

日結 30 則/月 ＋ 交易面事件（實測一天 0～2 則）30～60 ＋ 系統告警預留 30
≈ 90～120 則，落在 200 內。**餘裕仍然不大**，這是 D024 一路的處境。

---

## D054 — 訊號找到了，而且偵測得很好；但拿它降價是賠錢的

**日期**：2026-08-30
**狀態**：🟡 **上半場成功、下半場失敗**，兩半同等重要
🔴 **附帶一個更重要的方法論結論：D049／D050 的比較方式有系統性缺陷**

### 起點：D050 說「不要挑門檻，要換訊號」

D050 量到候選價位是個**落後**訊號（最長那段空掛裡前 32 小時完全沒動），
並列了三個候選訊號。其中兩個（短窗成交價中位數、簿子最高可見價）
**在現有資料上量不了**——`market_snapshots` 2026-08-23 才開始寫，
而 5 段長等待裡有 3 段發生在那之前。**又是 D048 那個形狀。**

第三個（「我的價位最近被掃到幾次」）只需要 K 線，5 段全部量得到。

### 上半場：`stale_ratio` 偵測得非常好

定義：`距離上次命中的小時數 ÷ (窗長 ÷ 窗內命中次數)`
——**「我已經等了常態間隔的幾倍」**。

**為什麼要正規化**：D046 警告過「不要用閒置時間」，而那個警告是對「往上調價」
說的；它自己也寫了「等太久支持降價」。但光看小時數不夠——同樣躺 10 小時，
在常態間隔 2 小時的價位上是異常，在 12 小時的價位上是正常。

| | 候選價位（現況） | `stale_ratio ≥ 3×` |
|---|---|---|
| 5 段長等待（≥6h） | **只有 1 段發話，且在第 30h** | **5／5 發話，第 0～11h** |
| 10 段快速成交 | 0 段 | **0 段（零誤報）** |

✅ **而且門檻從 1.5× 到 4× 都是同一個結果**（5/5 命中、0/10 誤報）。
**這是它與 `target_queue_usd` 的關鍵差別：不是挑出來的一個點，是一整片高原。**

### 🔴 下半場：拿它去降價，賠錢

政策：躺超過門檻就改掛「最近 24 小時**真的被掃到過**的最高價」。

**單跑一次看起來是贏的**（6.57% → 7.28%，+0.70pp，9 種參數組合全部贏）。
**把相位運氣平均掉之後是輸的：**
...
勝率只有 **7／22**，最好 +2.24pp、**最差 −4.67pp**。

**機制講得通**：降價會把一個較差的利率**鎖住最多 48 小時**，
而省下來的只是一段等待。**贏很多次、輸很大次。**

⚠ **只看勝率會得到相反的結論**：`lookback=12` 贏過半數的起跑點（14/22），
平均仍然是輸的。**平均與勝率一定要一起看。**

### 🔴 最重要的產出：D049／D050 的比較方式有系統性缺陷

**同一個政策，單一起跑點 +0.70pp，跨起跑點平均 −2.16pp。符號是反的。**

成因：在只有十幾次循環的歷史上，**改掛一次就會把後面每一次循環的進場時點
整個推移**。於是「政策 A vs 政策 B」比到的有很大一部分是
**「誰運氣好、剛好踏在好的進場點上」**，不是政策本身。

實例（D050 的後半段資料）：加了重掛之後，某次循環從 08-25 挪到 08-23，
結果**踏進一段 173 小時完全沒成交的死區**——實得年化從 5.93% 掉到 2.29%。
**那不是政策差，是相位差。**

**所以此後比較政策一律要用 `run_policy_across_starts()`，不可以單跑一次。**

⚠ **這不推翻 D049 的方向結論**（「48 太高」在四種持有模型下都成立，
那是換自變數不是換相位），**但它推翻 D050「六種政策分不出勝負」的力度**
——那個「分不出勝負」是單一起跑點的結果，只是恰好與跨起跑點的結論同向
（都沒有贏），所以結論還站得住，**理由要換掉**。

### 目前的結論

1. **不要把 `stale_ratio` 接上降價。** 量出來是賠錢的。
2. **但它是個好偵測器**，留在程式庫裡（`down_when_stale`）供重跑與推翻。
   它也許該用在別的地方——例如**下次掛單前先問「這個價位現在正常嗎」**
   （08-18 那段在掛出當下 `stale_ratio` 就已經 4.04×，等於掛在一個已經逾期的價位上）。
   ⚠ **那是一個還沒被量過的想法，寫在這裡不代表它會贏。**
3. **`run_policy_across_starts()` 是這次真正該留下來的東西。**

---

## D055 — 用對方法重做之後，D1 的證據夠了：那個 48 該改

**日期**：2026-08-30
**狀態**：🟡 **證據已足夠，但改不改由使用者決定**——這是定價路徑
🔴 **它推翻 D049 的兩條「不能改」的理由，並收窄 D054 的推論**

### 起因：D054 發現單一起跑點的比較會被相位運氣主導

D054 量到「同一個政策，單一起跑點 +0.70pp、跨起跑點 −2.16pp，符號相反」。
**而 D049 的兩條「不能改」的理由，都是單跑一次得到的。** 所以重做。

### 重做的結果：兩條理由都不成立

| D049 說的（單跑一次） | 跨起跑點重做 |
|---|---|
| **切半之後前半的結論反過來** | **全段／前半／後半三段全部同向**（P=11.61 都贏約 1.0～1.1pp） |
| **細掃是雜訊，曲線不單調** | **從 48h 往下單調上升**，48→6.20%、32→6.82%、24→6.99%、20→7.22%、16→7.44%、**14→7.45%（峰）** |

**D049 的方向結論是對的，兩條攔阻它的理由是方法造成的假象。**

### 🔴 而且有一片高原，這正是 `target_queue_usd` 失敗模式的反面

| 假設 P | 8h | 12h | 16h | 20h |
|---|---|---|---|---|
| 實得年化 | 7.39% | 7.41% | 7.44% | 7.22% |

**8～20h 之間差不到 0.2 個百分點——任何落在裡面的值都一樣好。**

D032 的教訓是「`target_queue_usd` 是人手算出來的**答案**，市場一變它就過期」，
而那個數字**沒有高原**：換一個值結果就不同。這次相反。

🟢 **而實測持有的平均（16.93h）與中位數（11.61h）都落在高原裡。**
所以「換成實測值」不再是憑空拍一個常數——**是落在一片被量出來的平坦區間中間**。

### 第二條獨立證據：不看任何訊號，單純把價位往下平移

| 平移 | +10% | 0% | −5% | **−10%** | −20% | −30% |
|---|---|---|---|---|---|---|
| 實得年化 | 4.69% | 6.20% | 7.03% | **7.17%** | 7.09% | 6.53% |

**單峰、平滑、最佳點在 −10% 附近。** 而把 `P` 換成實測值會讓選中價位下移約 **16%**。

🟢 **兩條完全獨立的路徑（改模型假設 vs 直接動價格）指向同一個量級。**
這是這次最強的一組證據——它不依賴 `P` 這個參數的任何理論意義。

### 🔴 順帶收窄 D054 的推論

D054 量到兩個「因為等太久就降價」的機制都賠錢，我當時差點寫成
**「任何往下走的機制都賠錢」**。**對照組推翻了那個推論**：單純往下平移是賺的。

**輸的不是方向，是那兩個機制**——它們反應的是「已經等很久」，
於是**先燒掉一段等待才降**，而且降幅由乾旱深度決定（容易過頭）。
**一個從一開始就便宜 10% 的報價，跟一個等了 12 小時才降 30% 的報價，不是同一件事。**

### 還沒被消掉的三個保留

1. **只有 16.8 天。** 高原與單調性都很乾淨，但樣本就是這麼多。
2. **模擬的成交偏快 0.93×**（D049 驗證過）。這影響絕對水準；
   方向上它對所有假設一視同仁，但**沒有證明它不會偏袒某一邊**。
3. **`P` 一直在替 `W` 的偏差擦屁股**（D049 的那張表、D047 的乾旱回饋圈）。
   **改 `P` 是在調一個補償項，不是修根因。** 真正的修法是同時處理 `W`，
   而那還沒有做。⚠ **這一條不會因為證據變強而消失。**

### 建議

**證據已經夠了：把 `offer_period` 從 2 天（48h）改成落在 8～20h 高原裡的值。**
建議取 **12h（0.5 天）**——它在高原中間、接近實測中位數 11.61h，
而且是個乾淨的天期值。

⚠ **但這是定價路徑的改動，由使用者決定。**
上線後要看的是：成交率（模擬說 91% → 98%）與選中價位中位數（應下移約 15%）。

---

## D056 — 把「合約天期」與「算式裡的 P」拆成兩個設定，並把 P 改成 12 小時

**日期**：2026-08-30
**狀態**：🟢 已實作並上線——**這是 D1 從 2026-08-22 卡到現在的收尾**

### 先講一個差點犯的錯

D055 的建議原文是「**把 `offer_period` 從 2 天改成 12 小時（0.5 天）**」。
**那個建議是錯的，而且錯得很具體**——寫成設定會直接壞掉：

1. `offer_period` 在 `orderbook_depth.py` 是 `int(...)`，**`0.5` 會變成 `0`**；
2. 就算是浮點數，它同時是**送給交易所的合約天期**，而
   **Bitfinex 最短只接受 2 天**——掛不出去。

**成因是一個錯誤的耦合**：`offer_period` 一直身兼兩職。

| 用途 | 語意 | 限制 |
|---|---|---|
| 送給交易所的 `duration` | **合約條款** | 整數、最短 2 天 |
| 算式裡的 `P` | **「會生息多久」的期望值** | 借款人隨時可還，所以合約天期只是上限 |

**兩者本來就不是同一個數。** 綁在一起的後果是：
**想修正模型的假設，就得去改合約條款——而那條路走不通。**

D055 的量測沒有錯（模擬只動到假設那一半），錯的是我把結論翻譯成設定變更時，
沒有先確認那個設定的另一半在做什麼。**回測驗證了假設，沒有驗證改法。**

### 拆法

新增 `strategy.assumed_hold_hours`（小時）。
`offer_period` 維持 2，只負責合約天期。

**沒設新鍵就退回 `offer_period * 24`**——不設定的人行為完全不變。
測試釘住這件事，也釘住「重播只准動假設值、不准動合約天期」。

### 為什麼是 12

- 實測 17 筆已結束部位：**平均 16.93h、中位數 11.61h**（`hold_report.py`），而不是 48h。
- 回測跨 22 個起跑點：**8～20h 是一片高原**，彼此差不到 0.2 個百分點（D055）。
- 12 在高原中間，接近實測中位數。

**高原的存在是這次敢動它的關鍵**——`target_queue_usd` 那次沒有高原，
換一個值結果就不同（D032）。
...
| | 改前 | 改後 |
|---|---|---|
| 選中價位（最後一窗） | **10.91%** | **9.09%**（下移 16.7%） |
| 回測實得年化 | 6.20% | **7.44%** |
| 回測成交率 | 91% | **98%** |
| 送給交易所的天期 | 2 天 | **2 天（不變）** |

🔴 **場上那張 10.91% 的單會被取消重掛。** 候選價位 9.09% 與它差 16.7%，
遠超 2% 容差，所以部署後第一輪就會換單。
**那也表示 D047 那個「已閒置 18 小時」的樣本會就此結束**——
它是右設限樣本，本來就沒有成交時間可收，損失的只是「下界會繼續長」。

### ⚠ 沒有被消掉的保留

1. **只有 16.8 天的歷史**，而且模擬的成交偏快 0.93×。
2. 🔴 **這個 12 有一部分是在補 `W` 的偏差**（D047 的乾旱回饋圈讓等待被低估），
   **不是純粹的「持有時間估計」**。**修了 `W` 之後這個值要重調**
   ——這一條寫在 `config.yaml` 的註解裡，不只寫在這裡。
3. **要退回只要把 `assumed_hold_hours` 拿掉或設成 48**，一行設定。

### 要看什麼才知道有沒有效

- **成交率**：模擬說 91% → 98%。空掛的時數應該明顯變少。
- **選中價位中位數**：應該從 9.11%～10.91% 那一帶下移約 15%。
- **帳本的實得年化**（`scripts/sync_earnings.py`）——**這是唯一不靠推論的裁判**（D051）。
  ⚠ 它需要幾天才看得出差別，**不要拿一兩天的日結去下結論**。

---

## D057 — 持有時間量測把同一筆錢數了兩次：`loan` 與 `credit` 是同一筆錢的兩個狀態

**日期**：2026-08-31
**狀態**：🔴 **已確認的量測缺陷**，而它是 D055／D056 選那個 12 的輸入之一

### 現象：文件寫的 17 筆，其中 3 筆是幽靈

`funding_positions` 至今已結束 **17** 筆，而 `hold_report.py` 透過
`Repository.all_positions()` 取樣時**不分 `kind`**（[db/repository.py](../db/repository.py)
的 `all_positions()` 只濾 `currency`）。攤開來是 **14 筆 `credit` ＋ 3 筆 `loan`**，
而那 3 筆 `loan` 與 3 筆 `credit` **是同一筆錢**：

| `loan` | `credit` | 金額 | 日利率 | `opened_at` |
|---|---|---|---|---|
| `61007698` | `464670446` | 150.00 | 0.00024971 | 2026-08-28T02:07:35 |
| `61007703` | `464670447` | 134.10 | 0.00024971 | 2026-08-28T02:07:46 |
| `61007704` | `464670448` | 60.77 | 0.00024971 | 2026-08-28T02:07:49 |

**金額、利率、`opened_at` 三者逐欄相同，連秒都一樣。** 而且三筆 `loan` 的
`closed_at` 都是 `02:37:51`，正好等於對應 `credit` 的 `first_seen_at`
——**那不是還款，是那筆錢從「已出借未被使用」變成「已被使用」的時刻**。
Bitfinex 的 `funding_loans` 與 `funding_credits` 是同一筆資金的兩個狀態，
不是兩筆生意。

### 後果：三個 0.50 小時的假樣本，把平均往下拉 17%

三筆 `loan` 的「持有時間」各是 **0.50 小時**（02:07 → 02:37），
而它們對應的 `credit` 實際各活到 13:44（11.6 小時）。

| | 文件寫的（17 筆） | 只算 `credit`（14 筆） | 差 |
|---|---|---|---|
| 平均持有 | **16.93h** | **20.45h** | **+3.52h（+17%）** |
| 中位數持有 | 11.61h | 11.62h | +0.01h |
| 平均完成率 | **35.3%** | **42.6%** | **+7.3 個百分點** |

**兩個數字都能逐位元重現出文件裡的值**，所以這不是另一套算法的差異：
14 筆合計 286.24h，加上 3 × 0.5033h ＝ 287.75h，÷17 ＝ **16.93h**；
17 筆排序後第 9 位 ＝ **11.61h**。

### 為什麼中位數幾乎沒動，而這件事仍然重要

三個假樣本都落在分佈的最短端，**中位數對它們幾乎免疫**（動 0.01h），
平均卻被拉走 3.5 小時。這與 D047 觀察到的
「中位數對窗尾補值幾乎免疫」是**同一個統計性質的第二次現身**
——⚠ 但方向的教訓相反：那一次中位數的穩健是優點，這一次它**掩蓋了一個真的錯誤**。
**穩健的統計量會讓缺陷更難被發現。**

### 它對已經上線的 D056 有多少影響

D055 選 12 的論據之一是「實測平均 16.93h 與中位數 11.61h **都落在 8～20h 的高原裡**」。
用修正後的數字：**中位數 11.62h 仍在高原中間，平均 20.45h 則落到高原的上緣外**。

🟡 **所以 D056 的結論方向不變，但它的其中一條支撐變弱了。**
12 當初是往中位數靠的，而中位數幾乎沒動——**如果當初是往平均靠，
現在就得重來**。這次是運氣，不是判斷。

### 修法與界線

- **修的是報表，不是機器人**：`hold_report.py` ／ `wait_report.py` 取樣時要濾掉
  `kind='loan'`，或改成「同金額同利率同 `opened_at` 就視為同一筆」去重。
  ⚠ **不要在 `all_positions()` 裡濾**——那支的 docstring 已經寫明「刻意不在 SQL
  裡過濾，上層永遠不會發現自己只看到一部分」，在那裡加條件會犯它自己警告的錯。
- **`kind='loan'` 的列要留著**，它記錄的是真實發生過的狀態轉換，
  而且 B6 還缺 `funding_loans` 的真實回應校正。**刪資料不是修法。**
- **這是唯讀報表的改動，場上有沒有單都能做。**

### 誠實界線

- 只有一批（3 筆）`loan` 樣本，都在 2026-08-28 02:07 那一次。
  **「`loan` 與 `credit` 一定成對」是從這 3 筆推的**，不是從交易所文件確認的。
- 修正後的 14 筆裡**仍有 1 筆右設限**（`464862123` 生息中），
  所以 20.45h 與 42.6% **本身也是下界**。

---

## D058 — 使用者的假說：`P` 可能不是常數，而是 `r` 的函式

**日期**：2026-08-31
**狀態**：🟡 **方向與資料一致，但強度遠不足以下結論**——列為待量測，不是待實作

### 假說

使用者 2026-08-31 提出：**「較低利率的單子，借款人借款的時間本來就會比較長。」**

**機制是講得通的**：提前還款對借款人是一個選擇權，而利率越高、行使它去找更便宜
的錢的動機越強。便宜的錢沒有理由急著還掉。

**如果它成立，模型的形狀要改，不只是換一個數字**：
現在算式是 `r × P ÷ (W + P)`，`P` 是設定檔裡的一個常數（D056 的 12）。
若 `P` 隨 `r` 下降，算式變成 `r × P(r) ÷ (W(r) + P(r))`，
**高利率會多吃一層懲罰，而模型目前完全看不到這一層**。

🟢 **方向與 D056 同向**：`P(r)` 會讓高價位更不划算，也就是往更低的價位推
——與把 `P` 從 48 調成 12 的效果同向，不是相反。

### 今天量到什麼（14 筆已結束的 `credit`，已依 D057 排除幽靈樣本）

| | Spearman `rho` | n=14 的 5% 顯著門檻 |
|---|---|---|
| 絕對利率 vs 持有時間 | **−0.290** | ±0.54 |
| 相對便宜度（開倉前 24h 市場 `high` 中位數正規化）vs 持有 | **−0.223** | ±0.54 |

**兩個都是負的，方向與假說一致；兩個都離顯著門檻很遠。**

🔴 **而真正致命的是變異數，不是顯著性**：

    利率 0.00024971（年化 9.1144%）逐位元相同的 6 筆，持有時間是
    1.28h、11.61h、11.61h、11.62h、48.26h、48.39h —— 最長是最短的 38 倍。

**同一個價位內部的散佈，已經蓋滿了整個觀測範圍。**
就算 `rho` 是真的，利率能解釋的變異也只有一小塊。

### ⚠ 一個差點被當成證據的東西

第一次分組時得到「便宜組中位數 11.62h vs 昂貴組 6.87h」，看起來漂亮地支持假說。
**那是浮點雜訊**：門檻取中位數 9.11%，而 `0.00024971`（6 筆）與 `0.00024972`（1 筆）
被切到兩組去——**兩者相差小數點後第八位，是同一個價位。**

這正是 STATUS 早就寫過的那個陷阱（「分組會塌成 1 筆／16 筆，而且不會自己好」）
的第三次現身，**而它這次是以「支持假說」的樣子出現的**——比以「無法分組」的樣子
出現危險得多。**分組前要先看相異值有幾個**：14 筆裡只有 **7 個相異利率**。

### 用「相對市場」正規化時踩到的第二個坑

一開始拿**部位存活期間**的市場 `high` 中位數當分母，得到 `rho = +0.002`。
那個數字沒有意義：**分母包含了持有期間本身**，借越久就吃進越多市場資料，
是循環論證。改用**開倉前 24 小時**的窗才是乾淨的（上表那個 −0.223）。

### 所以要做什麼

**不是現在把 `P` 改成函式。** 要做的是：

1. **接縫已經存在，用它**：`core/fill_simulation.py` 的 `HoldModel` 簽章
   就是 `(rate, cycle_index) -> float`，`empirical_hold()` 的 docstring 自己
   寫著「⚠ 它假設 `P` 與 `r` 無關，而那件事至今比不出來……**真正的修法是換掉
   這個函式，不是調它的參數**」。**這一則就是那個函式的規格。**
2. **寫一個 `rate_dependent_hold()`**，在模擬裡與 `empirical_hold()` 對打，
   **走 `run_policy_across_starts()`**（D054 的硬規矩）。
3. **先問「策略會不會因此改變決策」**，再問「哪一個比較賺」。
   若在 8～20h 的高原上 `P(r)` 的變動幅度落在高原內，**它可能根本不改變選價**
   ——那樣的話這一項就結案，不必進正式路徑。

### 誠實界線

- **n=14、7 個相異利率、16 天。** 這份資料量不可能證實或推翻這個假說。
- **樣本全部來自我們自己選出來的窄價帶**（9.11%～10.95% 佔 13/14），
  與 D045 的校準樣本是同一個毛病：**模型只在自己選過的地方留下觀測**。
- **共變因子沒有被排除**：便宜的單多半掛在市場走高的期間，
  「借款人不還」與「市場漲」在這份資料裡分不開（D040 早就警告過同一件事）。
- **最便宜的那筆（5.475%，持有 45.08h）對 `rho` 的貢獻很大**，
  而它是 2026-08-16 賣在區間底部的那一筆（D033），是個異常事件。

---

## D059 — 接下來的工作分三條車道，判準是「會不會改變送到交易所的東西」

**日期**：2026-08-31
**狀態**：🟢 **這是排序的判準，不是某個技術決定**

### 問題：現在有一筆錢鎖在場上，而待辦清單裡什麼能動、什麼不能動沒有講清楚

2026-08-30 22:57 D056 上線，機器人當場把那張 10.91% 換成 **9.00%**，
**33 分鐘後成交**（部位 `464862123`，345.06 USD，08-30 23:30 開倉，
至 08-31 22:41 仍在生息、已持有 **23.2 小時**）。

於是出現一個實際的排程問題：**D056 要觀察，而觀察期間到底能不能改程式？**
——如果答案是「都不能」，那就是好幾天的純等待；
如果答案是「都能」，D056 的效果就永遠歸因不回來。

### 判準：一刀切在「這次改動會不會改變機器人送到交易所的東西」

| | 會不會動到送出去的單 | 什麼時候能做 |
|---|---|---|
| **甲道** | **不會**（唯讀報表、離線回測、預設值＝現行行為的旋鈕） | **隨時**，場上有單也一樣 |
| **乙道** | 不會，但**結論要吃 D056 上線後的新資料** | 要等**完整循環**跑完 |
| **丙道** | **會**（定價路徑） | 要等乙道有結果，**而且要使用者拍板** |

**這條線不是「風險大小」，是「可不可歸因」。**
甲道的改動就算部署了，D056 的觀察也不受污染——這正是 M1-c 當初立的
「行為零變化」驗收（D046 條件 3）的同一套想法，只是這次拿它來排程。

⚠ **甲道不等於零風險**：它一樣會重啟容器。已經實測過重啟不會動到場上的單
（08-30 兩次部署都是同單號、同利率），但**那是觀測，不是保證**。

### 取樣速度：觀察 D056 是以「循環」計，不是以「小時」計

實測：**錢鎖在部位裡的期間 `pricing_decisions` 一列都不寫**
——08-30 23:27 至 08-31 22:41 共 23 小時、**0 列**（M1-b 早就寫過
「這張表幾乎不會長」，這是它的又一次現身）。

**所以 D056 至今的完整循環數是 0**，第 1 個還在生息中。
以目前一天約一個循環的速度，要看出 STATUS 列的那三個數字得**一週以上**。

🔴 **這正是甲道必須現在就做的理由**：觀察期是以天計的，
而那段時間如果什麼都不做，就是純粹的等待。

### 三條車道的內容與優先序

**甲道（現在就做，場上有單也沒關係）**

1. **修 D057 的幽靈樣本** —— 唯讀報表，最先做：它是後面每一項的輸入。
2. **`W` 的旋鈕化**（`ev_wait_statistic`：`mean`｜`median`；
   `ev_censoring`：`window_tail`｜`drop`｜`km`），**預設值＝現行行為**。
   對應 D045 第三則追記與 D047。
3. **`rate_dependent_hold()`**（D058）——純模擬側，機器人碰不到。
4. **`--wait` 聯合掃描報告**：`W` 的 6 種組合 × `P` 的網格，
   **走 `run_policy_across_starts()`**（D054），平均／勝率／最差三個一起印。

**乙道（要等這一筆收回來）**

5. **D056 第一筆的驗收**：等待 0.55h（對照被它取代的那張 10.91% 空掛 24.1h）、
   實際持有多久（目前 23.2h，**已是假設值 12h 的 1.9 倍**）、
   有沒有借到期。⚠ **n=1，只記錄不下結論。**
6. **帳本實得年化**（`sync_earnings.py`）——唯一不靠推論的裁判（D051）。

**丙道（要等乙道，而且要使用者拍板）**

7. **`W` 換預設值 ＋ `P` 重調，一起做**。
   🔴 **不可以各改各的**：D056 的 12 有一部分在補 `W` 的偏差
   （寫在 `config.yaml` 的註解裡），單獨改 `W` 會拆掉抵銷的一半。
8. **若 D058 成立，`P` 改成 `P(r)`** ——**這一項會改變第 7 項的形狀**
   （要找的不是一個更好的常數，是一條曲線），所以要先跑第 3 項再決定第 7 項怎麼收。

**不排進這三道的**：A2-b 的進場守門（D054 最後留的那個想法）與 M2 第 3 步／M3。
兩項都獨立可做，但**都不該插在丙道前面**——A2-b 至今沒有可上線的政策，
而 M3 的基準要等 `W`／`P` 定案才有意義。

### 誠實界線

- **「一天一個循環」是從 D056 上線後的 1 個循環推的**，n=1。
  市場一熱起來循環會快很多（08-28 那天一天走了 4 次）。
- **這條判準管不到「重啟本身的風險」**，只管歸因。

---

## D060 — D056 的驗收：它有效，而且效果比預期大；但它的理由是錯的

**日期**：2026-09-05
**狀態**：🟢 **乙道結案**（乙-1／乙-2 都有答案）；🔴 **而答案推翻了 D055／D056 選那個 12 的依據**

### 資料來源：5 天無人看管的實跑

容器自 2026-08-30 22:57 起 healthy，日誌自 08-22 起 **0 ERROR**，
跑完 **4 個完整循環**。以下每個數字都來自正式 DB 或帳本，**沒有一項是推論**。

### 乙-2：帳本說 D056 賺到了

| 項目 | D056 之前（08-17～08-30） | D056 之後（09-01～09-04 四整天） |
|---|---|---|
| **實得年化（帳本）** | 5.72% | **7.75%** |
| 換手空窗 | 3.6h／38.1h／24.7h | **0.68h／0.26h／0.13h** |
| 資金利用率 | 約 7 成 | **98.8%** |

**手續費對帳**：把 `funding_positions` 逐日攤成推算毛利息，與 `earnings_daily`
的帳本淨利息相比，**合計比值 81.2%**——與 Bitfinex 抽 15% 一致
（日對日會跳，因為利息是結算日入帳不是權責日）。**這是第一次有人對過這個比值。**

🔴 **瓶頸換人了。** 以前的敵人是「借不出去」（D035 那個空轉一整天的世界），
現在空窗接近 0，**唯一剩下的槓桿是價格**。而手續費是常數，改不動。

### 乙-1：而 `assumed_hold_hours: 12` 被實測否定

D056 後結清 3 筆 `credit`：

| 部位 | 年化 | 持有 | |
|---|---|---|---|
| `464862123` | 9.00% | **48.33h** | 借滿 2 天 |
| `464975916` | 8.93% | **48.19h** | 借滿 2 天 |
| `465097968` | 8.76% | 1.43h | 提前還款 |
| `465110649` | 8.76% | 39.3h | **仍在生息** |

**在 8.76～9.00% 這一帶，持有時間是 48h，不是 12h。**

### 為什麼「有效」與「理由是錯的」可以同時成立

D056 把 `P` 從 48 調到 12，效果是**把價格壓低、讓單子秒成交**。
它賺到的是**利用率**（7 成 → 98.8%），不是它宣稱的「持有時間就是 12 小時」。

**兩件事的方向剛好在當時同號**，所以 D055 的回測看起來是對的——
但等到利用率撞到 100% 的天花板，這個理由就沒有東西可以再換了，
而它壓低價格的副作用會繼續累積（見 D062）。

### 誠實界線

- **n=3（＋1 筆生息中）**。「借滿 48h」出現 2 次，不足以宣稱它是常態。
- **09-01～09-04 只有四天**，而 `earnings_daily` 在 08-16／08-20／08-21／08-31
  缺列（結算日不是每天都有）。多日合計才有意義，**日對日不要拿來比**。
- 5.72% 與 7.75% 之間隔著一個市場環境變化，**不是控制實驗**。

---

## D061 — 選中哪個價位，目前是由浮點雜訊決定的

**日期**：2026-09-05
**狀態**：🔴 **已確認的缺陷**，改法明確、風險極低，**但它會改變送出去的單（丙道）**

### 現象：正式環境在 8.76% 與 8.93% 之間來回跳

拿正式 DB 的最新一窗（552 根 K）跑 `expected_value.py` 本尊：

```
 8.7600%  實質=7.695058824   <== 選中
 8.9279%  實質=7.695017700
 8.7366%  實質=7.677884150
 8.8075%  實質=7.643486526
```

**前兩名差 4×10⁻⁵ 個百分點**，也就是第 6 位有效數字。
距最佳 1% 以內有 4 個候選，橫跨 **8.74%～8.93%**。

### 兩個成因，一個比一個更該修

**成因一：實質年化曲線在 8.3～9.0% 這一段是平的。**
`P=12` 之下，8.29% 到 9.00% 全部落在 7.50～7.69 之間——
**71 個基點的價差，換來 0.19pp 的目標函數差**。

**成因二：平手時系統性地選便宜的那一個。**
`choose_rate()` 的比較是 `if effective > best_effective`，而候選是
`sorted({...})` 升冪掃描。**嚴格大於 ＋ 升冪 = 平手保留先到的那個 = 最便宜的那個。**
這不是誰寫錯了，是「取最大值」的預設寫法**在高原上會有方向**，而沒有人挑過它的方向。

### 為什麼「用 4×10⁻⁵ 挑價位」是錯的

同一份資料裡，**`W` 的估計誤差是 4～9 倍**（12 個樣本，`mean` 中位高估 9.0×、
`median` 中位高估 3.9×）。**拿一個誤差以「倍」計的量，去分辨第 6 位有效數字的差別，
是在對雜訊做決策。** 這與 D032 記的 `target_queue_usd` 是同一個病的另一面：
那次是「拍一個門檻」，這次是「拍一個小數點後第六位」。

### 決定：高原容差 ＋ 平手偏高

在 `choose_rate()` 裡把「取最大」改成
**「落在最佳值 x% 以內的候選中，取價格最高的那一個」**。

- **為什麼偏高而不是偏低**：高原內的目標函數差是雜訊，但**價差是真的**
  ——8.93% 比 8.76% 多 1.9% 的利息，而它落在同一片高原上。
- **為什麼不是「隨機挑」**：隨機挑會讓每輪的候選變動變成重掛，
  而重掛有空窗成本（D034）。取最高是**確定性的**，同一窗永遠給同一個答案。
- **`x` 要當設定寫出來**，而且**預設值不是零**——零就是現行行為，
  而現行行為已經被證明是在挑雜訊。

🔴 **這一項會改變送到交易所的單（D059 的丙道）**，但它**不依賴 `P(r)` 的答案**
——不論 `P` 最後是多少，「不要用第 6 位有效數字挑價位」都成立。
**這是唯一一個可以先做的定價改動。**

### 實作結果（2026-09-05，PR #61，**已上線**）

**新增 `strategy.ev_plateau_tolerance_pct`，正式環境設 0.5**。
`choose_rate()` 改成呼叫 `_pick_from_plateau()`：把 `effective >= 最佳值 ×
(1 − 容差)` 的候選視為同分，在同分的那一群裡**取價格最高的那一個**。

🔴 **類別預設是 0，正式環境的值寫在 `config.yaml`。** 慣例與 `assumed_hold_hours`
相同，但這裡還有一個更重要的理由：**回測的迴歸釘子釘的是「當時那個策略會選什麼」**
（D049／D050／D054／D055 共 17 條）。把類別預設改成非零，那 17 條會同時換值，
於是「D054 的結論還成不成立」與「新旋鈕好不好」變成同一個問題——
**那正是 D036 記下的病：一次改兩件事，之後分不出是誰造成的。**

### 🔴 為什麼是 0.5 而不是 1.0：量出來的答案跟直覺相反

第一次量的是**選價會漲多少**，1.0% 看起來明顯比 0.5% 好
（中位漲幅 +0.141pp 對 +0.031pp）。**那個表沒有把等待成本算進去。**

跨 **113 個起跑點**、三種持有模型、再切半各跑一次（D054 的硬規矩）：

| 容差 | 實測分佈 | 固定 48h | 固定 22.6h | 前半 | 後半 |
|---|---|---|---|---|---|
| **0.5%** | **+0.061pp** | **+0.054pp** | **+0.064pp** | **+0.076pp** | **+0.046pp** |
| 1.0% | −0.013pp | +0.061pp | −0.010pp | +0.088pp | **−0.111pp** |
| 2.0% | −0.041pp | +0.031pp | +0.020pp | +0.086pp | **−0.167pp** |

**只有 0.5% 全部同向。** 1.0% 的符號在持有模型之間會翻、在切半之間也會翻
——**那正是 D054／D055 說的「相位假象」的長相**。

🔴 **而 1.0% 贏 94/113 個起跑點卻平均是負的**——「贏很多次、輸很大次」。
**這是 D054 那個陷阱的第二次現身：只看勝率會得到相反的結論。**
...
|---|---|---|
| 0（現行） | 8.76% | 1 |
| **0.5（新設定）** | **8.93%** | 3（8.74～8.93%） |
| 1.0 | 8.93% | 4 |
| 2.0 | 9.00% | 18（8.32～9.00%） |

**0.5% 正好修掉那個 4×10⁻⁵ 的平手**，而且不會把高原拉寬到 18 個候選。

### 誠實界線

- ⚠ **效果很小**：平均 +0.06pp，本金 345 USD 一年約 **0.2 美元**。
  **它值得做的理由不是這 0.06pp，是「不要用第 6 位有效數字挑價位」。**
- ⚠ **裁判本身有已知缺陷（D062）**：模擬器的持有時間與利率無關，
  而漲價正好是 `P(r)` 會影響的那件事。**0.5% 在三種持有模型下都為正**，
  所以它不是靠 `empirical_hold()` 的特定形狀撐著——**但那不是免疫**。
- ⚠ **`_plans_match()` 的重掛容差也是 2%**，而 8.76% → 8.93% 的漂移是 1.94%
  ——**原本就卡在那條線的內側**。改成 0.5% 之後選價變穩定（同一窗永遠同一答案），
  這個風險是往下降的，但**那是推論，不是量到的**。
- **退場鍵**：把 `ev_plateau_tolerance_pct` 設成 0 或刪掉該鍵。

### 🔴 上線當天就咬了一口：三個報告端各自重算了一次「誰被選中」

**PR #61 合併後幾小時內查出來的**（資金還鎖在部位裡，一列壞資料都還沒寫）。

`describe_decision()`／`chosen_forecast()`／`pricing_decision()` 三處都寫著

    chosen = max(self.last_evaluation, key=lambda item: item["effective"])

**在容差存在之前那是對的**——選中的就是最高分的那一個。容差一加，
`choose_rate()` 選的是**高原上緣**，而三個報告端還在報**最高分**。實測：

| | 值 |
|---|---|
| `choose_rate()` 真的會掛 | **8.9279%** |
| `pricing_decision()` 說 | 8.76% |
| `chosen_forecast()` 說 | 8.76% |
| `describe_decision()` 說 | 8.76% |

🔴 **三個都錯成同一個值，所以看起來完全正常。** 日誌、`offer_wait_forecasts`、
`pricing_decisions` 會**一致地**記下一個機器人沒有掛過的價格
——D026 那個家族的典型長相，這次是第六次現身。

🔴 **879 條測試全綠，而且一條都沒有比對過「報出來的」與「掛出去的」是不是同一個數字。**
每一條都只驗其中一邊。**這不是測試不夠多，是缺了那個不變式。**

**修法不是把三處各改一次**（那正是這個 bug 的成因）——是讓「誰被選中」
**只有一個來源**：`choose_rate()` 寫進 `self.last_chosen`，其他人讀。
它是第五個「本輪的狀態」，重置點與另外四個在同一處（D041 的規矩）。

⚠ **守門條件也跟著換**：三個報告端原本檢查 `if not self.last_evaluation`，
現在檢查 `if self.last_chosen is None`。**「評估過一輪」與「選出了價位」是兩件事**
——整組候選的實質年化都是 0 時，前者非空而後者是 None。

### 驗收條件（寫在合併當下，**「沒發生」不等於通過**——D044 的規矩）

**部署當下場上沒有掛單**（錢在部位 `465110649` 裡），所以部署本身沒有動到
任何送出去的單。**要驗的是下一次換手那一輪**：

| # | 條件 | 怎麼看 |
|---|---|---|
| 1 | 選中價位不再是高原的**下緣** | `pricing_decisions.chosen_rate` 應為 **8.93%** 而非 8.76%（同一窗） |
| 2 | 選價**變穩定**：同一窗永遠同一答案 | 連續數輪的 `chosen_rate` 不再在 8.76 ↔ 8.93 之間跳 |
| 3 | **沒有因此多出重掛** | `repost_comparisons.action` 不應出現由這次改動引發的 `repost` |

⚠ **條件 1 要等資金回來才驗得到**（約 09-06 02:30）。
⚠ **效果太小，驗不出「賺比較多」**：+0.06pp 在 345 USD 上是一年 0.2 美元，
**遠在帳本的雜訊之下**。這三條驗的是**行為**，不是收益。

### ✅ 驗收結果（2026-09-06，三條全過）

資金於 09-06 02:39 回到錢包，**觸發了 D061 上線後的第一次真實定價**。

| # | 條件 | 結果 |
|---|---|---|
| 1 | 選中價位不是高原下緣 | ✅ **8.9279%**（4 列定價全部）。**同一窗用舊行為重播是 8.7600%** |
| 2 | 選價變穩定 | ✅ **4 列的相異選價只有 1 個**，沒有在 8.76 ↔ 8.93 之間跳 |
| 3 | 沒有多出重掛 | ✅ `repost_comparisons` 只有 1 列且是 `hold_matched`，**沒有 `repost`** |

**順帶驗到 D064**：那 4 列的 `pricing_knobs_json` 有值了，內容是完整的九個旋鈕
——**第一列「自己記得當時設定」的紀錄**。在此之前 40 列全是靠慣例回推的。

⚠ **這三條驗的是行為，而行為驗過了。收益驗不出來，也不打算驗**
——+0.06pp 在 345 USD 上一年 0.2 美元，遠在帳本雜訊之下。

---

## D062 — 目標函數是自我矛盾的：`P` 是常數，但它依賴的正是被最佳化的那個變數

**日期**：2026-09-05
**狀態**：🔴 **D058 從「使用者的假說」升級為「目標函數的定義問題」**

### 證據一：`P` 選多少，直接決定選中哪個價位

同一窗、只改 `assumed_hold_hours`：

```
  掛價% |  P=12h  P=24h  P=48h  P=72h        P 假設 → 選中價
   8.38 |   7.58   7.96   8.17   8.24
   8.76 |   7.69   8.19   8.47   8.56          12h → 8.76%
   8.93 |   7.69   8.26   8.58   8.70          24h → 8.93%
   9.12 |   7.32   8.12   8.60   8.77          36h → 8.93%
   9.40 |   6.96   8.00   8.64   8.88          48h → 9.40%
  10.00 |   5.66   7.23   8.39   8.87
  10.22 |   4.32   6.11   7.70   8.44          72h → 10.22%
```

### 證據二：而 `P` 本身隨 `r` 變動

`rho(利率, 持有) = −0.383`（n=17，D058 當時是 −0.29／n=14）。逐筆看：

| 年化 | 持有 |
|---|---|
| 10.95% | 1.98h |
| 9.96% | 6.87h |
| 9.00%／8.93% | **48.3h／48.2h（都借滿）** |

**機制是合理的，不是巧合**：融資是可替換的商品，借款人拿到便宜的錢就抱著，
拿到貴的錢一有更便宜的就換掉。**貴的單被提前還款是市場在做它該做的事。**

### 於是問題不是「該填哪個數字」

照實測把 `P` 設成 48h，策略就跳到 9.40%——**但那個 48h 是在 8.9% 觀察到的，
9.40% 的持有時間我們沒有量過**。填 48 就會跳到一個「48 這個依據不成立」的價位上。

> **要解的是 `max_r  r · P(r) / (W(r) + P(r))`，這是不動點問題，不是參數問題。**
> **用一個常數 `P` 去最佳化 `r`，而 `P` 又是 `r` 的函式——目標函數在定義上就是自我矛盾的。**

這是 D058 的接縫（`HoldModel` 的簽章本來就是 `(rate, cycle_index) -> float`）
第一次有足夠證據說「非做不可」，而不是「值得量一量」。

### 🔴 而且現在的回測不能當裁判

`--simulate` 目前說 `P=48 → 6.18%`、`P=11.61 → 7.48%`，看起來支持「賣便宜是對的」。
**但那個模擬的持有時間是 `empirical_hold()` 從實測分佈整包抽的，與利率無關**
——**等於給高價位配了一個它拿不到的持有時間**。

**用一個假設 `P ⊥ r` 的模擬器，去檢驗「`P` 是不是 `r` 的函式」，是用結論證明結論。**
所以 `rate_dependent_hold()` 與 `empirical_hold()` 的修正**必須同一批做**，
否則做完前者仍然沒有東西能裁判它。

### 誠實界線

- **n=17，只有 7 個相異利率**（D058 已經警告過）；`−0.383` 與 `−0.29` 的差別
  來自新增的 3 筆，**不要把係數本身當成量到的東西**。
- **同一個價位內部差 38 倍**（9.11% 那組：1.28h ～ 48.39h）。
  `P(r)` 就算成立，它描述的也是**分佈的位移**，不是一條確定的曲線。
- 🔴 **修正的方向對「賣便宜」是有利的**：`P` 隨 `r` 遞減，代表現在的模擬器
  **高估**了高價位。修完之後很可能更支持現在的低價——**這一項不是為了漲價，
  是為了讓「該不該漲價」這個問題第一次有意義。**

### 實作結果（2026-09-05，PR #64）：**修好了，而結論沒有翻**

`rate_dependent_hold()` 上線（純模擬側，機器人碰不到）。
`OBSERVED_HOLD_SAMPLES` 是同一批 17 筆，**但帶著它們當初的掛單利率**
——舊的那份把利率丟掉了，於是任何用它的模擬都內建了 `P ⊥ r`。

**分界線不能用利率中位數**，而這件事是量出來的：17 筆的利率中位數是年化 9.11%，
**而其中 7 筆剛好等於 9.11%**（同一批資金連續重掛的那一段）。
以中位數切會變成 4 筆對 13 筆——**`hold_time.RateSplit.degenerate` 早就在講這件事**。
改切在 9.12% 與 9.50% 之間那道真的空隙（年化 **9.31%**），切出來 12 筆對 5 筆：
...
| 策略假設 `P` | 裁判＝`empirical`（`P ⊥ r`） | 裁判＝`rate_dependent` |
|---|---|---|
| **12h** | **8.119%** | **8.149%** |
| 22.6h | 7.407% | 7.490% |
| 48h | 6.279% | 6.190% |
...
**兩邊都指向 `P=12`。** 換裁判動 **0.065pp**，而換假設（12 對 48）動 **1.267pp**
——**差 19.4 倍**。**不是兩個裁判剛好同分，是它們的分歧比要回答的問題小一個量級。**

### 而更關鍵的那一題：如果**策略自己**也用 `P(r)`，選價會不會變

| 策略用的持有模型 | 選中年化 |
|---|---|
| 常數 `P=12`（現行） | **8.93%** |
| 常數 `P=22.6`（實測平均） | 8.93% |
| `P(r)` 用平均：25.29／16.14 | **8.93%** |
| `P(r)` 用中位：18.73／6.87 | **8.93%** |
| `P(r)` 誇大版：48／6（**遠超資料支持**） | 9.12% |

🔴 **原因是結構性的，不是巧合：選中的價位（8.93%）在分界線（9.31%）**底下**，
所以 `P(r)` 的那一階從來沒有被跨過。** 模型在高原上看到的候選全部落在便宜組，
而在那一組裡 `P(r)` 就是一個常數。

**STATUS 自己立的收斂條件因此成立**：「若 `P(r)` 的變動落在那片高原內，
它可能根本不改變選價——那樣這一項就結案。」

### 🔴 但「結案」的理由本身是有條件的，而那個條件正在往回指

整段歷史 509 個決策點裡，**選價 ≥ 9.31% 的只有 14 個（2.8%）**——用現在的 `P=12`。
**而正式環境 08-17 起真的掛出去的 16 張單，有 7 張（43.8%）在 9.31% 以上**
——因為那段期間跑的是 `P=48`。

> **是 D056 把作業點壓到分界線底下，才讓 D062 的疑慮變成無關緊要的。**
> **只要價格再往上走，`P(r)` 立刻重新變成決定性的。**

而「讓價格往上走」正是 D063 的探索要做的事。所以：

- **D062 不是一個可以獨立收掉的改動，它是 D063 的前置條件。**
  探索會把樣本推到分界線以上，**而在那之前，`rate_dependent_hold()` 的昂貴組
  只有 5 筆**——探索的結果要能被讀懂，就得先有這個模型接住它。
- **順序沒有排錯，但理由要換**：先做 D062 不是因為它會改善績效
  （量出來是 0.065pp，雜訊），是因為**沒有它，D063 的產出無處可去**。

### 誠實界線

- **昂貴組只有 5 筆**，其中一筆是 9.50% 借滿 48.56h。**它做的是分佈的位移，
  不是一條曲線**——光是 9.11% 那一個價位，內部就從 1.28h 到 48.39h，**差 38 倍**。
- **組內的變異比組間的差距大得多。** 這句話是這個函式能宣稱什麼的上限。
- **「結論沒有翻」這件事本身也會漂。** 換樣本、換分界線都可能讓它翻，
  所以它被釘成迴歸測試（`TestD062換掉裁判之後結論沒有翻`）。
- 🔴 **那條迴歸測試第一次寫成單一起跑點時，量到「換裁判 0.24pp、換假設 0.36pp」
  ——看起來同一個量級**。跨 24 個起跑點平均掉之後才是 0.065 對 1.267。
  **D054 的相位運氣在這裡第三次現身。**

---

## D063 — 所有樣本都是模型自己選出來的，所以量再久也不會有新資訊

**日期**：2026-09-05
**狀態**：🔴 **這是 D045／D047／D058 三者共同的上游病因**

### 兩個各自獨立的現身

**一、等待校準的 12 個樣本，全部落在機器人自己選出來的窄帶**

| 掛單價位 | 樣本數 |
|---|---|
| 8.76 ～ 9.50% | **10** |
| 10.91 ～ 10.95% | 2（**而且是意外**：D056 上線前那兩天的高價實驗） |

而那 2 個意外樣本，正好是 D058／D062 的**全部關鍵證據**。
**我們對 10% 以上的世界，知道的東西完全來自兩次意外。**

**二、M1-c 的儀器長出 187 列，其中 186 列的資訊量是零**

`repost_comparisons` 從 D046 部署後的 0 列，長到 **187 列**——
STATUS 一直在等的「條件 1／2 被觸發」發生了。但：

| 動作 | 列數 |
|---|---|
| `hold_matched`（候選＝場上同價，容差 2%） | **186** |
| `repost` | **1**（而且是 D056 部署自己觸發的，不是市場） |

**候選恆等於場上價，所以「保住 vs 改掛」這個問題每一列的答案都一樣。**
M1-c 問的是「照現在的規則會不會動它」，而現在的規則每輪都重新算出同一個價位。

### 共同的病因

> **模型只在它自己選中的價位上取樣，所以它永遠只會學到「我選的那個價位表現如何」，
> 學不到「我沒選的那些價位會怎樣」。**

這在 D047 已經以另一個面貌出現過（「乾旱會讓等待估計自己變好看」），
在 D045 以第三種面貌出現過（「五筆全落在模型自己選出來的 9.11～9.50% 窄帶」）。
**這是同一件事的第三次現身**，而前兩次都被當成各自的量測缺陷處理。

### 兩個決定

**一、加入探索：每 N 次換手，刻意掛一次高價**

- 取樣位置建議用**窗內 high 的 p75**（目前約 10.5%），
  因為那是「有需求掃到、但我們從來沒站在那裡」的區間。
- **成本算得出來**：假設探索一次多等 8h、每 20 次換手探索 1 次，
  在 48h 的循環裡是 `8/48/20 ≈ 0.8%` 的機會成本，**年化影響 < 0.1pp**。
- **這是付費取樣，不是碰運氣**：它的產出是 `P(r)` 與 `W(r)` 在高價端的樣本，
  而那正是 D062 現在缺的東西。

**二、M1-c 要換它問的問題**

不要再問「候選（同價）vs 場上」，要問
**「保住場上這張 vs 換到高原上緣（D061 的那個上緣）」**。
**同一張表、同一個寫入點，只換被比較的那個候選。**

### 🔴 2026-09-05 追記：第二項（3-2）不做，而理由推翻的是它自己

實作前查證，發現它換了問題也拿不到新資訊：

**沒被掛出去的那個價位，它的結果是不可觀測的。** 換一個候選進去，
落下來的仍然只是**模型對那個價位的意見**——而模型的意見我們已經有了
（那正是它沒被選中的原因）。**把模型的意見存進 DB，不會讓它變成證據。**

那「事後用 K 線檢查那個價位會不會成交」呢？**那件事已經有人在做了**
——`fill_simulation.simulate_fill()`＋`backtest_report --simulate`，
而且它的成交規則拿 12 筆真實成交驗過（總量比 0.93×，D049）。
**D061 與 D066 的那兩張表就是這樣算出來的。** 把同一個計算搬到即時寫入點，
只是提早算，不會多出任何資訊。

**M1-c 真正無可取代的只有四個欄位**——`live_offer_id`／`live_idle_hours`／
`live_queue_ahead`／`candidate_queue_ahead`。它們來自**訂單簿與即時掛單狀態**，
K 線重播算不回來。**而那四個欄位在 186 列 `hold_matched` 裡是有值的**，
它們沒有壞掉，壞掉的是那個「保住 vs 改掛」的二選一在候選＝場上時無話可說。

> **所以 3-2 併進 3-1：讓 M1-c 有話可說的唯一方法，是真的去掛一次不同的價位。**
> **這不是「換個問法」能解決的，是資料根本不存在。**

### 🔴 這一項改變了甲-2 的優先序

`W` 旋鈕化（甲-2）原本排在很前面。但實測顯示：

- 在**目前選中的價位**上，`W` 的誤差已經無關緊要（估 1.66h、實際 0.13～0.67h）；
- 在**候選的高價位**上，`W` 仍然是決定性的（10.22% 估 `W=9.51h`）——
  **而那一半正好一個校準樣本都沒有。**

**所以甲-2 的價值被探索卡住**：現在把 `mean` 換成 `median`，換到的是
「在低價端更準一點」，而低價端已經不重要了。**甲-2 要排在探索之後。**

### 誠實界線

- **「每 20 次換手探索 1 次」的 20 是拍的。** 它必須是設定、必須能關掉，
  而且**第一版不要用它去改任何決策**——先讓它產生樣本。
- **探索期間的錢是真的**。0.8% 的機會成本是算出來的，不是量出來的；
  真實的探索成本要等它跑過才知道。

---

## D064 — 驗收工具靜默失效了六天，因為「當初用什麼設定」沒有被存下來

**日期**：2026-09-05
**狀態**：🟢 **已修**（`pricing_knobs_json` ＋ 每列用自己的設定重播）；🔴 **而它是 D048 的同一個病第二次現身**

### 現象：40 列裡 34 列「不一致」，而那 34 列一列都沒錯

`--verify` 是 M2 唯一那條跑得起來的驗收標準（D048 換掉原本那條跑不動的）。
它拿 `pricing_decisions` 裡正式環境真的做過的決策，逐點對照重播的結果。
**2026-08-30 第一次跑：25 列全數一致。**

同一天 22:57，D056 把 `assumed_hold_hours` 從 48 改成 12。**從那一刻起這份驗收就壞了**
——重播用**現在的** `config.yaml` 去重跑**當初用別的設定**做的決策。
2026-09-05 查出來時是 **40 列裡 34 列不一致**。

🔴 **而它報的是「不一致」，跟「工具壞了」長得一模一樣。**
「不一致」本來就是這份報告預期會出現的輸出（`market_candles` 少存幾根 K 也會不一致），
所以**沒有任何一個訊號告訴人「這次的不一致是另一回事」**。六天沒有人發現。

### 這是 D048 的同一個病

D048 記的是「驗收標準比它依賴的那張表早寫了六天，而十三天內沒有人回頭確認它可不可跑」。
這一次是**寫得出來、也跑得動，但它悄悄開始量另一件事**——比 D048 更難發現，
因為它從頭到尾都有輸出。
...
D043 存 `hold_hours_assumed` 的時候已經寫過理由：
「**當初的預估是不存就永遠消失的那一半**」。
**那句話當時只被套用在一個旋鈕上，而它對每一個會改變答案的旋鈕都成立。**

- **新欄位 `pricing_decisions.pricing_knobs_json`**，由策略自己交代
  （`ExpectedValueStrategy.PRICING_KNOBS`／`pricing_knobs()`）。
  **只有策略知道自己讀了什麼**，而讓別人去猜正是這次失效的成因。
- **存 JSON 而不是一欄一個旋鈕**：下一次加旋鈕不必再動 schema，
  也就不會再出現「加了旋鈕、驗收工具隔天就開始說謊」。
- **`--verify` 每一列都用它自己記下的那一組重播**（`backtest.pricing_knobs()`，
  是 `assumed_hold_hours()` 的推廣）。
- 🔴 **只認策略自己宣告的白名單**：不在名單上的鍵一律忽略並**回報**。
  那一列 JSON 是好幾週前寫的，**不可以讓它往策略身上掛任何屬性**。

**結果：40 列 → 40 列全數一致。**

### 既有資料庫要補欄位，而這個專案沒有 migration 機制

`CREATE TABLE IF NOT EXISTS` 對**已經存在**的表什麼都不做，所以新欄位只會出現在
全新的資料庫裡——**正式環境那個從 2026-08-01 跑到現在的檔案永遠等不到它**。

新增 `Repository._ensure_columns()`：**只加欄位，不改型別、不刪、不搬資料**。
`ALTER TABLE ... ADD COLUMN` 在 SQLite 是常數時間、不重寫既有列。
用 `PRAGMA table_info` 判斷而不是 try/except——後者會把「欄位已存在」與
「表根本不存在」吞成同一件事，**而那兩者的意思完全相反**。

**拿正式 DB 的副本實測過**：197 列不變、只多一個欄位、其他四張表列數完好。

### 比欄位更早的那些列怎麼還原

`pricing_knobs_json` 之前的每一列都是 NULL。**NULL 是「當時沒有這個東西」，
不是「當時是預設值」**——所以資料層不替讀的人決定，由報告層處理。

報告層的做法（`_legacy_knobs()`）：**從策略的類別預設出發，
再用那一列記下的 `hold_hours_assumed`／`window_hours` 覆蓋**。

🔴 **依據是這個專案的一條慣例：新旋鈕的類別預設值＝那個旋鈕不存在時的行為。**
（`assumed_hold_hours` 退回 `offer_period * 24`、`ev_plateau_tolerance_pct` 預設 0。）
所以「用類別預設重播一列比旋鈕更早的紀錄」正好還原了它當時走的那條路。

⚠ **這是一句關於程式碼歷史的陳述，不是那一列自己說的**——**而報告會把這句話印出來**，
不會讓「還原得動」看起來跟「那一列自己記得」一樣。
哪天有人加了一個「預設值就改變行為」的旋鈕，這裡就會開始說謊，
**唯一擋得住的是那時候要記得填 `pricing_knobs_json`**（欄位已經在了）。

### 誠實界線

- **`--verify` 仍然只證明「工具沒有偏離正式環境」，不證明策略是對的**
  ——重播與正式環境跑同一條路徑，兩邊會一起錯（D048 就寫過這句）。
  這次修的是**它連自己那件事都不再做得到**。
- **`PRICING_KNOBS` 這份清單靠人維護**。測試擋得住「名字打錯」（每一個都必須
  真的是策略的屬性），**擋不住「忘了加」**。
- **目前 40 列全部是舊列**（欄位今天才加），所以「40/40 一致」有一部分靠的是
  上面那條慣例，不是那些列自己記得。**第一列真正自帶旋鈕的紀錄要等下一次定價。**

---

## D065 — 實得年化的分母不是「本金」，是「已部署資金」

**日期**：2026-09-05
**狀態**：🟢 **已實作**；🔴 **它推翻了 `_maybe_sync_earnings()` 自己寫的一句話**

### 問題：唯一那條不靠推論的績效數字，要靠人手打一個會漂的數字

`earnings_daily.principal_avg` 從 2026-09-01 起**全是 NULL**，因為
`bot_engine.py` 的帳本同步刻意傳 `principal_avg=None`。理由寫在那支的註解裡：
...
於是算實得年化只能這樣跑：
    ...
    python3 scripts/sync_earnings.py --since 2026-08-15 --principal 345.29

**而那個 345.29 是人從別的地方抄來的，還會漂**（20 天從 344.31 到 345.29）。
**這個專案唯一一條「交易所自己說的錢」，卡在一個手打的參數上。**

### 那句話對「本金」是對的，但實得年化的分母不是本金

分母應該是**已部署資金**：**錢包餘額 ＋ 場上掛單 ＋ 借出中的部位**。

| 那句話的顧慮 | 為什麼它反而是對的 |
|---|---|
| 「餘額含已賺到的利息」 | **複利滾進去的錢一樣在工作。** 不算進分母，年化會被高估 |
| 「含還掛在場上沒借出去的錢」 | 🔴 **空窗正是我們要懲罰的東西**（D034 那條算式的整個重點）。把閒置的錢從分母拿掉，等於把「借不出去」變成免費的 |

**而三項都是這一輪向交易所問到的觀測值，不是推論。**
`get_available_balance()`／`get_active_offers()`／`get_active_positions()`
——**`run_once()` 本來就已經全部問過了**，只是問完就丟。

### 觀測點放在六條提早 return 的路之前

`run_once()` 有六條會提早 `raise SkipCycleError` 的路，而**正式環境幾乎每一輪
都走其中一條**（餘額不足 → 略過）。放在任何一條之後，這個數字永遠不會被記下來
——**D052 踩過同一個坑**（帳本同步放進 `run_once()` 就永遠不會執行）。

### 🔴 只有「今天」那一列填得了

帳本一次會給出二十幾天的利息，而「已部署資金」是**這一刻的觀測**。
**把今天的數字寫進上個月那一列，就從觀測變成了推論。**
其餘日子維持 NULL——**沒有分母，好過一個編出來的分母。**

⚠ **欄位名叫 `principal_avg`，而它裝的不是平均**：巡檢 600 秒一輪，
寫進去的是**當天最後一次觀測到的值**。實測這個數字一天約漂 0.05%，
差異遠在帳本本身的雜訊之下——**但這件事要知道，所以寫在這裡。**

### 第二件事：帳本沒有任何東西在看它

2026-09-05 靠**人眼**發現 `earnings_daily` 在 08-16／08-20／08-21／08-31 缺列
——**而缺列與「那天真的沒賺」長得一模一樣**（D064 的同一個形狀：
失效時報的東西，跟它平常就會報的東西無法區分）。

`sync_earnings` 因此多一段**毛／淨對帳**：把 `funding_positions` 推算成
「照合約應該產生多少毛利息」，與帳本淨利息並排。

    推算毛利息 1.3900 USD ／ 帳本淨利息 1.1164 USD → 淨毛比 80.3%（抽成 15% 對應 85%）

**抽成 15% 是量出來的，不是查來的**（`FUNDING_FEE_PCT`）。
🔴 **它只用在對帳，不參與任何定價決策**——利息上的常數乘數不改變
`r × P ÷ (W + P)` 的極大點。

**偏離參考線時，兩種相反的可能都要講**：有幾天沒入帳（帳本缺列），
或推算把某些部位算多了。**這兩個數字分不出是哪一種**，所以不挑一個講。

### 誠實界線

- **推算值偏高，三個來源方向相同**：沒算複利、`closed_at` 被巡檢延遲高估、
  利息是**結算日**入帳不是權責日。**只有多日合計有意義，日對日必定對不齊。**
- **80.3% 對 85% 的差距**（4.7 個百分點）落在上面那些邊界效應裡，
  **但沒有人證明過那件事**——容差 ±8pp 是拍的，它擋的是「少了整整一天」等級的缺口。
- **這一項不改善任何績效**，它讓那條唯一的錨不必再靠手打的參數。

---

## D066 — `median` 是更準的預測值，卻是更差的決策規則

**日期**：2026-09-05
**狀態**：🟢 **旋鈕已上線，預設值不變**；🔴 **它把 D045 第三則追記的方向反過來了**
...
到 2026-09-05 累積到 12 個樣本，方向不變：
`mean` 中位高估 **9.0 倍**、`median` 中位高估 **3.9 倍**。

### 而跨 114 個起跑點跑下來，`median` 輸 1.0 個百分點

`ev_wait_statistic`（mean｜median｜p75）× `ev_censoring`
（window_tail｜drop｜full_window）六種組合，裁判用 `rate_dependent`（D062）：

| `W` 統計量 | 設限記帳 | 今天選價 | 平均實得年化 | 對現行 | 勝率 |
|---|---|---|---|---|---|
| **mean** | **window_tail** | 8.93% | **8.158%** | — | — |
| mean | drop | 8.93% | 6.621% | **−1.537pp** | 5/114 |
| mean | full_window | 8.93% | 8.253% | +0.095pp | 53/114 |
| median | window_tail | 9.12% | 7.126% | **−1.032pp** | 12/114 |
| median | drop | 9.12% | 6.480% | **−1.679pp** | 10/114 |
| median | full_window | 9.12% | 7.136% | **−1.022pp** | 13/114 |

**`median` 在每一種設限記帳下都輸 1pp 以上。**

### 為什麼「比較準」不等於「比較好」

> **長期年化是一個更新報酬過程：`E[利息] ÷ E[週期]`。**
> **那兩個期望值要用平均，不是中位數。**

用中位數會系統性地低估等待，於是高價位看起來比實際好 → 選到 9.12% →
真實世界等更久 → 實得年化更低。**理論與模擬指向同一件事。**

而 D045 量的是**點預測的準度**，**在一個被選擇過的子樣本上**（12 個樣本
**全部是成交的那半**——D047 追記早就說過沒成交那半的偏誤方向會變）。

> **一個更會預測「成交那半」的統計量，不是一個更好的「期望值比值」的輸入。**
> **D045 追記問對了「哪一個比較準」，但那不是算式需要的那個問題。**

### 🔴 意外發現：現行的設限記帳比「整批丟掉」還低估

寫測試時才發現的。`window_tail` 把設限起點記成「等到窗尾」，
而**設限起點必定落在窗的後段**——於是它們拿到的是 1 到 N 根這種**很短**的等待。

實測同一份資料：`drop` **46.41h**、`window_tail` **40.57h**。
**丟掉最長的那些等待，算出來反而比現行記帳法還長。**

**這是 D047「乾旱會讓等待估計自己變好看」的另一個面貌**：
**越晚的起點、越沒等到，記下來的等待反而越短。**

（唯一必然成立的關係是 `full_window ≥ window_tail`，因為 `total ≥ total − start`。）

### 為什麼 `full_window` 也不改預設

它是六種裡唯一平均為正的（+0.095pp），但**沒過穩健性標準**：

| 情境 | `rate_dep` | `empirical` | `fixed48` |
|---|---|---|---|
| 全段 | +0.095pp（53/114） | +0.101pp（61/114） | **−0.009pp**（36/114） |
| 前半 | +0.153pp（37/57） | +0.082pp（34/57） | +0.020pp（25/57） |
| 後半 | +0.037pp（16/57） | +0.121pp（27/57） | **−0.039pp**（11/57） |

**符號在持有模型之間會翻**，而且 9 格裡有 4 格勝率低於五成。
**與 D061 的 1.0% 容差同一個死法**，而 +0.095pp 本身也在雜訊裡
（兩個裁判的分歧就有 0.065pp，D062）。

### 決定

- **六個組合都做成旋鈕，預設值＝現行行為（`mean` ＋ `window_tail`），行為零變化。**
- **兩個旋鈕都進 `PRICING_KNOBS`**（D064 的規矩）。
- 🔴 **認不得的統計量直接拋，不默默退回 `mean`**：設定檔打錯一個字，
  後果會是「換了旋鈕但行為沒變」——**而那看起來跟「換了沒有用」一模一樣**（D064 的形狀）。
- **KM／RMST 沒做**。它換掉的是記帳方式，不是「窗外看不到」這件事，
  而且有兩條它自己解決不了的界線（起點重疊 → 樣本不獨立；RMST 仍被窗長夾住）。
  **在六個可比較的記帳法都指向「不要改」的情況下，先不加第七個。**

### 誠實界線

- **這一整張表是模擬出來的**，而模擬的成交規則本身是近似（D049 驗過總量比 0.93×）。
- **12 個校準樣本全部落在 8.76～9.50%**（D063）。**`median` 在高價端準不準，沒有樣本。**
- 🔴 **丙-1 仍然沒有被回答**：`W` 與 `P` 要一起調（D059），而這裡只掃了 `W`。
  但這張表把丙-1 的空間縮小了——**`median` 那三列可以先劃掉。**

---

## D067 — 等更多單子不會改變答案，而探索的實驗設計被檢定力否決了

**日期**：2026-09-05
**狀態**：🟢 **這是一個「停下來」的決定**，而它有三條各自獨立的依據

### 問題：做完六個順位之後，該繼續改，還是該等更多資料

09-05 做完的每一項，答案都是「不要改」（D062／D066／D061 的 1.0%／`full_window`）。
於是出現一個實際的問題：**是我們資料不夠，還是這件事真的到頂了？**
...
21 天累積 17 段，**每月約 28 段**。而 D056 之後產生的每一段，利率都是
8.76／8.93／9.00% ——**全部在便宜組**。

> **模型只在它自己選中的價位上取樣（D063）**，所以再跑一年，
> **昂貴組仍然只有那 5 段。** 等待累積的是同一個問題的重複答案。

### 依據二：就算把持有時間換成最有利的假設，最佳 `P` 還是 12

這一條是決定性的。拿三種對「現在這個價位實際借多久」的假設重跑掃描：

| 實際持有假設 | 最佳的策略假設 `P` | 平均實得年化 |
|---|---|---|
| 全部 17 段（現行） | **12h** | 8.124% |
| **只用 D056 之後那 3 段**（48.33／48.19／1.43） | **12h** | 8.367% |
| **假設每一筆都借滿 48h**（最有利的極端） | **12h** | 8.397% |

🔴 **連「每一筆都借滿 48 小時」都不能讓最佳假設離開 12。**

**因為 `assumed_hold_hours` 不是一個對持有時間的估計，它是一個「多討厭等待」的旋鈕。**
名字取錯了，而那個錯誤名字讓 D055／D056／D062 三次都把它當成
「要不要把估計改對」的問題。**它從來不是。**

> **所以「等更多持有時間資料」這件事，在結構上就不可能改變結論。**

### 依據三：沒有不花錢的替代路徑

查過了：

- **訂單簿與成交帶沒有多餘資訊**。窗內成交量中位數是 **1.7 億 USD**，
  而我們的部位 345 USD 佔 **0.0002%**。**在這個規模下排隊位置完全不相干**
  ——`estimate_wait()` 那句「high ≥ r 就會成交」是對的。
- **K 線的 `high` 已經包含了公開成交帶的資訊**，那正是目前 `W` 的來源。
- 「事後用 K 線檢查沒掛過的價位」`--simulate` 已經在做（D063 追記）。

### 🔴 而原本規劃的探索方式，檢定力差了 20 倍

D063 寫的是「每 20 次換手刻意掛一次高價，年化影響 < 0.1pp」。**成本估對了，
檢定力沒算過。** 用「借滿 48h 的比例」做兩比例檢定
（便宜組 0.42、昂貴組 0.20，α=.05、power=.8）：

**每組需要約 70 段。**

| 探索頻率 | 每月昂貴樣本 | 湊到 70 段要 |
|---|---|---|
| 每 20 次 1 次（**原規劃**） | 1.4 段 | **50 個月** |
| 每 5 次 1 次 | 5.6 段 | 12.5 個月 |
| **每 1 次 1 次**（等於換策略） | 28 段 | **2.5 個月** |

> **「灑一點探索」在這個循環速度下不是實驗，是四捨五入的誤差。**
> **要有檢定力，就得整段時間換過去——那不叫探索，叫換策略。**

### 決定：停在這裡，並寫下重啟的條件

**不做 3-1。** 理由不是它有風險（錢的風險是每年 0.3 美元），
是**它在目前的規模與循環速度下答不出問題**。

**要重新啟動這條線，要有下面任何一個：**

1. **本金顯著變大**（10 倍量級）。⚠ **注意這不會提高檢定力**
   ——雜訊底線是百分比，跟本金無關。它改變的是「值不值得花這些時間」。
2. **循環變快**（市場變熱，一天多段），那 70 段就不用等半年。
3. **帳本實得年化掉到 7% 以下**，代表現在這個作業點不再站得住。
4. **想改的是方法而不是收益**——那就走「整段時間 A／B」，
   用帳本實得年化當裁判（連續量測，比逐段的比例檢定有力得多），
   **一個月一臂**，而不是灑探索。

### 誠實界線

- **這是「在目前條件下答不出來」，不是「高價比較差」。** 那個問題仍然開著。
- **依據二最強，但它建立在模擬器上**，而模擬器的成交規則是近似（總量比 0.93×）。
- **70 段是用目前那 17 段估出來的效果量**算的；真實效果若更大，需要的樣本更少。

### 🟡 2026-09-06 追記：條件 2（循環變快）可能正在發生，但還不能算數

D061 上線後那半天，**12 小時內換手 3 次**，持有時間是 **1.33h 與 1.18h**
——而在此之前 18 段的中位持有是 **16.29h**。

如果這個速度是常態，每月的段數會從 28 段跳到數百段，**條件 2 就成立了**，
70 段不用等半年。

🔴 **但現在還不能算數，兩個理由**：

1. **n=2。** 前面才剛記取過「借滿 48h」出現兩次就差點被當成常態的教訓（D060）。
2. **短持有本身不是好消息。** 每段賺得少、換手成本（空窗）攤在更多次上。
   **它讓實驗變可行，但同時也可能讓實得年化變差**——那兩件事要分開看。

**判準寫在這裡，免得下次靠印象決定**：
連續 **7 天**的中位持有低於 **4 小時**，且期間段數 ≥ **40 段**，才算條件 2 成立。
在那之前這只是一則觀察。

---

## D068 — 一個中文檔名讓「只動了文件就別部署」的閘門失效

**日期**：2026-09-06
**狀態**：🟢 **已修**；🔴 **同一個症狀的第三次，但這次的病因完全不同**

### 發生什麼

2026-09-06 15:01 推了一次**只動 `.project-docs/` 的文件變更**，
而 **CI 照常部署、重啟了容器**。

**沒有造成損害**（資金當時鎖在部位 `465247022` 裡，場上沒有掛單，重啟後 0 ERROR），
**但那是運氣，不是設計**——若當時場上躺著一張單，那次重啟就是一次不必要的風險。

### 病因：`git diff --name-only` 會把非 ASCII 路徑加引號

那次推送新增了一個檔案：`archive/TASKS-六個順位-2026-09.md`。
`git diff --name-only` 的預設行為（`core.quotePath=true`）是把非 ASCII 路徑
**加引號並轉成八進位跳脫**：
...
而閘門的白名單是 `^(\.project-docs/|README\.md$|…)`。
**那個開頭的 `"` 讓最後一行對不上**，於是判成「有文件以外的變更，照常部署」。

### 🔴 這是同一個症狀的第三次，而前兩次的修法在這裡沒有用

工作流程裡本來就寫著：

> **白名單是踩出來的，不是想出來的**（TASKS.md C2）：原本只認 `^\.project-docs/`，
> 於是 2026-08-19 與 08-23 兩次「只改文件」的推送都因為同時動到根目錄的
> README.md 而觸發部署……**所以要修的是涵蓋範圍，不是規則本身。**

**那句話對前兩次是對的，對這一次是錯的。**
這次的路徑**本來就在涵蓋範圍裡**——它是被**引號**擋在外面的。

> **同一個症狀（純文件推送卻部署了）有兩個完全不同的病因，
> 而第一個病因的修法會讓人以為第二次也是同一件事。**

### 修法

1. **`git -c core.quotePath=false diff --name-only`** ——真正的修法，**白名單一個字都沒改**。
2. **檔名改回 ASCII**（`TASKS-priorities-2026-09.md`），並把
   **「內容用中文，檔名用 ASCII」** 這條慣例寫進 `archive/README.md`。

⚠ **第 2 項不是必要的**（第 1 項已經修好了），做它的理由是：
**檔名會被 shell、正規式、grep 一路傳下去，而中文檔名在每一關都是一個額外的失敗機會。**
這次是 CI，下次可能是別的地方。

### 🔴 誰造成的，以及它為什麼躲過檢查

**是這一輪自己造成的**：09-06 歸檔 TASKS 的六個順位時，順手用了中文檔名
——而專案裡其他每一個檔案都是 ASCII。

**而它躲過檢查的方式很典型**：閘門是 **fail-open** 的（算不出來就照常部署），
**所以它「失效」的樣子就是「多部署一次」，跟正常行為長得一模一樣。**
👉 **與 D064 完全同一個形狀**：失效時報的東西，跟它平常就會報的東西無法區分。

fail-open 這個選擇本身仍然是對的（少部署一次的代價大得多），
**但它的代價是這類失效不會自己現身**——只能靠有人剛好去看容器的啟動時間。

### 誠實界線

- **這次是「剛好去看了」才發現的**，沒有任何自動偵測。
- **沒有為它加測試**：閘門是 shell 腳本、跑在 GitHub Actions 裡，
  現有的 pytest 套件碰不到它。**這一條是已知的空缺，不是疏漏。**

---

## D069 — 放寬合約天期到 30 天，以及一個被用了一週的錯數字

**日期**：2026-09-12
**狀態**：🟢 已上線（`offer_period` 2 → 30、`assumed_hold_hours` 12 → 48）
**相關**：D030（天期分析與「驗證階段」的但書）／D032（不要挑最大值）／
D045（等待估計的校準）／D062（`P` 這個旋鈕會挑價）／D065（實得年化的分母）／
D067（停下來的決定與重啟條件）

### 這不是新發現，是一張到期的支票

D030（2026-08-16）自己就寫過這句話：

> **寫死 2 天或寫死 7 天都是錯的架構**，正解是每輪比較各天期的實質年化再選。

而當時仍然掛 2 天，理由**與定價無關**：

> 驗證階段不要把資金鎖住。「成交 → 記錄 → 通知」整條鏈一次都沒跑通過，
> 先確認它會動，才有資格談天期最佳化。

**那個條件在 2026-09-12 已經滿足**：真金成交 28 筆、帳本對得上帳（淨毛比 82.9%）、
量測層完整（936 條測試、三張落地表、四份報告）。所以這一則不是推翻 D030，
**是執行 D030 自己寫下的下一步**。

### 🔴 一、先修正一個被用了一週的數字

STATUS 從 2026-09-06 起寫著「實得年化（帳本）**7.75%**」。**那是錯的。**

帳本的利息**按每筆單子的 24 小時結算點入帳，不是按日曆日**。所以會出現「空白日」
——2026-09-09 整天沒有入帳列，而 09-10 那筆 0.0784 USD 就是前一筆 26.26h 的全額。
**空白日必須留在分母裡**，而 7.75% 是拿 09-01～09-04 四天算的，那四天**剛好沒有空白日**。

誠實的數字：

| 期間 | 實得年化 |
|---|---|
| 最近 7 天（09-06～09-12） | **6.83%** |
| 最近 14 天 | **6.46%** |
| 上線至今（27 天） | **6.14%** |

最近 14 個 7 日滾動點裡有 **11 個低於 7%**。
**也就是 D067 的重啟條件 3（帳本實得年化掉破 7%）其實已經成立兩週了。**

**根因是工具沒接上，不是算錯**。`scripts/sync_earnings.py` 的 `elapsed` 邏輯是對的
（它從 `--since` 起算、還會警告空白日），但 **`format_summary()` 只在給了
`--principal` 時才印實得年化**，而它從來沒讀回 D065 落地的 `earnings_daily.principal_avg`。
於是照 STATUS 寫的維運指令跑，**根本印不出實得年化**，只能手算——然後算錯。

👉 **D065 的追記「本金不必再手打了」對機器人成立，對這支報告不成立。**
這是 D048／D064 那個家族的第三次現身：**報告端與寫入端各自為政**。

### 🔴 二、9% 的目標從一開始就差一個 0.85

使用者設定的目標是「9% 年報酬率」。**Bitfinex 抽 15% 的放貸利息**，所以：

| | 數字 |
|---|---|
| 上線至今**加權名目**利率 | **8.81%** ← 幾乎已經達標 |
| 上線至今**入袋**年化 | **6.14%** |
| 要淨拿 9% 所需的名目利率 | **10.59%**（而且要 100% 利用率） |

**掛 9% 然後期待拿到 9%，數學上做不到。** 這不是策略跑得差，
**是目標設定時少乘一個 0.85**。這一條必須寫下來，因為它會反覆被重新問。

### 三、績效是被利用率決定的，不是被價格

三週的分解（金額加權）：

| 期間 | 利用率 | 加權名目 | 理論淨＝利用率×名目×0.85 | 帳本實得 |
|---|---|---|---|---|
| 08-22～08-28 | 99.2% | 9.22% | 7.77% | 7.96% |
| 08-29～09-04 | 81.6% | 8.98% | 6.23% | 6.24% |
| 09-05～09-11 | 90.2% | 8.96% | 6.87% | 6.83% |
| 上線至今 | 84.8% | 8.81% | 6.35% | 6.14% |

**理論值與帳本三週都對得上（差 ≤0.2pp）。** 名目價格三週幾乎沒動，
**變的全是利用率**。本週的缺口幾乎全來自 09-08 一段 **16.20 小時的空手**
（當時市場成交最高只有 7.30%，我們掛 8.93% 搆不到；策略重估 96 次都選同一個價）。

### 四、為什麼是天期，而不是別的

**先否決兩個選項**：

- **「躺太久就降價」——回測否決。** `--repost` 五種政策：不重掛 7.35%／現況 7.27%／
  躺 12.6h 才往下 7.27%／躺 18.9h 7.28%。差距 ≤0.08pp，在雜訊底線 0.065pp 附近，
  **而且最好的是「不重掛」**。這個否決有份量，因為成交模擬器**重現了 09-08**
  （實際 16.20h vs 模擬 15.85h，0.98×；整體總量比 0.95×）。
- **D067 的重啟條件 2（循環變快）——明確否決。** 持有中位 16.29h、24 段。
  09-06 那個 n=2 的訊號沒有延續（09-08 借 26h、09-10 借滿 48h）。**這一項結案。**

**然後修正一個我自己先講錯的東西**：天期溢價**沒有**「2～4pp」那麼大。
那個數字來自 `trade_period_rates_json`，它是成交的**加權中位數**——
但我們根本不掛在中位數，**我們掛在掃單的高點**，而各天期的 K 線 `high` 中位數
幾乎一樣（天期 2 是 9.12%、天期 30 是 9.24%）。

**真正的槓桿是：長天期把「等待」攤平了，於是策略敢挑更高的價。**

### 五、旋鈕怎麼選：挑高原，不挑最大值（照 D032）

天期 30 下掃描 `assumed_hold_hours`（2026-09-12，240 根 1h K）：

| 旋鈕 `P` | 選出的價 | 窗內命中 | 預估等待 |
|---|---|---|---|
| 12 | 8.994% | 119 | 1.2h |
| **24／36／48** | **10.500%** | **46** | **4.7h** |
| 72 | 10.731% | 26 | 6.4h |
| 96／168／336 | 11.258% | 11～12 | 11.7h |
| 720 | 11.268% | 🔴 **7** | 14.1h |

**24／36／48 選出同一個價、命中 46 次——那是高原。**
最底下的 11.268% 只被掃到 7 次，正是 `ev_min_hits`（門檻 5）當初要擋的尾端。
**挑它就是 `target_queue_usd` 的死法**（D032）。

⚠ **兩個旋鈕必須一起改**：只改 `offer_period` 而把 `assumed_hold_hours` 留在 12，
會選出 **8.994%——比改之前的 9.125% 還低**，等於鎖了 30 天卻沒收它的好處。
這一條已釘在 `tests/integration/test_dry_run_cycle.py`。

⚠ **這個 48 不是「預測借款人會借 48 小時」。** 它跟 D060 之後的 12 一樣，
仍然是 D067 說的那個「**多討厭等待**」的旋鈕。換掉它的仍然是 `HoldModel`。

### 🔴 六、誠實界線：這一步賭的是什麼

**賭的是「合約天期拉長，借款人會借比較久」，而那件事現在量不到。**

- 24 筆已結束的部位裡，**9 筆是被 48h 上限截斷的**。
  Kaplan-Meier 在最後一個觀測點 48.6h 時存活率仍有 **37.5%**
  ——`hold_report` 印的平均 23.24h **是下界，不是估計**。48h 之後零觀測。
- **放寬天期本身就會把這個盲區打開**，這是它除了收益以外的第二個理由。

🔴 **而且等待估計在這個天期下會被低估。** `estimate_wait()` 是數「幾根 K」
再乘 `candle_hours=1.0`，但**天期 30 的 K 線有 23% 的小時是缺的**
（168 根實際涵蓋 210 小時）。所以：
...
案子成立靠的是截斷論證（holds 會變長），不是靠這張表的絕對值。

⚠ **這個缺根偏誤是既有缺陷，不是這次引入的**，而且它與 D045 量到的偏誤**方向相反**
（D045：`W` 被高估 2.56 倍）。兩者部分互相抵消，**淨效果不能手算宣告**。
修它屬於 `W` 的校準（M2 的題目），**刻意不併進這次改動**。

### 七、停損與軟判準

門檻是**拿現有設定的歷史校準過的**，不是拍的——會在「什麼都沒改」時誤觸發的門檻
不是停損，是雜訊：

- ❌ **「3 日空手比例 > 20%」不能用**：空手比例是**雙峰**的（中位 0.7%、p90 79%），
  這個門檻在現有設定下會誤觸發**三次**（08-19、08-30、09-08）。
- ✅ **硬停損：7 日滾動實得年化跌破 6.0% → 把 `offer_period` 改回 2。**
  現有設定成熟後的區間是 6.10%～8.02%，**6.0% 在歷史地板之下，不會誤觸發**。
- 🟡 **軟判準：兩週後（2026-09-26）若 7 日滾動 < 7.5% → 回頭。**
  不是壞掉，是**不值得為了這點差距鎖 30 天**。

**怎麼看**：`python3 scripts/sync_earnings.py --since 2026-09-12`。
⚠ 但那支目前不會自己印實得年化（見第一節），**修它是下一件事**。

### 八、風險裡有一項比想像小

**鎖倉風險不大**：成交價 10.500% 本來就高於現在的 8.93%，被鎖 30 天不是壞事；
借款人提前還款的話行為跟今天一樣。**真正的壞情況是掛著不成交**，
而那個一週內就會反映在帳本上，正是上面那條硬停損在看的東西。

**上線時機是挑過的**：改動當下錢在部位裡（`465606454`，09-14 凌晨到期），
**場上沒有掛單**，所以這次改設定沒有動到任何掛著的單。

### 🔴 追記（2026-09-12 晚間）——修那個報告時又抓到兩個鄰居

第一節說「修 `sync_earnings.py` 是下一件事」。修的時候試跑不同的 `--since`，
**又抓到兩個同一家族的缺陷**，兩個都會直接害到上面那條停損：

**① 曆日數少算一天。** `elapsed = (last - start).days` 是相減，
而曆日數是**含頭含尾**。更糟的是**同一個函式裡的空白日判斷用的是 `elapsed + 1`**
——同一份報告裡兩種天數。09-05 → 09-12 其實是 8 天，算成 7 天讓年化
從 6.87% 變成 **7.86%（高估 14%）**。

👉 **與第一節那個錯數字是同一個病的兩個版本**：一個用列數當分母，一個少算一天。
**「分母」這件事在這支報告裡被弄錯過兩次。**

**② 毛／淨對帳的兩半涵蓋不同期間。** `expected_gross_interest()` **根本沒有窗參數**
——它把所有部位無條件加總，而帳本那半已經被 `--since` 篩過了。
於是 `--since 2026-09-05` 印出 **淨毛比 27.1%**，觸發一句
「🔴 比參考線低 57.9 個百分點」的**假警報**（正確值 85.0%）。

🔴 **它原本只在 `--since` 等於專案起點時才碰巧正確——而那正是唯一被跑過的用法。**
這是 D048／D064 家族的又一次：**工具在它唯一被用過的那條路上是對的，
而我們以為那代表它是對的。**

**修完之後的驗收**（三個窗，分解值 vs 帳本實得）：
...
**分解值與帳本差 0.00～0.06pp，淨毛比全部落在 85% 參考線上。**

**順帶把 TASKS 甲-2 一起做了**（`format_utilization()`）：報告現在自己印
「利用率 × 名目 × 0.85 = X%，帳本實得 Y%」並比較兩者。
**它存在的理由是停損觸發時的第一個問題**——「是借不出去還是賣太便宜」
——在這之前得自己從 `funding_positions` 手算。

### 這支報告以前一條測試都沒有

🔴 **`scripts/sync_earnings.py` 的排版從來沒被驗過**，而那就是上面三個缺陷
能一起存活的原因。新增 `tests/unit/test_sync_earnings.py`，
**釘的不是「算得對不對」，是「該印的有沒有印出來」**。測試 936 → **960**。

其中一條特別值得指出：`test_兩邊對不上就要說對不上` 釘的是
**對帳段真的在比，而不是無條件印一個 ✅**
——一個永遠說「對得上」的對帳段跟沒有對帳段一樣，**那正是 D064／D068 的 fail-open 長相**。

---

## D070 — 長天期第一週的成績，以及一個我用錯的判準

**日期**：2026-09-19
**狀態**：🟢 已定案（**維持** `offer_period: 30`、`assumed_hold_hours: 48`，一行程式不改，跑到 2026-10-14）
**相關**：D030（每輪比較各天期才是正解）／D032（挑高原不挑最大值）／
D040（`P` 的校準）／D045（`W` 的校準）／D063（樣本全落在模型自己選的窄帶）／
D069（這一步的上一步）

### 🔴 一、先記一個我自己用錯的判準——這一條比任何數字都重要

D069 寫下的硬停損是「**7 日滾動實得年化跌破 6.0% 就改回天期 2**」，
理由是「現有設定成熟後的區間是 6.10%～8.02%，**6.0% 在歷史地板之下，不會誤觸發**」。

2026-09-19 第一次拿它去判長天期，得到 5.13%，於是我建議執行停損。
**使用者反駁：短天期測了一個月才換策略，長天期只測一週就判死刑，不公平。**

**去把短天期那一個月的每一個 7 日窗攤開來看，使用者是對的，而且差得很遠**：

| 7 日窗 | 實得年化 |
|---|---|
| **08-17～08-23（短天期第一週）** | 🔴 **4.03%** |
| 08-18～08-24 | 4.97% |
| 08-19～08-25 | 5.41% |
| 08-20～08-26 | 5.64% |
| 08-21～08-27 | 6.85% |
...
| …（其餘 15 個窗落在 6.09%～7.75%） | |
| 09-07～09-13 | 6.75% |
| **整個月（08-17～09-13，28 天）** | **6.20%** |

👉 **短天期自己的第一週是 4.03%，比長天期的第一週（5.14%）還差。**
**那條 6.0% 的停損線如果在 08-23 就存在，短天期活不過第一週。**

🔴 **根因**：D069 算歷史區間時寫的是「**現有設定成熟後**的區間是 6.10%～8.02%」
——「成熟後」三個字把前六個窗（4.03%～5.64%）排除在外了，
**然後那條線被拿去判一個還沒成熟的設定**。
**校準的樣本期間與適用的樣本期間不同，而那個「不同」躲在一個形容詞裡。**

這是 D048／D064／D069 那個家族的再一次，只是這次不在程式裡，在判準裡：
**工具（門檻）只在它被校準的那條路上是對的，而我們以為那代表它是對的。**

### 二、第一週量到了什麼（這些數字本身是可信的）

窗＝09-14～09-19（第一張長天期單成交起算，6 天，純新設定）。
跑 `scripts/sync_earnings.py --since 2026-09-14`：

| | 利用率 | 加權名目 | ×0.85 | 帳本實得 |
|---|---|---|---|---|
| 09-05～09-11（舊，對照） | 90.2% | 8.96% | 6.87% | **6.83%** |
| **09-14～09-19（新）** | 🔴 **51.1%** | **10.87%** | 4.72% | **5.13%** |

**分解與帳本對得上（差 0.41pp），名目確實拿到了 D069 想要的那 2pp。**
掉下來的是利用率，而利用率掉下來是因為等待變長：

| | 天期 2（n=10） | 天期 30（n=3～4） |
|---|---|---|
| 平均等待 | 1.89h | **15.64h**（8.3 倍） |
| 中位等待 | 0.13h | **14.55h**（112 倍） |
| 平均持有 | 27.15h（**下界**，5/10 被 48h 截斷） | 19.98h |

### 🔴 三、D069 賭的那件事，量到了，而且是反的

D069 第六節：「**案子成立靠的是截斷論證**（48h 上限拿掉後持有會變長）。」

上限已經從 48h 放到 720h。三筆已結束的長天期部位：
...
**沒有一筆碰到舊的 48h 上限，最長 32.17h。**
舊設定那 5 筆「借滿 48h」不是被上限擋住的長借款——**換一批更貴的借款人之後就消失了。**

`hold_report.py` 獨立給出同方向訊號（n=28）：
便宜組（< 9.11%）中位持有 **45.08h**／昂貴組（≥ 9.11%）中位 **11.62h**，差 33.46h。
👉 **決定持有時間的是價格，不是合約天期。**

⚠ **但 n=3，而且這正是 D063 警告的那種樣本**（全落在模型自己選出來的價位上）。
**這一節是「第一週的觀察」，不是結論**——結論等 10-14。

### 四、兩個副作用，都記著但都不動它

**① `estimate_wait()` 在天期 30 下低估 3.1 倍**（D069 第六節只預告 1.25 倍）

| 掛出 | 預估 | 實際 | |
|---|---|---|---|
| 09-14 01:01 | 5.53h | 18.16h | 3.3× |
| 09-14 21:34 | 5.38h | 8.28h | 1.5× |
| 09-16 14:12 | 5.11h | 25.00h | 4.9× |
| 09-18 16:34 | 4.20h | 10.95h | 2.6× |
| **合計** | **20.22h** | **62.39h** | **3.1×** |

**而同一支函式在天期 2 下是高估**（總量比 1.09×、最差 1377×）。
**偏誤方向隨天期翻轉**——這不是「除以一個數字」修得掉的，是 `W` 的校準（M2 的題目）。

**② `market_rate()` 在冷門天期會湊不到樣本**

`strategies/orderbook_depth.py` 的 `market_rate()` 只數 `period == offer_period` 的成交，
門檻 `min_trade_samples: 50`／`trade_window_hours: 6`。天期 30 是冷門天期，
於是 09-14 起觸發「近期成交樣本不足，本輪不掛單」**298 次**（前一整個月只有 4 次）。

**那支函式的 docstring 自己早就寫過這句**：「代價是天期冷門時樣本會不足，那時寧可不掛。」
**D069 翻旋鈕時沒有人回頭讀它。**

🔴 **但實際損失是 0**：298 次裡「當時錢是自由的（餘額 ≥ 150 USD）而真的被擋住沒掛單」
**0 次**——每一次觸發時錢要嘛在部位裡、要嘛場上已有掛單（149 次明確記錄
「場上已有 1 筆掛單，維持不動以保住排隊位置」）。**它是地雷，不是正在流血的傷口。**

### 五、排除掉「只是這週運氣差」

`market_snapshots` 每輪存**所有天期**的成交中位，與我們掛在哪無關：

| 期間 | period=2 成交中位 | period=30 成交中位 | period=2 簿子占比 | period=30 簿子占比 |
|---|---|---|---|---|
| 08-29～09-04 | 7.28% | 8.73% | 76.3% | 0.2% |
| 09-05～09-11 | 5.64% | 10.33% | 84.4% | 0.1% |
| 09-12～09-19 | **7.65%** | 10.84% | 71.4% | 🔴 **0.4%** |

**2 天期市場這週不但沒變差，還從 5.64% 漲到 7.65%。**

🔴 **而最右邊那一欄是整件事最便宜的一個數字**：天期 30 只佔簿子 **0.4%**，天期 2 佔 **71.4%**。
**這句 SQL 從 08-23（`market_snapshots` 第一列）起就查得到，不必花一週真金去問。**
👉 **往後改天期之前，先查該天期的簿子占比與成交筆數。**（進 TASKS）

### 六、決定：跑滿一個月，一行程式都不改

| | |
|---|---|
| **做什麼** | 維持 `offer_period: 30`、`assumed_hold_hours: 48` |
| **跑到** | **2026-10-14**（從第一張長天期單成交的 09-14 起滿一個月） |
| **D069 的硬停損 6.0%** | 🔴 **撤銷**（理由見第一節：它會在短天期自己都通不過的位置誤殺） |
| **D069 的軟判準 09-26 / 7.5%** | 🔴 **一併撤銷**（同一個病：拿成熟期的區間判未成熟期） |
| **期間動手的範圍** | **零**。每週跑一次 `sync_earnings.py` 看分解，只看不動手 |
| **只有這種情況會提前中止** | 「機器壞了」等級：連續 72 小時完全沒有成交，或單一部位被鎖超過 7 天 |

**為什麼連那個 `market_rate()` 的地雷也不修**：
**測試期間改程式，正是會讓測試不公平的那件事**——與第一節使用者糾正我的是同一個道理。
它實際損失是 0，等 10-14 之後再處理。

**代價算過了**：若長天期一直維持 5.13%，再跑 21 天與改回短天期（6.14%）相比
差 **0.20 美元**；若爬到 6.00%，差 **0.03 美元**。
**0.2 美元換一個對等的答案。**

### 🔴 七、10-14 要怎麼判（先寫下來，免得到時候又臨時拍一個）

**不看單一數字，看同一組 7 日窗的走勢**——短天期是 4.03% → 8.00% 爬上去的，
**題目是「長天期會不會也爬」**，不是「它現在是幾趴」。

| 判準 | 動作 |
|---|---|
| 長天期整月 **≥ 6.20%**（短天期整月的同期數字） | 🟢 留著，並開始做 D030 的正解 |
| 落在 **5.0%～6.20%** | 🟡 兩邊都不夠好——回到 M2／M3 的量測工程，不要再拍天期 |
| **< 5.0%**，或利用率整月 **< 55%** | 🔴 改回天期 2，並把這個月的資料餵進 M2 |

**對照組用「短天期整月 6.20%」而不是「最好的那一週 8.00%」**——
