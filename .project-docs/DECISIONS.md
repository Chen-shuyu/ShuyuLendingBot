...
| D061 | **選中哪個價位目前是由浮點雜訊決定的**：前兩名差 4×10⁻⁵ pp | 🔴 已確認的缺陷。成因有二：高原（8.3～9.0% 只差 0.19pp）＋ **平手系統性偏便宜**（`>` ＋升冪）。改法＝**高原容差 ＋ 取最高價**；**它不依賴 `P(r)`，是唯一可以先做的定價改動**。🟢 **2026-09-05 已實作**（`ev_plateau_tolerance_pct: 0.5`）——🔴 **量出來的答案跟直覺相反**：1.0% 贏 94/113 個起跑點卻**平均是負的**，且符號在持有模型與切半之間都會翻；**只有 0.5% 全部同向** |
...
### 實作結果（2026-09-05，分支 `feature/plateau-tolerance`）
