...
- **A2 重掛守門檻已退化成「永遠擋下往下調價」**。不是有人決定的，是連帶效果。
  **2026-08-18 拆成兩件事（D037）**：
  - **A2-a（仍未做，2026-08-19 順序調後）**：越界時**棄權**而非偷偷否決。
    這是修 bug，不選策略。原訂「連同閒置時間量測一起做」，
    **08-19 先做完了量測那一半（D038），A2-a 本身留到量測累積出資料之後**——
    它一放行就鬆開目前唯一擋著往下調價的東西，而 08-19 正是市場走弱的日子，
    手上還沒有閒置資料就放行是在最糟的時機賭一把。
  - **A2-b（移到第 2 期）**：新策略下的重掛政策，要在第 1 期的回測工具上跑過才定。
- **A3 排隊位置日誌變成截斷值**。與 A2-a 同根因、同批處理，仍未做。
  **2026-08-19 現場重現**：當下 250 檔可見最高只有年化 9.04%，而我們掛 9.78%，
