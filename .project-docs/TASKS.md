...
**狀態（2026-08-16 夜間第三段）**：main 在 `8fc022a`（PR #29 已合併並自動部署）。
分支 `fix/requeue-queue-position-guard` 待開 PR——**P1-4 完成，但做法與 D031 的
原規劃相反**（見 D034）：查證發現「前方 0 USD」是舊版 `<` 造成的假象、且日誌的主詞
一直是候選價位而非場上那張單；順著這條線推回去，19:31 那一輪要做的其實是
**把價格往下調**，與 21:31 的半價事故是同一個機制。原處方「前方金額低於門檻就不動」
實測方向錯誤，改為「往下調價要先證明划得來」。

**優先級 1 至此全部完成**，剩下的 P1-5（D032，期望值取代人工旋鈕）本次證明它比原本
想的更必要——沒有不需模型的捷徑。
...
- [ ] **P1-5　定價與天期改用期望值計算，取代人工旋鈕**（2026-08-16 新增，見 DECISIONS.md D032）
  ...
  - **策略每輪要能自己回答四個問題**（前兩個已經有了）：
    1. 市場利率水準在哪？（已有：讀訂單簿）
    2. 掛在候選價位，前面排多少錢？（已有：`describe_queue()`）
