...
- [ ] **驗證成交後：把定價正式改為市場百分位**（要寫 D028）
  - 每輪打公開端點 `/v2/trades/fUSD/hist`（不需簽章），取最近成交的第 90 百分位
    當 `base_rate`。這樣不管 FRR 飄到哪、市場漲跌，掛單自動落在「剛好還會成交的最高處」。
...
- [ ] **小事：容器日誌時間戳是 UTC，主機與文件是 CST，差 8 小時**
  - 對帳時很容易誤判（例如看到日誌停在 04:23 而以為機器人掛了，其實是 12:23 CST）。
    考慮讓容器帶 `TZ=Asia/Taipei`，或在日誌格式裡標出時區。
