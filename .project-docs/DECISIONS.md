...
| D058 | **使用者的假說：`P` 可能不是常數，而是 `r` 的函式**（越便宜借越久） | 🟡 **方向與資料一致**（`rho` = −0.29）**但強度遠不足**：n=14、只有 7 個相異利率，**同一個價位內部就差 38 倍**。列為待量測，接縫（`HoldModel`）早就在了 |
| D059 | **接下來的工作分三條車道**，判準是「會不會改變送到交易所的東西」 | 🟢 **排程判準**：甲道（行為零變化）隨時可做、乙道要等完整循環、丙道要拍板。🔴 觀察 D056 是以**循環**計不是以小時計——錢鎖在部位裡時 `pricing_decisions` 一列都不寫 |
