...
1. **讓錢真的借得出去**（P1）——P1-1（訂單簿排隊定價）、P1-3（文件變更不觸發部署）
   **已完成並由第一筆成交驗證**；**P1-6（成交價下限，D033）已完成**
   ——那是第二筆成交用半價借出去之後補上的防線；
   **P1-4（重掛判準，D034）已完成**——做法與原規劃相反，理由見上一節。
   **只剩 P1-5** 定價與天期改用期望值計算（D032，把等待時間成本搬進程式）。
   P1-2（天期）併入 P1-5 一起處理——寫死任何天期都是錯的架構。
   **P1-4 讓 P1-5 更必要**：本次證明重掛永遠是取捨、沒有不需模型的捷徑，
   而 `queue_clear_usd_per_hour`（只有一個校準樣本）正是 D032 要吃掉的第一個旋鈕。
