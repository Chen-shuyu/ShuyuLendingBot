...
**狀態（2026-08-16 稍晚）**：main 在 `f740ecc`（PR #27 已合併）。
分支 `feature/orderbook-pricing-and-fill-detection` 完成 **P1-1（訂單簿定價）**、
**P2-1（成交偵測）** 與 spread 地雷拆除，見 DECISIONS.md D030。測試 347 → 437 項。

**根因終於查明並修掉**：掛單一直落在**整個供給側的最後面**——舊策略掛 0.000272，
而當時簿子頂端就是 0.000272。新策略改以「排隊位置」定價，實測會掛 0.000250
（年化 9.12%）、前方排隊 73 萬 USD。**尚待實單驗證是否真的成交。**
...
機器人從上線至今近 100 輪從未出過事——自動重啟、健康檢查、失效告警、CI 部署、
日誌輪替全部運作正常；但它存在的理由是把錢借出去賺利息，而這件事**一塊錢都還沒發生過**。
下方的工作計畫就是照這個判斷排的。
...
- [x] **P2-1　成交偵測：機器人目前察覺不到自己借出去了**
  ...
  - **已知缺口（要追蹤）**：credits／loans 的欄位索引**取自官方文件，尚未經真實回應核對**
    ——探測當下帳號一筆都沒成交，兩個端點都是空清單。解析已寫成防禦式（欄位不足就跳過
    該筆並把原始內容寫進日誌）。**第一筆真實成交後要回來核對並更新註解**，
    這正是 B6／D027 的做法。
  - 以下為原始診斷紀錄，保留備查。
...
- [ ] **P5-1　清理已合併的殘留分支**：本地與遠端共 **23 條**已完全合併進 main 的分支未刪。
      另有 1 條 `fix/m1-frr-and-loop` 未被 main 完全包含，刪除前要先確認它的內容確實已進 main。
