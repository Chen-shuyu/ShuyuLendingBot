# PROGRESS 2026-07-26 ～ 2026-08-16

> **這份是歷史，不是現況。** 現在的狀態看 [`../STATUS.md`](../STATUS.md)，
> 最近的進度看 [`../PROGRESS.md`](../PROGRESS.md)（該檔頂端有涵蓋全部日期的索引表）。
>
> **涵蓋期間**：專案開工（2026-07-26）到真金上線後的第二天（2026-08-16）。
>
> **這裡面有什麼**：
>
> | 期間 | 內容 |
> |---|---|
> | 07-26 ～ 08-09 | M1～M4 的整個建置期：致命問題修正、策略與風控、資料與可觀測、容器化部署與 systemd 接管 |
> | 08-15 | **真金上線當天**（8 個段落）：分層搬遷、金鑰強化、LINE 接上、首次實單被拒、取消掛單失效 |
> | 08-16 | **436 行，全專案資訊量最大的一天**：查出定價基準用錯、時區統一、**第一筆成交**、用半價借出去（D033）、重掛判準改推導式（D034） |
>
