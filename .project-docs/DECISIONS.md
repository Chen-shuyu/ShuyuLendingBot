...
| D069 | **放寬合約天期到 30 天**（`offer_period` 2 → 30、`assumed_hold_hours` 12 → 48） | 🟢 **2026-09-12 上線**，是 D030 那句「驗證階段不要把資金鎖住」的**到期解除**，不是新發現。🔴 **先修正一個被用了一週的錯數字**：STATUS 的「實得年化 7.75%」是拿**入帳日數**當分母算的，誠實的 7 日滾動是 **6.83%**、上線至今 **6.14%**——D067 的重啟條件 3（跌破 7%）**其實已經成立兩週**。🔴 **9% 的目標從一開始就差一個 0.85**：Bitfinex 抽 15%，9% 淨年化需要 10.59% 名目 ＋ 100% 利用率。**選價 9.125% → 10.500%**（高原，命中 46 次；**不是**那個命中 7 次的 11.268%）。⚠ **賭的是持有時間會變長，而那件事現在量不到**（24 筆有 9 筆被 48h 上限截斷，KM 在 48.6h 仍有 37.5% 存活）。🔴 **K 線缺根讓等待被低估 1.25 倍**，修正後損益兩平點 15.4h → **19.2h**，落在實測中位（16.29h）之上、實測平均（23.24h，本身是下界）之下——**餘裕很薄，靠的是截斷論證**。停損：7 日滾動跌破 **6.0%** 就改回天期 2 |
