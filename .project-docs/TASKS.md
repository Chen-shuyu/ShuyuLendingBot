...
**狀態（2026-08-19 23:45）**：main 在 `da546c2`——**PR #33（D038）已合併並完成部署**。
容器 23:44:55 重啟、healthy、部署後無任何 WARNING／ERROR，新表 `offer_wait_forecasts`
已建立。測試 535 項全過（單元＋功能＋整合）。**手上沒有進行中的分支。**

新舊版同一分鐘的日誌對照（部署前最後一輪 vs 部署後第一輪）：
...
最後那句「沒有留下當初的等待預估」正是設計要的——那張單是新表出現前掛的，
所以它誠實說不知道，而不是拿今天的模型硬湊一個數字回填。

**資金狀態**：344.36 USD **掛在場上未成交**（掛單 `5084375241`，年化 9.78%，
2 天期），**已閒置 18.1 小時**（05:03:24 掛出）。
前一筆部位 `464047005` 於 **08-19 05:03:19 被提前還款**（原定 08-20 22:11 到期，
提前 41 小時，實際只借 6 小時 52 分）。

**撐住的損益兩平點是等待 38.6 小時**（約 08-20 19:40）：在那之前成交仍勝過
「當初就用 5.47% 賣掉」。**部署不會動到那張單**——新算法這一輪同樣選 9.78%，
`_plans_match()` 的 2% 容差判定一致，不會取消重掛。
...
| 2 | **A2-a ＋ A3** | 修 bug，不需要市場知識；但要等順位 1 的資料先跑出來 |
...
- [ ] **A2　重掛守門檻（D034）已退化成「永遠擋下往下調價」**
  ...
  - **一個永遠給同一個答案的判斷式，等於沒有判斷式**——差別只在它看起來像有。
    這是 D026「靜默失效」的第四次現身。
- [ ] **A3　排隊位置日誌現在是截斷值，不是量測值**（**2026-08-19 現場重現**：
...
- [ ] **B9　兩處註解與現況矛盾**（**2026-08-19 刻意留著**：使用者當次指示「不要動到程式」，
      而這兩處都在原始碼裡。**第一條已在 08-19 拿到現場證據**——當天 250 檔供給側
      總額只有 1,306,715 USD，離「約 500 萬」差很遠。）
