...
- 附帶發現（**已於 2026-08-24 修正**，原記為 TASKS.md B5，見 CHANGELOG 的
  Fixed 2026-08-24）：ccxt 把這個**餘額不足**的錯誤歸類成 `AuthenticationError`，
  於是日誌寫的是「建立放貸掛單認證失敗」。訊息會把人引去查 API 金鑰，
  而真正的問題在金額——而且分類成 `FatalError` 會讓機器人**永久停止**。
  修法是**分類看訊息、不看型別**（型別在這裡本來就不可信），歸為 `SkipCycleError`。
