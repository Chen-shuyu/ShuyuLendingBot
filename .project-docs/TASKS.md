...
**狀態（2026-08-18 22:39）**：main 在 `205a757`——PR #32（盤查與路線圖重排）與
A1（定價決策進日誌）都已合併，CI 四個 job 全綠，容器 22:39:02 重啟並確認跑新版。
**手上沒有進行中的分支。**

**資金狀態**：344.36 USD 借出中（部位 `464047005`，年化 9.96%，2 天期），
**到期 2026-08-20 22:11**，也可能被提前還款。資金回來的那一刻機器人會重新掛單。

### 🔴 下一批工作（2026-08-18 定序，理由見 D037）

| 順位 | 項目 | 為什麼是這個順序 |
|---|---|---|
| 1 | **A2-a ＋ A3 ＋ 閒置時間量測** | 修 bug，不需要市場知識，不受 D036 等待條款約束 |
| 2 | **M1 市場資料落地** | 08-18 那筆是史上資訊量最大的觀測，卻只留下四行日誌 |
| 3 | **P2-2 `earnings_daily` 接資料源** | 三筆部位跑完了還是答不出「賺了多少」 |

**⚠️ 順位 1 的時效性**：資金 08-20 22:11 前後會回來，屆時機器人重新掛單，
A2-a 的行為才會再次有機會生效。**要修就趕在資金回來之前，否則又要多等一個週期
才驗得到。**（而且部署重啟在資金借出期間是零成本——現在正是這個視窗。）
