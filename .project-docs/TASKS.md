...
**狀態（2026-08-16 夜間）**：main 在 `123c606`（PR #28 已合併並自動部署）。
分支 `docs/first-fill-and-pricing-roadmap` **只做文件規劃，不動程式碼**（使用者指示）。
...
- [x] **P1-6　🔴 定價補上成交資料源：我們用半價把錢借出去了**（2026-08-16 夜間，見 DECISIONS.md D033）
  ...
  - **已做**：新增 `get_recent_trades()`；常態成交價 = 同天期成交的**金額加權中位數**；
    ...
    日誌加一行「市場常態成交價」；`minimum_rate` 0.0001 → 0.00018。
  ...
  - **驗收**：把那道牆放回真實簿子重演——舊版掛 0.00014999（5.47%），
    新版掛 0.00021590（7.88%）；沒有牆時下限不生效。測試 469 → 477 項。
