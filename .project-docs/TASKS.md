...
**狀態（2026-08-15 收工）**：**機器人已在真金模式常駐運作**，main 在 `e266821`，
工作區乾淨。目前掛單 `5081124199`（160 USD、0.000523/日、2 天期），**尚未被借走**。
今日零損失。

明天的起點：先確認掛單是否成交（見下方「明天第一件事」），再決定要不要調 `premium_rate`。

（今日上線四條分支：#19 LINE 通知 D024、#20 go-live、#21 金額四捨五入 D025、
#22 取消掛單 id 型別 D026。M4 四個 milestone 全部完成。）

## 🟡 明天第一件事

- [ ] **確認掛單有沒有成交，並決定是否調低 `premium_rate`**
  - **現況**：掛單利率 = FRR + `premium_rate` = 0.00032 + 0.0002 = **0.000523/日
    （年化約 19%）**，而 fUSD 訂單簿上的市場掛單落在 **0.000142 ~ 0.00028**。
    我們的單排在明顯偏上的位置，**很可能一直借不到**。
  - **怎麼確認**：查 funding credits 有沒有紀錄，或看 Bitfinex 網頁的「已提供的融資」。
  - **借不到的話有兩條路**：
    1. **調低 `premium_rate`**（例如 `0.0002` → `0.00005`，掛單利率降到 0.00037 附近）。
       最直接，但要注意這是全域參數，之後金額變大時同樣適用。
    2. **改用 FRR 浮動利率掛單**（`type="FRRDELTAVAR"`）。Bitfinex 支援跟著 FRR 浮動的
       掛單型別，成交機率高、也不需要每輪重掛來追市場——但那會動到
       `create_loan_offer()` 的掛單型別與整個「每輪全取消重掛」的設計前提（D011），
       是策略層的改動，不是調參數。**要做的話值得先寫一則 DECISIONS 再動手。**
  - **為什麼不急著今天決定**：掛著沒成交不會有損失，只是資金閒置。
    而且「成交 → 產生利息 → `earnings_daily` 落帳」這條路徑本來就還沒接
    （見下方 M3 延後項），先想清楚再改比較划算。
