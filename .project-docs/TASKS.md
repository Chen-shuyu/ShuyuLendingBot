...
**狀態（2026-08-16）**：main 在 `4d1457d`，工作區乾淨。機器人真金運作中，
今日合併三條 PR：#24（定價驗證）、#25（時區統一 D028），另 #23 為昨日文件同步。
掛單利率已由 0.000522 降到 0.000273（年化 9.97%），**但仍未成交**。
...
- [ ] **P1-1　定價改為「跟著市場成交價走」**
      （要寫 ADR，取下一個未使用的編號——D028 給時區、D029 給訊息規範用掉了）
  - **白話**：現在的算法是「FRR 加減一個固定值」，而 FRR 是**落後指標**——它是過去一段
    時間的平均，市場跌了它還停在高點，我們就掛太高借不到；市場漲了它又跟不上，
    我們就掛太低少賺。目前用負數硬壓只是權宜之計，FRR 一飄就又跑掉。
  - **做法**：每輪打公開端點 `/v2/trades/fUSD/hist`（不需簽章），取最近成交的百分位
    當 `base_rate`；`minimum_rate` 保留為底線防賤賣，FRR 降級為純參考值。
  - **百分位取中位數附近（50～70），不是 90**——理由見下方「市場定價驗證」的陣發分析。
- [ ] **P1-2　天期 `short_duration` 由 2 天改為 7 天**
  - **白話**：同樣的錢，借 3～7 天的利率比借 2 天高約 0.5 個百分點（9.95% vs 9.39%），
    因為短天期的競爭更激烈。幾乎是白撿的，代價只是資金多鎖幾天。
...
- [ ] **P2-1　成交偵測：機器人目前察覺不到自己借出去了**（2026-08-16 讀程式碼時發現）
...
- [~] **P2-4　交易面通知擴充：掛單／成交／日結**
      —— **掛單那一半已於 2026-08-16 完成**（分支 `feature/notify-format-and-trade-events`，
      見 D029）。**成交與日結仍待 P2-1／P2-2**，所以標「部分完成」。
...
- [ ] **P3-2　`maxtolend` 由「單輪量控版」升級為「含已放貸的真實總曝險版」**
