...
**狀態（2026-08-23 12:00）**：分支 `fix/stale-cross-round-state` 完成
...
是漏記。**但 D040 的新程式碼仍未被執行到**——`_log_hold_times()` 要等部位收回，
而部位 `464242253` 到現在還開著（已借出 36.7 小時，最晚今晚 22:31 到期）。
**連續第三次「部署完成 ≠ 新程式碼被執行過」。**
...
（手算，`earnings_daily` 仍是空表）。

**資金狀態**：344.41 USD **借出中**（部位 `464242253` @ 年化 9.50%、2 天期，
08-21 22:31:00 起），最晚 08-23 22:31 到期。**場上沒有我們的掛單——
這正是部署成本為零的視窗**（同 08-18、08-20 兩次的判斷）。
