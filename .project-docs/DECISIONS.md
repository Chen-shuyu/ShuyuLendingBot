...
**8. `minimum_rate` 由 0.0001 提高到 0.00018**（年化 6.57%）。它的角色是**絕對地板**，
只擋「連成交資料都算出離譜低價」的狀況；正常情況下不會擋到任何東西，
因為歷史成交帶是 8.68%～10.00%。代價是市場真的跌破年化 6.57% 時機器人會整天不掛單。
...
新版會掛            ：0.00021590  年化 7.88%
```

同一時刻沒有牆的真實簿子上，下限不生效，排隊規則照常運作（掛 0.00031799，
年化 11.61%）——**下限只在被賤賣時才介入**。測試 469 → 477 項。
