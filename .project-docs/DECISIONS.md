...
| D064 | **驗收工具靜默失效了六天**：`--verify` 拿今天的設定去重跑當初的決策 | 🟢 已修（`pricing_knobs_json` ＋ 每列用自己的設定重播，**40 列 → 40/40 一致**）。🔴 **D048 的同一個病第二次現身**——這次更難發現，因為它從頭到尾都有輸出，而「不一致」跟「工具壞了」長得一模一樣。**驗收標準本身不會被驗收** |
...
| D067 | **等更多單子不會改變答案，而探索的檢定力差了 20 倍** | 🟢 **停在這裡的決定**，三條獨立依據：①新樣本全落在便宜組（D056 後昂貴組 **+0 段**）；②**連「每筆都借滿 48h」都不能讓最佳 `P` 離開 12**——`assumed_hold_hours` **不是持有時間估計，是「多討厭等待」的旋鈕**；③沒有不花錢的替代路徑（345 USD 佔成交量 0.0002%）。🔴 原規劃「每 20 次探索 1 次」湊到 70 段要 **50 個月**。**重啟條件寫在 ADR 裡** |
