...
**狀態（2026-08-16）**：機器人真金運作滿 12 小時，健康狀態全綠（心跳正常、連續失敗 0、
12 小時內零 ERROR），但**掛出的 78 輪掛單一筆都沒成交**。零損失，也零收益。
根因已查明並非參數大小的問題，見下方「市場定價驗證」。

目前分支 `tune/market-rate-validation`：`premium_rate` 暫設負值做成交驗證。
...
- [ ] **驗證成交後：把定價正式改為市場百分位**（要寫 D029；D028 已給時區用掉）
