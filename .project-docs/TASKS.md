...
**狀態（2026-08-21 00:20）**：分支 `fix/honest-queue-position` 已完成
**A2-a ＋ A3 ＋ B9**（見 DECISIONS.md D039），測試 509 → 520 項（單元＋功能）、
整合 13 項（not live）全過，並以真實簿子與正式 `config.yaml` 各驗收一次。
**PR #34 已合併並完成部署**（main 在 `277620f`，容器 2026-08-21 00:34:05 重啟、
healthy、部署後 0 個 WARNING／ERROR，容器內檔案與 main 的 sha256 一致）。
**但新程式碼的關鍵路徑到現在一次都沒被執行過**——錢還借在外面，餘額低於下限，
策略整輪不產生計畫，而排隊位置日誌與守門檻都在「有計畫」之後才走得到。
真正的上線驗證要等資金回來（最晚 08-22 19:10）。

**合併後的盤查抓到 D4**（見下方）：D039 的棄權理由會指錯對象。
**PR #35 已合併並完成部署**（main 在 `66e7d3c`，容器 2026-08-21 01:09:09 重啟、
healthy、部署後 0 個 WARNING／ERROR，容器內檔案與 main 的 sha256 一致）。
**手上沒有進行中的分支。**

**兩次部署都還沒真正驗證到新程式碼**：資金 08-20 19:10 借出後一直沒回來，
餘額 0.01 USD 低於 150 下限，策略整輪不產生計畫——而排隊位置日誌與守門檻
都在「有計畫」之後才走得到。**資金回來的那一輪才是真正的上線驗證**
（部位 `464168644` 最晚 08-22 19:10 到期，也可能提前還款）。

**2026-08-20 的成交**：15:15:21 掛出 344.41 USD @ 年化 9.50%、2 天期，
**19:10:59 成交**，等待 3.93 小時——對照掛單當下的預估（平均 6.1h／中位數 3.5h／
四分之三在 10.0h 內），這是 D038 量測上線後**第一組「預估 vs 實際」**，落在中位數
附近。但整段週期要誠實看：資金 08-19 05:03 就回來了，**空轉 38.1 小時**，
即使借滿 2 天，週期實質年化只有 **5.29%**，而當時立刻賣掉是 5.42%。

**首次成交至今 99.5 小時，資金使用率 57.9%、整體實質年化 3.70%**
（手算，`earnings_daily` 仍是空表）。

**資金狀態**：344.41 USD **借出中**（部位 `464168644` @ 年化 9.50%、2 天期，
08-20 19:10:59 起），最晚 08-22 19:10 到期。**場上沒有我們的掛單——
這正是部署成本為零的視窗**（同 08-18 那次的判斷）。
...
| 3 | **M1 市場資料落地** | 08-18／08-19 兩次最有資訊量的觀測都只留下幾行日誌 |
| 4 | **P2-2 `earnings_daily` 接資料源** | 三筆部位跑完了還是答不出「賺了多少」 |
