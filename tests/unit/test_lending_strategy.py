# -*- coding: utf-8 -*-
"""`modules/lending_strategy.py` 的單元測試。
...
from modules.lending_strategy import LendingStrategy, OfferPlan
...
class TestMinimumThreshold:
    ...
    def test_balance_below_min_required_returns_empty(self, strategy_config):
        strategy = LendingStrategy(strategy_config(min_required_usd=150))
    ...
    def test_balance_exactly_at_min_required_still_lends(self, strategy_config):
        ...
        strategy = LendingStrategy(strategy_config(min_required_usd=150, min_loan_size_usd=150))
    ...
    def test_zero_balance_returns_empty(self, strategy_config):
        strategy = LendingStrategy(strategy_config())
    ...
    def test_lendable_below_min_loan_size_returns_empty(self, strategy_config):
        ...
        strategy = LendingStrategy(strategy_config(min_required_usd=100, min_loan_size_usd=150))
...
class TestRateLadder:
    ...
    def test_base_rate_is_frr_plus_premium(self, strategy_config):
        strategy = LendingStrategy(strategy_config(spread_count=1))
    ...
    def test_negative_frr_still_respects_minimum_rate(self, strategy_config):
        ...
        strategy = LendingStrategy(strategy_config(spread_count=1, minimum_rate=0.0001))
    ...
    def test_zero_step_pct_gives_flat_ladder(self, strategy_config):
        strategy = LendingStrategy(strategy_config(spread_step_pct=0.0))
...
class TestSpreadCount:
    ...
    def test_full_count_when_balance_is_enough(self, strategy_config):
        strategy = LendingStrategy(strategy_config(spread_count=3, min_loan_size_usd=150))
    ...
    def test_every_offer_meets_min_loan_size(self, strategy_config):
        ...
        strategy = LendingStrategy(strategy_config(spread_count=4, min_loan_size_usd=150))
...
class TestDuration:
    ...
    def test_low_rate_uses_short_duration(self, strategy_config):
        strategy = LendingStrategy(strategy_config(spread_count=1, long_duration_threshold=0.00082))
    ...
    def test_each_offer_decides_its_own_duration(self, strategy_config):
        ...
        strategy = LendingStrategy(strategy_config(long_duration_threshold=0.0005))
        plans = strategy.build_offer_plan(600.0, 0.0002)
...
class TestLendLimit:
    ...
    def test_absolute_limit_caps_total(self, strategy_config):
        strategy = LendingStrategy(strategy_config(max_to_lend_usd=300))
        total = sum(plan.amount for plan in strategy.build_offer_plan(1000.0, 0.0002))
        assert total == pytest.approx(300.0)

    def test_percent_limit_caps_total(self, strategy_config):
        strategy = LendingStrategy(strategy_config(max_percent_to_lend=20))
        total = sum(plan.amount for plan in strategy.build_offer_plan(1000.0, 0.0002))
        assert total == pytest.approx(200.0)

    def test_both_limits_take_the_smaller(self, strategy_config):
        strategy = LendingStrategy(strategy_config(max_to_lend_usd=300, max_percent_to_lend=20))
        total = sum(plan.amount for plan in strategy.build_offer_plan(1000.0, 0.0002))
        assert total == pytest.approx(200.0)

    def test_limit_never_exceeds_actual_balance(self, strategy_config):
        """上限設得比餘額高時，仍以實際餘額為準。"""
        strategy = LendingStrategy(strategy_config(max_to_lend_usd=99999))
    ...
    def test_limit_below_min_loan_size_blocks_lending(self, strategy_config):
        ...
        strategy = LendingStrategy(strategy_config(max_to_lend_usd=100, min_loan_size_usd=150))
